...
- **Session close-out**: `docs/agents/session-closeout-sop.md` — the hands-off WRITE/READ/EXECUTE chain (successor next-goal strict v2 + validator/doctor, retention, LATEST symlink) with the devops CLI command forms; the spec is the `self-reflect-next-goal` ext skill alone.
