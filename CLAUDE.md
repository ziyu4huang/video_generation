...
- **Registration**: ONE entry in `bun-apps/s2-agent/src/registry-config.ts` (`REGISTRY`; `load: dynamic` or `static` — never both), then `bun run --cwd bun-apps/s2-agent regen:manifest` (+ `regen:static` for static). `src/run-dir/manifest.json` is DERIVED (freshness-gated — never hand-edit).
...
- **Issues**: GitHub Issues (`ziyu4huang/video_generation`) via `gh` — see the `issue-tracker` skill (`bun-apps/s2-agent-ext-devops/skills/issue-tracker/SKILL.md`).
- **Session close-out**: the `session-closeout-sop` skill (`bun-apps/s2-agent-ext-devops/skills/session-closeout-sop/SKILL.md`) — the hands-off WRITE/READ/EXECUTE chain (successor next-goal strict v2 + validator/doctor, retention, LATEST symlink) with the devops CLI command forms; the spec is the `self-reflect-next-goal` ext skill alone.
- **Domain docs**: each domain owns `CONTEXT.md` + `docs/adr/` (root `CONTEXT-MAP.md` lists contexts; see the `domain-docs` skill, `bun-apps/s2-agent-ext-devops/skills/domain-docs/SKILL.md`). Cite ADRs as `ADR-<context>-NNNN`, never bare numbers — contexts number independently. `bun run test:adr` (from `bun-apps/`) blocks unresolved citations.
