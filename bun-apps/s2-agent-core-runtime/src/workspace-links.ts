 ...
 *
 * CORE-RUNTIME COPY (self-arc-22 t01): check-deps.ts runs `bun install` as the
 * boot self-heal — and that install is what re-breaks the links — so the
 * repair must live on THIS side of the dependency direction (devops imports
 * core-runtime, never the reverse). The devops deploy preflight keeps its own
 * pre-deploy call; do not let the two drift in SEMANTICS.
 *
