 ...
 * the same way in every mode s2-agent runs in.
 *
 * WHY THIS EXISTS
 * ---------------
 ...
 * ever was. `bun build --compile` got its `$bunfs` special case (that mode is
 * retired — deleted with #1866, its special case with crossos-deploy ticket
 * 07, 2026-08-27); the sh deploy did not. A deployed extension's dir is a
 * perfectly ordinary path under
 ...
 * The other mode is recognised POSITIVELY — by an artifact its build wrote —
 * never by guessing at the shape of a path:
 *
 ...
 *
 * Keep this the only implementation. `extension-dep-probe.test.ts` in s2-agent
...
/**
 * True when `dir` is an extension directory inside a s2-agent-sh deploy.
 *
