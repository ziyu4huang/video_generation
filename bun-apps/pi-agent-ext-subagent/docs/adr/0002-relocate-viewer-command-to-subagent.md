...
`subagent-progress-widget` in `pi-agent-ext-workflow`. The blocker was a
...
Moving the viewer into `pi-agent-ext-subagent` would have dragged `display.ts`
along, and `display.ts ⟹ workflow.ts` would have pulled the workflow engine
back in — re-creating the monolith with a cycle.

## Decision

**Move the viewer, command, and progress-widget into `pi-agent-ext-subagent`,
...
hub, and nothing in `pi-agent-ext-subagent` imports it.

## Consequences

- **The `/subagents` TUI is self-contained in this package.** A host that loads
  only `pi-agent-ext-subagent` gets working tools AND the viewer/command — no
  workflow dependency.
