...
Add **Loop 2** — a `/list` goal queue — to `pi-agent-ext-core-task`. Multiple objectives queue in a **tail**; the active goal is the **head**. Completing the head auto-promotes the next tail item; a stuck/paused head can be manually advanced (`/list next`, lossless park). Reuses the existing state machine + session-store + inline audit with **no new status states** — a queued item is not a goal yet (it has no status; it becomes `active` via `createGoal` on promotion).
