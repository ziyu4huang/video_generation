...
**Spec:** `bun-apps/pi-agent-ext-core-task/docs/2026-07-25-goal-loop-hardening.md` (D1 = medium split, D2 = all-three hardening).
...
- **Test gate after every task:** `( cd bun-apps/pi-agent-ext-core-task && bun test )` green **and** `bun run typecheck` clean.
...
- [ ] **Step 2: Run — verify fail** — `( cd bun-apps/pi-agent-ext-core-task && bun test src/goal/__tests__/overflow.test.ts )` → FAIL (`cannot find module ../overflow.js`).
...
- [ ] **Step 4: Run full gate** — `( cd bun-apps/pi-agent-ext-core-task && bun test && bun run typecheck )` → all green.
...
> **🛑 Phase 1 checkpoint** — `( cd bun-apps/pi-agent-ext-core-task && bun test && bun run typecheck )` green, `goal.ts` ≈ 500 lines, zero behavior change. Review before Phase 2.
