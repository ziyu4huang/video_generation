 ...
 * wiring seam. See guarded-invalidate.ts for the full rationale.
 *
 ...
 * from StatusPanelOverlay. The existing no-op invalidate() STAYS in place; this
 * guard is additive belt-and-suspenders so a FUTURE overlay reintroducing the
 * bug cannot crash the agent with a RangeError cascade.
 *
 * Mocking style mirrors panel.test.ts's "tuiInvalidate -> overlay.invalidate"
 ...
 * violates the cache-bust contract the way StatusPanelOverlay did pre-e411f7fa.
...
// the panel is open. The cascade terminates instead of recursing to RangeError,
...
test("guard neutralizes a reentrant overlay on state-change invalidateFn() — propagates exactly once", () => {
  ...
  assert.doesNotThrow(() => guardedFn());
  assert.equal(count(), 1); // exactly one propagation; reentry suppressed
});

// Companion documenting test: the RAW pre-guard wiring (what both editors used
...
test("UNGUARDED wiring with a reentrant overlay recurses to RangeError — documents why the guard exists", () => {
  ...
  assert.ok(threw instanceof RangeError, "unguarded reentrant overlay must recurse to RangeError");
});
