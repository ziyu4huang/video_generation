# s2-agent-ext-webui — Architecture v2 (re-design)
...
> specs (`.planning/done/2026-08-10-s2-agent-ext-webui-from-scratch/specs/`),
...
Make the s2-agent **TUI optionally use the webui** to render agent output and
...
1. **Optionality** — the webui becomes opt-out-able (env `WEBUI_DISABLED`,
   `deps.enabled`) and the s2-agent CLI gains `--no-webui` / `--webui-port N`.
...
8. s2-agent CLI flags (--no-webui / --webui-port) + cli-argv tests.
...
- **Files added**: `src/webui-config.ts` (optionality gate), `src/session-store.ts`
  ...
  `tests/session-store.test.ts`, `tests/cli-argv.test.ts` (s2-agent).
- **Files changed**: webui-wiring (gate, store wiring, watchdog sync, session
  ...
  index.ts (real lib entry), package.json (description), s2-agent cli.ts +
