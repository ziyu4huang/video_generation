...
- **s2-agent CLI** (TUI path): `bun bun-apps/s2-agent/src/cli.ts --no-webui` disables it;
...
## webui v3 — pure HITL companion (tabs)
...
- **Diagram** — archify artifacts, full-fidelity, in the shell (see below).
...
`snapshot`, `appexec`, `view_opened`, `diagram_deck`. TUI-only (never broadcast): `message_*`,
...
- Config (`src/webui-config.ts` → `resolveFileRoots`): wiring `fileRoots` (e.g.
  ...
  default none.
- Cross-package example: `s2-agent-ext-archify` emits `webui:open` after every successful
...
## Diagram pane + the `webui:deck` event

Rendered diagrams get a first-class surface in the shell instead of only a link that opens a
separate browser tab. The **Diagram** tab (`#deck`) shows one artifact at a time with slide
navigation, and its fidelity comes from pointing an iframe at the EXISTING `/files` route —
whose `Content-Security-Policy: sandbox allow-scripts allow-downloads` lets archify's own
runtime (theme toggle, hover-trace, export menu) run under an opaque origin. **No `srcdoc`,
no new transport, no weakened CSP.**

- **Everything is a deck.** `diagram_deck` frames arrive as named decks; single `view_opened`
  renders accumulate into a synthetic "Recent renders" deck, so the pane has ONE model. A
  single archify render therefore reaches the pane with **zero archify changes** — no
  `diagram_open` frame was needed, because `view_opened` already carries `{view,title,url,ts}`
  and is already replayed.
- **`webui:deck` event** (`src/deck-event-handler.ts`) — ANY extension may emit
  `{deckId, title?, slides:[{path, title?, subtitle?}]}` on the shared `pi.events` bus. Every
  `path` is validated through the SAME `locateFileInRoots` containment core the `/files` route
  uses, so route and event can never disagree about what is servable. Two deliberate
  differences from `webui:open`: a **partially-servable deck keeps its servable slides**
  (losing a whole collection because one path was misconfigured is worse than losing the one),
  and it does **not** `ui.notify` (the per-render open announcements already bell the TUI). It
  never throws; every ignore path logs a reason.
- **`diagram_deck` frame** carries RESOLVED `/files` URLs (never raw paths — the `view_opened`
  precedent) and is replay-eligible, so a refresh restores the deck and the active slide.
  `deckId` is stable identity: re-exporting the same deck replaces it rather than stacking.
- **Thumbnails**: when the producer supplies a `thumb` path per slide, the rail shows the
  image instead of just the title — validated through the SAME containment core as the slide
  itself, so an unservable thumbnail drops silently and never costs you the slide. The images
  are generated by the PRODUCER (archify, via `Bun.WebView`), not here: webui has no runtime
  rendering dependency and this feature does not add one.
- **Controls**: prev/next + `←`/`→` (active pane only, never while typing), a slide
  rail, `fit`/`actual` zoom, and the same two escape hatches the Report tab proved —
  `fullscreen` and `open standalone` (a sandboxed frame has an opaque origin, so the parent
  can never measure or script it). A loading veil covers the swap, because a rendered artifact
  is ~600 KB and a blank stage reads as broken.
- **Producer**: `s2-agent-ext-archify` emits `webui:deck` after every PPTX export, pointing at
  the interactive slide HTML it keeps beside the `.pptx`. webui imports nothing from archify
  and vice versa — the string-literal channel is the whole contract, pinned on this side by a
  verbatim-captured payload fixture in `tests/deck-event-handler.test.ts`.
