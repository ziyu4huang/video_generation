 ...
 *
 * Load (source mode):
 *   bun bun-apps/s2-agent/src/cli.ts -e bun-apps/s2-agent-ext-flux2/extensions/flux2.ts -p "..."
 * Bundle:
 *   bun scripts/build-bundle.ts  →  dist/pi-extensions/s2-agent-ext-flux2.bundle.js
 *
...
function scenePipelineDoc(): string {
  return (
    ...
    "(default: central capabilities.vision from model-tiers.json).\n" +
...
function selfImproveDoc(): string {
  return (
    ...
    "  bash bun-apps/s2-agent/scripts/run-self-improve-loop.sh --pose-id <id|L1-01..L4-02> " +
    "[--attempts N] [--seed S] [--steps N]   # pose library → pose_dsg judging\n" +
    "  bash bun-apps/s2-agent/scripts/run-self-improve-loop.sh --prompt \"<text>\"            " +
    "# non-pose → holistic score judging\n" +
    "Poses live in bun-apps/s2-agent-ext-flux2/workflows/poses.json. The runner prints a " +
