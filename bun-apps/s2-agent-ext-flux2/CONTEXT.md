...
The ubiquitous language of s2-agent-ext-flux2 — an agent-optimized wrapper around the `flux2` pure-Swift/MLX image generator (Flux2 Klein 9B + SAM3.1). Exposes flux2's 18 subcommands through one dispatcher tool so an agent can generate, gate, and chain images without memorizing flags.
...
_Avoid_: subcommand (ambiguous — see s2-agent), action
