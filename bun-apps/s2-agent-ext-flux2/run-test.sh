#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for s2-agent-ext-flux2.
...
print_list() {
	cat <<EOF
$(Y "s2-agent-ext-flux2 run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/s2-agent-ext-flux2-runtest.log 2>&1
	rc=$?
	...
	if [ "$rc" -ne 0 ]; then sed 's/^/      /' /tmp/s2-agent-ext-flux2-runtest.log | tail -n 25 >&2; fi
}
...
echo "$(Y "▶ s2-agent-ext-flux2 run-test.sh — tier=$TIER")"
case "$TIER" in
