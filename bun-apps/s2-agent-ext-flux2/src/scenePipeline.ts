 ...
 * `resolveLLM`, defaulting to lm-studio/prism-ml/bonsai-27b) — the same
 * subagent plumbing pi-file2md itself and `s2-agent cli` already use, so this is
...
export interface ScenePipelineOptions {
   ...
   */
  verifyMatch?: string[];
  /** "provider/modelId" override for the VLM subagent. Default: lm-studio/prism-ml/bonsai-27b. */
  vlmModel?: string;
