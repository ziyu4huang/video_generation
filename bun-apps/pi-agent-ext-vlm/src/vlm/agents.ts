 ...
 *
 * The model defaults to `lm-studio/google/gemma-4-26b-a4b-qat` (a vision-capable
...
/** Common frontmatter/footer rules shared by all profiles. */
const COMMON_RULES = `
共同規則：
- 用繁體中文「整理與摘要」；但忠實度優於改寫：定義、方法步驟、演算法、實驗設定與數據、關鍵引用，一律原樣保留原文（可直接以原文引述），避免因翻譯或過度濃縮而流失資訊。專有名詞、人名、術語、公式保留原文。
...
const PAPER_SYSTEM = `你是一個專門把學術論文 PDF 頁面轉成結構化 Obsidian 筆記的 VLM 助理。
...
const SLIDES_SYSTEM = `你是一個把簡報投影片轉成結構化 Obsidian 筆記的 VLM 助理。
...
const POSTER_SYSTEM = `你是一個把學術/會議海報轉成結構化 Obsidian 筆記的 VLM 助理。

${COMMON_RULES}
- kind 欄位固定為 poster。
- 正文依海報區塊（如 Abstract / Introduction / Methods / Results / Conclusion）分段整理；圖表逐一說明標題與結論。`;

const DIAGRAM_SYSTEM = `你是一個把圖表 / 流程圖 / 架構圖轉成結構化 Obsidian 筆記的 VLM 助理。

${COMMON_RULES}
- kind 欄位固定為 diagram。
- 正文：說明圖的類型與目的；以文字還原流程/架構（可用條列或巢狀清單）；記錄所有標籤、節點、箭頭方向與註解。若有數學式則用 LaTeX。`;

/** System prompt for generic standalone images. */
const IMAGE_SYSTEM = `你是一個把單張圖片內容轉成結構化 Obsidian 筆記的 VLM 助理。
...
/** Resolve the system prompt for a profile. */
export function systemPromptFor(profile: DocProfile): string {
  switch (profile) {
    case "paper": return PAPER_SYSTEM;
    case "slides": return SLIDES_SYSTEM;
    case "poster": return POSTER_SYSTEM;
    case "diagram": return DIAGRAM_SYSTEM;
    case "image": return IMAGE_SYSTEM;
    default: return IMAGE_SYSTEM;
  }
}
...
export async function explainPage(
  ...
  const { session } = await createSharedSession(llm, {
    appendSystemPrompt: [systemPromptFor(profile)],
  });
