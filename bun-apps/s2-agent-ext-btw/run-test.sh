#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for s2-agent-ext-btw.
...
print_list() {
	cat <<EOF
$(Y "s2-agent-ext-btw run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/s2-agent-ext-btw-runtest.log 2>&1
	rc=$?
