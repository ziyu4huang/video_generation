# Development History — pi-agent Patches
...
pi 的 `version-check.js` 在啟動時 fetch `https://pi.dev/api/latest-version`，若有新版則印 "Update Available" 提示 `pi update`。對 pi-agent bundle/binary 這是無意義的：
...
真實的 pi TUI 只從 `~/.pi/agent/settings.json` 讀取 default model/provider，不認 `PI_MODEL` / `PI_PROVIDER` / `PI_THINKING` 環境變數。pi-agent-cli 有吃這些 env var，但 pi-agent（wrapper）會忽略它們。
...
所有 >4KB 的 extension（`pi-agent-ext-flux2`, `pi-hermes-memory` 等）都無法載入。
