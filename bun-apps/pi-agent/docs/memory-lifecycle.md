...
> (`pi-hermes-memory`) into the durable vault (`pi-agent-ext-obsidian` +
> `pi-agent-ext-knowledge-card`). The READ side (retrieval architecture) is a
...
- `pi-agent-ext-hermes-memory/src/store/vault-converge.ts` — the soft-import seam
  (A and B both go through it).
- `pi-agent-ext-hermes-memory/src/store/converge-health.ts` — the health + state
  bookkeeping (B's observability layer).
- `pi-agent-ext-hermes-memory/src/handlers/converge-health-command.ts` — the
