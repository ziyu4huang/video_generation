...
- All paths relative to `bun-apps/pi-agent/` unless noted.
...
async function stageSnapshot(bunAppsDir: string) {
  ...
  // Copy pi-agent source tree (including generated files)
  ...
  // Write run.sh
  writeRunSh(target, false);  // uses system bun with pi-agent/src/cli.ts
  if (!NO_FREEZE) stageFreeze(target);
  console.log("▶ done");
}
```
...
function writeRunSh(outDir: string, useLocalBun: boolean) {
  ...
  const entry = useLocalBun ? 'pi-agent.js' : 'pi-agent/src/cli.ts';
...
ls /tmp/plan-test-bundle/pi-agent.js  # should exist

# --snapshot
rm -rf /tmp/plan-test-snapshot
bun scripts/deploy.ts /tmp/plan-test-snapshot --snapshot --no-freeze
ls /tmp/plan-test-snapshot/pi-agent/src/cli.ts  # source should exist
...
/tmp/plan-test-exe/pi-agent doctor --json  # ok:true
```
