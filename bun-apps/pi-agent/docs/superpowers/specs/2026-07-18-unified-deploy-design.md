...
- **Default:** `../../dist/pi-agent` (same as current `build.ts` output dir)
...
```
target/
├── pi-agent.js                # bundled pi-agent (minified)
├── pi-agent.js.map            # only with --sourcemap
├── ext-bundles/*.thin.js      # one per extension
├── skills/                    # skill directories
├── run.sh                     # bun run pi-agent.js "$@"
...
2. bundle()                # bun build src/cli.ts → pi-agent.js
3. bundleExtensions()      # thin-bundle each ext → ext-bundles/*.thin.js
4. copySkills()            # manifest.skills → target/skills/
5. writeRunSh()            # run.sh with bun pi-agent.js "$@"
6. freeze()                # chmod -R a-w (unless --no-freeze)
```

**No `node_modules/`** — pi-agent.js + thin bundles resolve deps via baked
...
```
target/
├── pi-agent/                    # full source tree (with generated files)
...
2. copyWorkspace()         # cp -R bun-apps/pi-agent → target/pi-agent/
3. copyNodeModules()       # cp -R bun-apps/node_modules → target/node_modules/
4. writeRunSh()            # run.sh with bun pi-agent/src/cli.ts "$@"
...
**Purpose:** Ship pi-agent to a machine without Bun installed.
Copies local bun binary alongside the bundle.

**Output:**
```
target/
├── pi-agent.js                # bundled pi-agent (same as --bundle)
...
6. writeRunSh()            # run.sh with ./bun pi-agent.js "$@"
7. freeze()
```
...
```
target/
└── pi-agent                # standalone executable (~74 MB)
```

**Stages:**
```
1. codegen()               # pi-pkg-dir.ts, run-dir-base.ts, embedded-assets.ts (with type:file imports)
2. compile()               # bun build --compile src/cli.ts → target/pi-agent
...
- `useBun=false` → `bun pi-agent.js "$@"` (system bun)
- `useBun=true`  → `./bun pi-agent.js "$@"` (local bun binary)
