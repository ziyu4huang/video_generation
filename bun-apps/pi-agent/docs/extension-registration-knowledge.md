...
The `inspect_extensions` tool (in `pi-agent-ext-power-tool`) classifies tools as **active** vs **lazy-loaded** by comparing:
...
| File | Purpose |
|------|---------|
| `bun-apps/pi-agent-ext-workflow/extensions/workflow.ts` | Reference implementation with both hooks |
| `bun-apps/pi-agent-ext-power-tool/src/index.ts` | `inspect_extensions` implementation |
| `bun-apps/pi-agent/run-dir/manifest.json` | Extension registry (`extensions` array + `lazyExtensions`) |
