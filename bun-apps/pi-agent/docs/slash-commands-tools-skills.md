...
> Study notes, written 2026-07-15. `pi-agent` itself ships no slash-command
> engine — it's a thin wrapper (see [`../PRD.md`](../PRD.md)) around
> `@earendil-works/pi-coding-agent`. The engine, and every concrete
> `/command`, lives in that SDK plus the sibling `bun-apps/pi-agent-ext-*/`
...
- **Pure command**: `pi-agent-ext-goal-todo/src/goal/goal.ts:331` —
...
- **Command spawning a workflow**:
  `pi-agent-ext-workflow/src/builtin-commands.ts:40` —
...
- **Tool + command from one factory**:
  `pi-agent-ext-obsidian/extensions/obsidian.ts:1792` —
...
- [`../PRD.md`](../PRD.md) — pi-agent's own scope (thin wrapper, deploy modes)
