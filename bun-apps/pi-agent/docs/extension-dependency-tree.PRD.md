...
> Architecture baseline of the `bun-apps/pi-agent-ext-*` extension graph.
...
| From | → dep | Type |
|------|-------|------|
| `pi-agent-ext-flux2` | `pi-agent-ext-file2md` | `dependencies` |
| `pi-agent-ext-movie-director` | `pi-agent-ext-flux2` | `dependencies` |
| `pi-agent-ext-movie-director` | `pi-agent-ext-krea2` | `dependencies` |
...
### 5a. Static — `pi-agent/src/static-extensions.ts`
...
### 5b. Dynamic — `pi-agent/run-dir/manifest.json` `extensions[]`
...
| **Total registered** | **21** (= all `pi-agent-ext-*` packages) |
...
best-effort-builds `pi-agent-ext-workflow`'s dist if missing.
...
- **deps ∩ peerDependencies redundancy** — `pi-agent-ext-ltx` and
  `pi-agent-ext-movie-director` list `@earendil-works/pi-coding-agent` and
...
- **`pi-agent` `dependencies` under-declares** the true extension set (5 of
...
- **Adding an extension** → create `pi-agent-ext-<x>/`, pick STA or DYN
