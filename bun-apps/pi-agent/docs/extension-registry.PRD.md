...
`./pi-agent.sh -p "…"` failed in source/dev mode with one of:
...
(`pi-agent-ext-flux2/src/binary.ts`, krea2's, `pi-hermes-memory`'s 40 KB+
...
3. **jiti's `try-native` fails first.** In source mode `tryNative` defaults on,
   ...
   the **importing file's location** (`bun-apps/pi-agent-ext-*/extensions/`).
...
binary cannot load `.ts` extensions at all (see `pi-agent-three-launch-modes`
...
1. Create `bun-apps/pi-agent-ext-<name>/` (workspace member).
...
- **Patch ran?** `BUN_PI_DEBUG_PATCHES=1 ./pi-agent.sh …` → look for
  `✓ ensure-extension-deps`.
- **Unit:** `bun test` in `bun-apps/pi-agent` (the patch table's
