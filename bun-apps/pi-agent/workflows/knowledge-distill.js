 ...
 *
 * This lives in bun-apps/pi-agent/workflows/ (the ENGINE dir) on purpose:
...
Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/pi-agent' src/cli.ts cli zk-query --health 2>&1")
...
1. Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/pi-agent' src/cli.ts cli zk-extract '${src}' --folder '${FOLDER}' --max-notes ${MAX_NOTES} --model lm-studio/google/gemma-4-26b-a4b-qat --thinking ${THINKING} -p 2>&1 | tail -40")
...
2. If EXISTS: Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/pi-agent' src/cli.ts cli zk-ingest '${kf}' --source-label 'workflow-jsonl:${NAME}' 2>&1 | tail -20")
...
1. Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/pi-agent' src/cli.ts cli zk-card check 2>&1 | tail -30")
2. Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/pi-agent' src/cli.ts cli zk-query --health 2>&1")
