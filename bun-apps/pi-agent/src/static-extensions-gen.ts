 ...
 * (.planning/pi-agent-optimization/plans/2026-08-20-phase-d-scaffold-codegen.md).
 *
 ...
 * ("pi-agent-ext-task") — the manifest currently carries full names. Both
...
function suffixOf(entry: string): string {
	return entry.replace(/^pi-agent-ext-/, "");
}
...
export function buildStaticExtensionsSource(input: StaticExtGenInput): string {
	...
	const imports = suffixes.map(
		(s) => `import ${camel(s)}Extension from "../../pi-agent-ext-${s}/extensions/${s}.ts";`,
	);
	const rows = suffixes.map((s) => {
		const binding = `${camel(s)}Extension`;
		const row = `\t{ name: "pi-agent-ext-${s}", factory: ${binding} },`;
