...
export function scanSkills(repoRoot: string): SkillFile[] {
	const dirs = [
		join(repoRoot, "bun-apps/pi-agent-ext-superpowers/skills"),
		join(repoRoot, "bun-apps/pi-agent-ext-wayfind/skills"),
	];
...
export async function loopStatus(repoRoot: string, args: string[]): Promise<number> {
	if (args[0] !== "status") {
		console.log("usage: pi-agent.sh loop status   (report-only drift report; always exits 0)");
		return 0;
	}
	const lines: string[] = [];
	// 1. death rate
	const statsText = run(repoRoot, ["bun", "bun-apps/pi-agent-ext-subagent/scripts/runs-stats.ts"]);
	...
	const schema = run(repoRoot, ["bun", "bun-apps/pi-agent/src/cli.ts", "schema-cost"]);
	...
	const cov = run(join(repoRoot, "bun-apps/pi-agent-ext-wayfind"), ["bun", "test", "--coverage"]);
