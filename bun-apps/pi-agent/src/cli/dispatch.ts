/**
 * pi-agent's non-interactive CLI — pi-obsidian baked in.
 *
 * NOT a process entry point. This module is reached only through pi-agent's
 * single binary, under the `cli` namespace token, via the exported `runCli()`
 * at the bottom of this file. `pi-agent` with no `cli` token still starts the
 * interactive TUI.
 *
 * Two invocation styles:
 *
 *   1. Agent commands (one CLI = one agent workflow):
 *        pi-agent cli file2md <files...> [options]       PDF/image → Obsidian md
 *        pi-agent cli zk-extract <files.../folders...> [options] markdown → Zettelkasten notes
 *        pi-agent cli zk-card <sub> [options]                 CRUD for Zettelkasten notes (add/find/update/remove/check)
 *        pi-agent cli zk-ask <question> [options]             graph-enhanced vault Q&A
 *        pi-agent cli zk-ingest <jsonl-files...> [options]    converge structured records → shared knowledge-graph vault
 *        pi-agent cli zk-query [options]                       cross-workflow retrieval + graph health (READ side)
 *        pi-agent cli pipeline pdf-to-vault <pdf> [options]   PDF → md → vault (resumable)
 *
 *      Plus meta commands: list | version | help.
 *
 *   2. Pi-compatible passthrough (anything else) — mirrors `pi -p` / `pi --mode json`:
 *        pi-agent cli --model sonnet -p "What files are here?"
 *        pi-agent cli --mode json -p --no-session --tools read,bash "task"
 *
...
async function dispatch(argv: string[]): Promise<void> {
  ...
  // Agent commands: pi-agent cli [global flags] <command> ...
  if (found) {
