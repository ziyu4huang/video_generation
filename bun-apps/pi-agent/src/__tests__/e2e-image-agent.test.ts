 ...
 * ACTUAL pi-agent launcher with a real model + a natural-language prompt and
 ...
 *   ./pi-agent.sh --model <model> -p "verify I can use Flux tool, reply in zh-TW,
 ...
 *   `bash bun-apps/pi-agent/scripts/run-image-agent-e2e.sh`. A flaky model reply
...
/**
 * Model id. The example used google/gemma-4-26b-a4b-qat; any model the operator
 ...
 */
const MODEL = process.env.PI_AGENT_E2E_MODEL || "google/gemma-4-26b-a4b-qat";
...
async function runLauncher(prompt: string, timeoutMs: number): Promise<RunResult> {
  const launcher = join(REPO_ROOT, "pi-agent.sh");
  ...
  const timer = setTimeout(() => {
    timedOut = true;
    try {
      process.stderr.write(`\n[e2e-image-agent] timeout (${timeoutMs}ms) killing pi-agent.sh\n`);
      proc.kill();
    } catch {
      /* already exited */
    }
...
describe.skipIf(!IMAGE_E2E_ENABLED)("e2e: image agent — real model drives flux2 tool", () => {
  test.skipIf(!BINARY_READY)(
    ...
    async () => {
      ...
      if (res.timedOut) {
        throw new Error(`pi-agent.sh timed out after ${RUN_TIMEOUT_MS}ms.\n--- tail ---\n${combined.slice(-800)}`);
      }
      if (res.code !== 0) {
        throw new Error(`pi-agent.sh exited ${res.code}.\n--- stdout tail ---\n${res.stdout.slice(-800)}\n--- stderr tail ---\n${res.stderr.slice(-800)}`);
      }
