...
describe("parseExtNewArgs", () => {
	...
	test("throws when the pi-agent-ext- prefix is supplied by the user", () => {
		expect(() => parseExtNewArgs(["pi-agent-ext-foo"])).toThrow();
	});
});
...
describe("validateName", () => {
	...
	test("rejects the pi-agent-ext- prefix and reserved dir names", () => {
		expect(validateName("pi-agent-ext-foo")).toBe(false);
		expect(validateName("pi-agent")).toBe(false);
...
describe("buildScaffoldFiles (default in-file layout)", () => {
	...
	test("package.json content", () => {
		...
		expect(pkg.name).toBe("@repo/pi-agent-ext-foo-bar");
	...
	test("README documents test/typecheck/registration/self-gate", () => {
		...
		expect(readme).toContain("pi-agent-ext-foo-bar");
