 ...
 * the third copy impossible: the probe lives in `@repo/pi-agent-core-runtime`
...
/** Every `bun-apps/pi-agent-ext-*` directory. */
function extensionPackages(): string[] {
	return readdirSync(BUN_APPS)
		.filter((n) => n.startsWith("pi-agent-ext-"))
		.filter((n) => {
			try {
...
describe("the dependency probe has exactly one implementation", () => {
	test("no pi-agent-ext-* package defines its own", () => {
		...
		expect(
			...
			`These files define a dependency probe locally. Import { missingExtDeps, findWorkspaceRoot } ` +
				`from "@repo/pi-agent-core-runtime" instead — see src/ext-deps.ts for why a private copy ` +
				`is always wrong in at least one deploy mode.`,
		).toEqual([]);
	});

	test("no pi-agent-ext-* package hand-rolls the node_modules walk", () => {
		const offenders: string[] = [];
		for (const pkg of extensionPackages()) {
			for (const file of sourceFiles(join(BUN_APPS, pkg))) {
		...
		expect(
			...
			`These files look for node_modules/<pkg>/package.json by hand. In a compiled binary and ` +
				...
				`as missing. Use missingExtDeps() from "@repo/pi-agent-core-runtime".`,
		).toEqual([]);
	});
