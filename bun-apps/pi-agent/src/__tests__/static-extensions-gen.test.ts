...
test("output shape: ends with ]; and each binding appears exactly twice (import + row)", () => {
	const src = buildStaticExtensionsSource({ staticExtensions: ["task", "prompt-history", "subagent", "workflow"] });
	expect(src.endsWith("];\n")).toBe(true);
	for (const binding of ["taskExtension", "promptHistoryExtension", "subagentExtension", "workflowExtension"]) {
...
test("import order follows input order", () => {
	const src = buildStaticExtensionsSource({ staticExtensions: ["task", "prompt-history", "subagent", "workflow"] });
	const positions = [
		src.indexOf('import taskExtension from "../../pi-agent-ext-task/extensions/task.ts";'),
		src.indexOf('import promptHistoryExtension from "../../pi-agent-ext-prompt-history/extensions/prompt-history.ts";'),
		src.indexOf('import subagentExtension from "../../pi-agent-ext-subagent/extensions/subagent.ts";'),
		src.indexOf('import workflowExtension from "../../pi-agent-ext-workflow/extensions/workflow.ts";'),
	];
...
test("manifest-style full package names normalize to the same output as bare suffixes", () => {
	// manifest.json currently carries full names ("pi-agent-ext-task"), while
	...
	const full = buildStaticExtensionsSource({ staticExtensions: ["pi-agent-ext-task", "pi-agent-ext-webui"] });
