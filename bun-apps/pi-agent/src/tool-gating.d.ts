 ...
 *
 * ── Host copy ───────────────────────────────────────────────────────────────
 * This copy lives in the HOST (`pi-agent/src/`) so the monorepo-wide typecheck
 * (which follows imports into every extension's `src/`) accepts `gating` on
 * ToolDefinition. Each extension package also carries its own identical copy for
 * its isolated typecheck; this host copy makes the augmentation visible in the
 * compile context that spans all extension sources.
 *
