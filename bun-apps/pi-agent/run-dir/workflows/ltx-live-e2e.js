/**
 * ltx-live-e2e — REAL end-to-end smoke test for bun-apps/pi-agent-ext-ltx.
 *
 * Runs the ACTUAL `ltx-video` Swift/MLX binary via `runLtx()` (no mocks),
 * through pi-agent's own dynamic-workflows engine — the same path a user
 * invokes with `bun-apps/pi-agent/run.sh -e workflow -p "…"`. Mirrors
 * `.claude/workflows/pi-agent-ext-flux2-self-improve.js`'s `runLiveE2eLane()`
...
export const meta = {
  name: 'ltx-live-e2e',
  description: 'Real end-to-end smoke test for pi-agent-ext-ltx: runs the actual ltx-video binary via runLtx() (models/t2i/gate, optionally native-i2v and an A/B upscale+ffprobe correctness check) through pi-agent itself — not just bun test.',
  phases: [
...
2. Write a scratch bun script at /tmp/ltx-live-e2e-smoke.ts (the SCRIPT ITSELF can live in /tmp — only the paths passed as runLtx() output/input options must be under OUT_DIR) that:
   - imports { runLtx } from "\${REPO_ROOT}/bun-apps/pi-agent-ext-ltx/src/index.ts" via an absolute path
