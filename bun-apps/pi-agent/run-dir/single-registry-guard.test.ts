/**
 * The single-registry guard: pi-agent.registry.yaml has ONE parser.
 *
...
const AUTHORITY = "bun-apps/pi-agent/run-dir/registry.ts";
...
describe("single registry guard", () => {
	test("only run-dir/registry.ts both mentions and parses pi-agent.registry.yaml", () => {
		const offenders: string[] = [];
		for (const root of SCAN_ROOTS) {
			for (const file of collectTs(join(REPO_ROOT, root))) {
				const text = readFileSync(file, "utf8");
				if (!text.includes("pi-agent.registry.yaml")) continue;
