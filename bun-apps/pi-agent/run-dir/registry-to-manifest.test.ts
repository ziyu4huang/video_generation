...
const registry: Registry = {
  ...
  lazyExtensions: { "lazy-one": "pi-agent-ext-lazy-one/extensions/lazy-one.ts" },
};
...
describe("buildManifestObject", () => {
  test("staticExtensions is the registry-ordered list of load:static packages", () => {
    ...
    expect(m.staticExtensions).toEqual(["pi-agent-ext-alpha", "pi-agent-ext-beta", "pi-agent-ext-gamma"]);
  });

  test("skills includes BOTH deployed and non-deployed skill carriers, in registry order", () => {
    const m = buildManifestObject(registry);
    expect(m.skills).toEqual(["pi-agent-ext-alpha/skills", "pi-agent-ext-beta/skills"]);
  });

  test("binarySkills lists only binarySkills:true carriers and is a subset of skills", () => {
    const m = buildManifestObject(registry);
    expect(m.binarySkills).toEqual(["pi-agent-ext-alpha/skills"]);
  ...
  test("extensions carries only load:dynamic entries with package-prefixed entries and version only when set", () => {
    ...
    expect(m.extensions[0]).toEqual({
      name: "delta",
      entry: "pi-agent-ext-delta/extensions/delta.ts",
      version: "0.1.0",
    });
    expect(m.extensions[1]).toEqual({ name: "epsilon", entry: "pi-agent-ext-epsilon/extensions/epsilon.ts" });
    expect("version" in m.extensions[1]).toBe(false);
  });

  test("lazyExtensions passes through verbatim", () => {
    const m = buildManifestObject(registry);
    expect(m.lazyExtensions).toEqual({ "lazy-one": "pi-agent-ext-lazy-one/extensions/lazy-one.ts" });
  });

  test("$generated is present and the first key of the emitted object", () => {
    const m = buildManifestObject(registry);
    expect(m.$generated).toBe("from pi-agent.registry.yaml by regen:manifest — do not edit");
