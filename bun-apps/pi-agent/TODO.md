...
### 2. `doctor --fix` (auto-remediate)

Mirror `bun-apps/pi-agent-cli/src/commands/doctor.ts` `--fix` (create default
output dir, `~/.pi/agent`, etc.) so pi-agent's doctor closes the
diagnose→fix loop. pi-agent-specific fixes: run `bun install` in a `--portable`
deploy when the host-deps check FAILs. Adds a `fix` step to `runDoctor`
(mutating — keep the pure `runChecks` unchanged, same pattern as `--smoke`).

### 3. Patch unit-coverage gaps
