...
# invoked `pi-agent -e workflow -p "read … and execute"`, but the LLM agent in
...
#
# USAGE
#   bash bun-apps/pi-agent/scripts/run-self-improve-loop.sh              # default = first pose
...
DRIVER="$REPO_ROOT/bun-apps/pi-agent-ext-flux2/scripts/self-improve-loop.driver.ts"
POSES_JSON="$REPO_ROOT/bun-apps/pi-agent-ext-flux2/workflows/poses.json"
