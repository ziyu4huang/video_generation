#!/usr/bin/env bash
########################################
# run-ext-e2e.sh — run the L2 judgment e2e workflow for every pi-agent extension.
#
# Discovers bun-apps/pi-agent-ext-*/workflows/test-*-e2e.js and runs each via the
# pi-agent `workflow` tool. OPT-IN: this spends LLM tokens and is non-deterministic,
...
#   bash bun-apps/pi-agent/scripts/run-ext-e2e.sh flux2        # one extension
#   bash bun-apps/pi-agent/scripts/run-ext-e2e.sh --list       # list, run nothing
########################################
set -uo pipefail
...
WORKFLOWS=()
for d in "$EXTS_DIR"/pi-agent-ext-*/workflows/test-*-e2e.js; do
  [ -f "$d" ] || continue
  if [ "$mode" = "one" ]; then
    case "$d" in *"/pi-agent-ext-$filter/"*) WORKFLOWS+=("$d");; esac
  else
    WORKFLOWS+=("$d")
  fi
done
...
if [ "${#WORKFLOWS[@]}" -eq 0 ]; then
  echo "No extension e2e workflows found under $EXTS_DIR/pi-agent-ext-*/workflows/test-*-e2e.js"
