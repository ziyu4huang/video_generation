# PRD — pi-agent extension e2e testing (unified)

## Problem

Every `pi-agent-ext-*` package wraps a Swift/MLX image-director CLI as one agent
tool. Each has a deterministic `bun test` suite (flag mapping, path safety, manifest
parsing, coerceOptions) and `pi-agent/run-test.sh` covers the pi-agent core across
...
**In scope:** a single, unified e2e test **method** for every `pi-agent-ext-*`
...
- `pi-agent/run-test.sh` (the CI-safe deterministic backbone) — stays as-is; the
...
| --- | --- | --- | --- | --- |
| L0 unit/contract | `bun test` per package — flag↔CLI, path safety, manifest parse, coerceOptions | yes | ~0 | each `pi-agent-ext-*` |
| L1 core integration | `run-test.sh` tiers (build/deploy/cwd/readonly) | yes | ~5–40s | `pi-agent` |
| **L2 judgment e2e** | **this PRD** — one `workflows/test-*-e2e.js` per extension, VLM-scored | **no** | tokens + minutes | each `pi-agent-ext-*` |
| **L3 real-model integration** | `bash scripts/run-image-agent-e2e.sh` — spawn `pi-agent.sh` with a real model + NL prompt, assert zh-TW reply + a new timestamped PNG | **no** | tokens + minutes | `pi-agent` |
...
```
./pi-agent.sh --model <model> -p "verify I can use the Flux tool, reply in zh-TW,
...
```
bun-apps/pi-agent-ext-<ext>/workflows/test-<ext>-e2e.js
```
...
entry is `scored:false`. See `bun-apps/pi-agent-ext-workflow/tests/regression-rca.test.ts`
...
`bun-apps/pi-agent/scripts/run-ext-e2e.sh` — discovers and runs every extension's L2
workflow:

```bash
for wf in bun-apps/pi-agent-ext-*/workflows/test-*-e2e.js; do
...
- [x] Prototype: `pi-agent-ext-flux2/workflows/test-flux2-e2e.js` — proved the
...
- [x] Add L2 workflow for `pi-agent-ext-krea2` (`workflows/test-krea2-e2e.js` +
...
- [x] Add L3 real-model integration e2e (`src/__tests__/e2e-image-agent.test.ts` +
      `scripts/run-image-agent-e2e.sh`). Spawns `pi-agent.sh` with a real model +
