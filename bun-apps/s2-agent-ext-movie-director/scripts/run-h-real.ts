 ...
 * same `movie generate` code path the s2-agent drives), not a lavfi stub.
 *
 ...
 *
 * Run:
 *   bun run --cwd bun-apps/s2-agent-ext-movie-director scripts/run-h-real.ts
 *
...
main().catch((err) => {
  if (err instanceof NoConfiguredProviderError) {
    ...
    console.error("(bun-apps/s2-agent-ext-movie-director auto-builds it on first generate; ensure swift toolchain is installed.)");
  }
