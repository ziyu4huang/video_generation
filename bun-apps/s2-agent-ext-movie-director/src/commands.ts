 ...
 *     (`bun bun-apps/s2-agent/src/cli.ts -e <this-extension> -p <prompt>`) with
...
/** A top-level CLI command (mirrors s2-agent's Command shape). */
 ...
 *
 * Order: `PI_BIN` env override → the in-repo `bun-apps/s2-agent/src/cli.ts`
...
export function resolvePiBin(opts: { exists?: (p: string) => boolean; dir?: string } = {}): string {
	...
	// import.meta.dir = .../s2-agent-ext-movie-director/src; repo root = 3 levels up.
...
export const agentCommand: Command = {
	...
	details: `Usage:
  bun .../s2-agent-ext-movie-director/src/cli.ts agent <natural-language request...> [options]
...
Implementation: shells out to the pi binary (\`bun bun-apps/s2-agent/src/cli.ts\`)
