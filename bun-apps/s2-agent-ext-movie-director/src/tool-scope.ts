 ...
 *
 * The agent-first thesis puts an unsupervised LLM (bonsai-27b) in the driver's
 * seat of the `movie` tool. That agent also retains s2-agent's built-in `edit`
 ...
 * prevents: a denylist enforced at the extension layer via the s2-agent
...
/** Repo root = the extension's grandparent (this pkg is bun-apps/s2-agent-ext-movie-director). */
