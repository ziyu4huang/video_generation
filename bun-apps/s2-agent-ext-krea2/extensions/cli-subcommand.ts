 ...
 *
 * Lets `s2-agent` expose the krea2 extension as a top-level sub-command:
 *
 *   s2-agent cli krea2 <natural-language request...>
 *   s2-agent cli --model sonnet krea2 generate a red cube on a table, t2i
 *
 ...
 * direction is s2-agent → s2-agent-ext-krea2, so the spec is typed with a
 * local structurally-compatible interface (TS structural typing makes it
 * assignable to s2-agent's `ExtensionSubcommandSpec`).
 */
...
export const krea2Subcommand: ExtensionSubcommandSpec = {
  ...
  details: `Usage:
  s2-agent cli krea2 <natural-language request...> [options]
...
are NOT parsed by the CLI. Two ways to pass them:
  ...
  2. Use the \`--\` end-of-options separator to pass raw flags through verbatim:
       s2-agent cli krea2 -- t2i --prompt "a red cube" --width 1024
...
Examples:
  s2-agent cli krea2 generate a red apple on a wooden table using t2i
  s2-agent cli krea2 edit <png> into a green apple on marble, strength 0.9
  s2-agent cli krea2 -- t2i --prompt "a red cube" --width 1024`,
  factory: extension,
  tools: ["krea2"],
  task: (parsed) => {
