#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for pi-agent-ext-obsidian.
...
print_list() {
	cat <<EOF
$(Y "pi-agent-ext-obsidian run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/pi-agent-ext-obsidian-runtest.log 2>&1
	rc=$?
	...
	if [ "$rc" -ne 0 ]; then sed 's/^/      /' /tmp/pi-agent-ext-obsidian-runtest.log | tail -n 25 >&2; fi
}
...
echo "$(Y "▶ pi-agent-ext-obsidian run-test.sh — tier=$TIER")"
case "$TIER" in
