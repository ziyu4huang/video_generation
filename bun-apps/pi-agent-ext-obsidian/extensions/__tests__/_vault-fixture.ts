...
export const VAULT_SYNC_CMD = "git submodule update --init vaults_root/pi-agent-vault";
...
export function vaultDriftReason(): string | null {
	...
	try {
		const status = execSync("git submodule status vaults_root/pi-agent-vault", {
			cwd: REPO_ROOT,
			encoding: "utf8",
		...
		if (flag === "+") {
			return `vaults_root/pi-agent-vault is DRIFTED (checked-out commit ≠ recorded by superproject). ` +
				`Baseline byte-diff is meaningless until synced. Fix:\n  ${VAULT_SYNC_CMD}`;
		}
		if (flag === "-") return `vaults_root/pi-agent-vault not initialized. Fix:\n  ${VAULT_SYNC_CMD}`;
		if (flag === "U") return `vaults_root/pi-agent-vault has a merge conflict. Resolve, then:\n  ${VAULT_SYNC_CMD}`;
		return null;
	} catch {
