...
The ubiquitous language of pi-agent-ext-obsidian — tools to read, write, search, and graph a project-local Obsidian vault, plus optional semantic (vector) search and subagent-driven distill/garden operations. Otherwise hermetic: filesystem only, no network except the opt-in semantic service.
