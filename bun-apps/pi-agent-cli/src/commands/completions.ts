 ...
 *
 * Unlike other commands, this is handled INLINE in cli.ts (like `list` /
 * `list-tools`) rather than via a `run()` function. The reason: it needs the
 * COMMANDS / PIPELINES arrays from cli.ts, and a dynamic `import("../cli.ts")`
 * inside a `run()` creates a circular-dependency hang. Handling it inline lets
 * cli.ts pass its own arrays directly.
 *
 * This module exports only the generator functions + metadata; cli.ts calls
...
/**
 * Print a completion script for the given shell. Called inline from cli.ts
