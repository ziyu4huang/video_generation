...
| Session runner | Delegates to the shared `runPrettyTask` / `runJsonTask` in `src/sessions/task-runner.ts` (same runners as `zk-extract` / `zk-card`). |
