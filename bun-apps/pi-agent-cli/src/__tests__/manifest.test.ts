...
import { readManifest, validateManifest, type Manifest } from "../manifest.ts";

/**
 * `manifest.ts` — the workflow-pack manifest model (Decision 2: minimal
 * load-bearing). Required: name/description/entry. Optional: args/model/thinking/
 * howToRun. Pure: no engine, no network.
 *
 * Phase 1 contract: every later phase (resolver, runner, list, examples) reads a
 * pack through readManifest/validateManifest, so these are the load-bearing
 * tests for the whole feature.
 */
