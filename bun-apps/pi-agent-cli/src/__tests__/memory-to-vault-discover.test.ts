...
describe("discoverMemoryFiles", () => {
	...
	beforeEach(() => {
		...
		// 2 real project dirs
		for (const p of ["video_generation__memory", "pi-agent"]) {
			const d = join(projectsDir, p);
			mkdirSync(d, { recursive: true });
			writeFileSync(join(d, "MEMORY.md"), `${p} lesson §\n`);
		}
		// NOISE: verify + tmp + backup + empty
		mkdirSync(join(projectsDir, "pi-agent-verify-ABC"), { recursive: true });
		writeFileSync(join(projectsDir, "pi-agent-verify-ABC", "MEMORY.md"), "should be excluded §\n");
