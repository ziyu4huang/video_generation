...
| Term | Meaning |
|---|---|
| **GATE_DEFS** | The shared gate registry (`@repo/pi-agent-core-interface`) — each family declared ONCE by id (`{ id, keywords?, requires?, description }`). Populated by the owning extension at module load. |
...
- `@repo/pi-agent-core-interface` `src/gates.ts` + `src/tool-gating.d.ts` — the gate contract: `Gate` type, `GATE_DEFS` registry, `Gating` field type. **The single shared source of truth** — owning extensions populate `GATE_DEFS`; tool-gate resolves it.
