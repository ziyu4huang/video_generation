...
> Keeps core tools always active while gating heavy domain-specific tools behind prompt keyword matching — saving ~5,500 tokens per request (~38%).

## The Problem

Every registered tool adds to the API request's tools schema. With a full extension ecosystem loaded, a single pi session can carry **~45 tools → ~14,400 tokens per request** — a fixed overhead charged on every turn, even when 95% of those tools are never used.
