 ...
 *
 * Question: does tool-gate's "~12,300 tok/req saved" claim hold?
 *
...
/** The savings figure tool-gate's README/banner claims (~12,300 tok/req; zai-mcp env-gated — see caveats()).
 *  THE single source of truth — every prose mention cites ~12,300 and points to
 *  `bun run qa:savings` for live numbers. Do not hardcode a competing figure. */
export const CLAIMED_SAVED_TOK = 12300;
 ...
 *  ZAI_API_KEY ≈ 9% of the claim), so a tighter band would flake whenever zai
 ...
 *  by design: with zai-mcp loaded (~+1.1k) measured gross ≈ 13,400 vs the upper
 *  edge 14,760 (~9% headroom) — a high-side "OUTSIDE ✗" correctly signals the
 ...
 *  independent. Prose cites ~12,100 (round-to-100); the live measured net is
 *  reported by `bun run qa:savings`. */
export const CLAIMED_NET_TOK = CLAIMED_SAVED_TOK - ENABLE_TOOL_OVERHEAD_TOK; // 12,057
...
export const SANCTIONED_PROSE_TOK: ReadonlySet<number> = new Set([
	CLAIMED_SAVED_TOK, // ~12,300 gross
	Math.round(CLAIMED_NET_TOK / 100) * 100, // ~12,100 net (claim rounded)
