...
const extRoot = resolve(here, ".."); // bun-apps/pi-agent-ext-tool-gate
...
test("the canonical gross claim is actually cited in the prose (guard against silent removal)", () => {
	// If someone deletes the ~12,300 claim from every file, the sanctioned-set
	...
	expect(readme).toContain("~12,300");
});
...
test("sanctioned set is internally consistent (gross + net derive from the claims)", () => {
	...
	expect(CLAIMED_NET_TOK).toBe(CLAIMED_SAVED_TOK - 243); // net derives from gross − overhead
});
