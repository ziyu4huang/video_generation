#!/usr/bin/env bun
/**
 * run-test.ts — tiered test launcher for s2-agent-ext-hermes-memory.
 *
 ...
 *                    runContract() comment).
 * USAGE (from anywhere):
 *   bun bun-apps/s2-agent-ext-hermes-memory/run-test.ts              # = quick
 *   bun bun-apps/s2-agent-ext-hermes-memory/run-test.ts full
 *   bun bun-apps/s2-agent-ext-hermes-memory/run-test.ts --list       # print the tier table, exit 0
 *   bun bun-apps/s2-agent-ext-hermes-memory/run-test.ts quick --bail # extra flags are forwarded
...
const PKG = "s2-agent-ext-hermes-memory";
// extension-contract.test.ts re-asserted standalone (the `full` tier).
const CONTRACT_TEST = "tests/extension-contract.test.ts";
