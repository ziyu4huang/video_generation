...
describe("recall-audit harness (fixture corpus, offline)", () => {
  it("runs kcard-only against a temp corpus and produces a scored receipt", async () => {
    ...
    const battery = {
      ...
      kcard: [
        {
          q: "sequencing the relay handoff before the downlink window closes",
          vaultTargets: ["fixture-aurora-constellation-deploy"],
        },
        {
          q: "which berth needs a doubled spring line at high tidal range",
          vaultTargets: ["fixture-harbor-lantern-mooring"],
        },
