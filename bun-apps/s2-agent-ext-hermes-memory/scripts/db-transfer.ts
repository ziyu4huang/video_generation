...
import { DEFAULT_SURREAL_DATABASE, derivePerUserNamespace } from "../src/store/surreal/per-user-db.js";
...
export function sessionToSqlite(row: SurrealRow): SqliteSessionRow {
  return {
    ...
    ended_at: (row.endedAt as string | null) ?? null,
...
export function sessionFileToSqlite(row: SurrealRow): SqliteSessionFileRow {
  return {
    ...
    mtime_ms: Number(row.mtimeMs ?? 0),
    indexed_at: String(row.indexedAt ?? ""),
  };
}
...
function parseArgs(argv: string[]): Args {
  ...
  if (!direction) {
    console.error("one of --to-surreal / --to-sqlite is required");
    console.error(usage);
    process.exit(2);
  }
...
export function surrealFidelityClauses(
  e: {
    status?: string;
  ...
  const sets = [
    `status = $${prefix}st`,
    `parentIds = $${prefix}pids`,
    `mwSuccess = $${prefix}mws`,
    `mwFail = $${prefix}mwf`,
  ];
  const params: Record<string, unknown> = {
    [`${prefix}st`]: e.status ?? "active",
    [`${prefix}pids`]: e.parentIds ?? [],
    [`${prefix}mws`]: e.mwSuccess ?? 0,
    [`${prefix}mwf`]: e.mwFail ?? 0,
  };
  if (e.severity != null) {
    sets.push(`severity = $${prefix}sev`);
    params[`${prefix}sev`] = e.severity;
  }
  if (e.pin === true) {
    sets.push(`pin = $${prefix}pin`);
    params[`${prefix}pin`] = true;
  }
  if (e.supersedes != null) {
    sets.push(`supersedes = $${prefix}sup`);
    params[`${prefix}sup`] = e.supersedes;
  }
  if (e.supersededBy != null) {
    sets.push(`supersededBy = $${prefix}supd`);
    params[`${prefix}supd`] = e.supersededBy;
  }
  return { sets, params };
}
...
async function runCreates(
  ...
  try {
    ...
    console.error(
      `  ! batch failed (${err instanceof Error ? err.message : String(err)}), replaying rows individually`,
    );
    for (const r of rows) {
      try {
...
async function main(): Promise<number> {
  ...
  log(
    `  sqlite ${path.join(args.memoryDir, "sessions.db")}  <->  surreal ${args.endpoint} ${args.namespace}/${args.database}`,
  );
  ...
  // ── memories (repo path: C6 identity dedup; never mutates existing rows) ──
  {
    ...
    if (args.direction === "to-surreal") {
      ...
      report.src = entries.length;
      report.dst = dst.length;
      for (const e of entries) {
        if (dstIds.has(memoryIdentity(e))) {
          report.skipped++;
          continue;
        }
        if (e.mdId && dstMdIds.has(e.mdId)) {
          report.skipped++;
          continue;
        }
        if (args.dryRun) {
        ...
        try {
          ...
          await sq(`UPDATE type::record("memories", $fid_id) SET ${f.sets.join(", ")};`, {
            fid_id: String(created.id),
            ...f.params,
          });
          report.created++;
        } catch (err) {
          report.failed++;
          console.error(`  ! memory failed: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    } else {
  ...
  // ── sessions (key: id) ──
  {
    ...
    if (args.direction === "to-surreal") {
      const src = sqliteDb
        .prepare("SELECT id, project, cwd, started_at, ended_at, message_count FROM sessions")
        .all() as unknown as SqliteSessionRow[];
      const dst = new Set(
        (await dumpSurreal(sq, "sessions")).map((r) =>
          typeof r.sid === "string" ? r.sid : surrealIdKey(r, "sessions"),
        ),
      );
      report.src = src.length;
      report.dst = dst.size;
      ...
      const dst = new Set(
        (sqliteDb.prepare("SELECT id FROM sessions").all() as unknown as Array<{ id: string }>).map((r) => r.id),
      );
      report.src = src.length;
      report.dst = dst.size;
      ...
      if (!args.dryRun) {
        const insert = sqliteDb.prepare(
          "INSERT OR IGNORE INTO sessions (id, project, cwd, started_at, ended_at, message_count) VALUES (?, ?, ?, ?, ?, ?)",
        );
        for (const r of missing) {
          try {
            insert.run(r.id, r.project, r.cwd, r.started_at, r.ended_at, r.message_count);
            report.created++;
          } catch (err) {
            report.failed++;
            console.error(`  ! session failed: ${err instanceof Error ? err.message : String(err)}`);
          }
        }
      } else report.created = missing.length;
    }
  ...
  // ── messages (key: id; full paginated dump + client-side diff — no IN traps) ──
  {
    ...
    if (args.direction === "to-surreal") {
      const src = sqliteDb
        .prepare("SELECT id, session_id, role, content, timestamp, tool_calls FROM messages")
      ...
      const sessionMeta = new Map(
        (
          sqliteDb.prepare("SELECT id, project, cwd FROM sessions").all() as unknown as Array<{
            id: string;
            project: string;
            cwd: string;
          }>
      ...
      const dst = new Set(
        (sqliteDb.prepare("SELECT id FROM messages").all() as unknown as Array<{ id: string }>).map((r) => r.id),
      );
      report.src = src.length;
      report.dst = dst.size;
      ...
      if (!args.dryRun) {
        const insert = sqliteDb.prepare(
          "INSERT OR IGNORE INTO messages (id, session_id, role, content, timestamp, tool_calls) VALUES (?, ?, ?, ?, ?, ?)",
        );
        for (const r of missing) {
          try {
            insert.run(r.id, r.session_id, r.role, r.content, r.timestamp, r.tool_calls);
            report.created++;
          } catch (err) {
            report.failed++;
            console.error(`  ! message failed: ${err instanceof Error ? err.message : String(err)}`);
          }
        }
      } else report.created = missing.length;
    }
  ...
  // ── session_files (key: path) ──
  {
    ...
    if (args.direction === "to-surreal") {
      const src = sqliteDb
        .prepare("SELECT path, session_id, size, mtime_ms, indexed_at FROM session_files")
        .all() as unknown as SqliteSessionFileRow[];
      const dst = new Set(
        (await dumpSurreal(sq, "session_files")).map((r) =>
          typeof r.path === "string" ? r.path : surrealIdKey(r, "session_files"),
        ),
      );
      report.src = src.length;
      report.dst = dst.size;
      ...
      const dst = new Set(
        (sqliteDb.prepare("SELECT path FROM session_files").all() as unknown as Array<{ path: string }>).map(
          (r) => r.path,
        ),
      );
      report.src = src.length;
      ...
      if (!args.dryRun) {
        const insert = sqliteDb.prepare(
          "INSERT OR IGNORE INTO session_files (path, session_id, size, mtime_ms, indexed_at) VALUES (?, ?, ?, ?, ?)",
        );
        for (const r of missing) {
          try {
            insert.run(r.path, r.session_id, r.size, r.mtime_ms, r.indexed_at);
            report.created++;
          } catch (err) {
            report.failed++;
            console.error(`  ! session_file failed: ${err instanceof Error ? err.message : String(err)}`);
          }
        }
      } else report.created = missing.length;
    }
  ...
  for (const table of ["session_assembly_meta", "session_assembly"] as const) {
    ...
    if (args.direction === "to-surreal") {
      ...
      if (!args.dryRun && missing.length > 0) {
        const cols =
          table === "session_assembly_meta" ? "(session_id, hash, captured_at)" : "(session_id, md_id, used_at)";
        const insert = sqliteDb.prepare(`INSERT OR IGNORE INTO ${table} ${cols} VALUES (?, ?, ?)`);
        for (const r of missing) {
          try {
            if (table === "session_assembly_meta")
              insert.run(String(r.sessionId), String(r.hash), String(r.capturedAt));
  ...
  if (args.json) {
    ...
    for (const r of reports) {
      console.error(
        ` ${r.table.padEnd(20)} ${String(r.src).padStart(6)} ${String(r.dst).padStart(7)} ${String(r.created).padStart(8)} ${String(r.skipped).padStart(8)} ${String(r.failed).padStart(7)}${r.note ? `   (${r.note})` : ""}`,
      );
    }
