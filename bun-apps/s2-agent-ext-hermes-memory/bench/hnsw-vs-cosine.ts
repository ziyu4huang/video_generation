...
const _SQL_BODY_LIMIT_B = 1_048_576;
...
function bruteTopK(qv: number[], vectors: number[][], k: number): void {
  const scored = vectors.map((vec, i) => ({ i, cos: cosine(qv, vec) })).sort((a, b) => b.cos - a.cos);
...
async function queryCosineConcurrent(vectors: number[][], queries: number[][], concurrency: number): Promise<number[]> {
  const lat: number[] = [];
  for (let i = 0; i < queries.length; i += concurrency) {
    const batch = queries.slice(i, i + concurrency);
    const batchStart = ns();
...
function recordRow(scale: number, path: Path, mode: Mode, lat: number[], buildMs = NaN, notes = ""): void {
  lat.sort((a, b) => a - b);
  rows.push({
    scale,
    path,
...
function buildMarkdown(): string {
  ...
  L.push(
    `- Vectors: synthetic ${DIM}-dim unit (Gaussian → L2-normalize), seeded. K=${K_QUERIES} queries, TOP_K=${TOP_K}, EF=${EF}.`,
  );
  L.push(`- Client: the REAL \`SurrealClient\` (HTTP POST /sql, 10s timeout, 3 retries) — RTT is in the measurement.`);
  L.push(
    `- Build: bulk insert in ${BATCH}-CREATE batches (SurrealDB /sql body capped at 1MiB ⇒ max ~143 768-dim CREATEs/body; 500/batch impossible); wall-ms recorded separately from query latency.`,
  );
  L.push(
  ...
  for (const scale of SCALES) {
    ...
    if (!h || !c || !Number.isFinite(h.p95) || !Number.isFinite(c.p95)) {
      L.push(
        `| ${scale.toLocaleString()} | ${h ? fmt(h.p95) : "pending"} | ${c ? fmt(c.p95) : "pending"} | — | (insufficient data) |`,
      );
      continue;
    }
  ...
  if (crossoverN !== null) {
    L.push(
      `- **Crossover:** HNSW becomes faster than in-memory cosine at **${crossoverN.toLocaleString()}** vectors (single-mode p95).`,
    );
  } else {
  ...
  const largestHnsw = SCALES.filter((s) =>
    rows.some((r) => r.scale === s && r.path === "hnsw" && r.mode === "c16" && Number.isFinite(r.p95)),
  );
  if (largestHnsw.length) {
    ...
    if (c16 && Number.isFinite(c16.p95)) {
      L.push(
        `- At **${s.toLocaleString()} / 16-concurrent**: HNSW p95=${fmt(h16.p95)}ms vs cosine p95=${fmt(c16.p95)}ms — HNSW **${fmt(c16.p95 / h16.p95)}×** faster (server parallelism vs JS serialization).`,
      );
    }
  }
  L.push(``);
...
function computeVerdict(crossoverN: number | null): { tag: string; lines: string[] } {
  ...
  const hnswFasterAt100k =
    !!h100k && !!c100k && Number.isFinite(h100k.p95) && Number.isFinite(c100k.p95) && h100k.p95 < c100k.p95;
...
async function runScale(scale: number, queries: number[][]): Promise<void> {
  ...
  try {
    ...
    if (build.capped) {
      ...
      const c8 = CONCURRENCIES[0];
      assert.ok(c8 !== undefined, "c8 concurrency configured");
      const latC8 = await queryHnswConcurrent(queryClient, queries, c8);
      recordRow(scale, "hnsw", "c8", latC8);
      const c16 = CONCURRENCIES[1];
      assert.ok(c16 !== undefined, "c16 concurrency configured");
      const latC16 = await queryHnswConcurrent(queryClient, queries, c16);
  ...
  try {
    ...
    const cc8 = CONCURRENCIES[0];
    assert.ok(cc8 !== undefined, "c8 concurrency configured");
    const latC8 = await queryCosineConcurrent(vectors, queries, cc8);
    recordRow(scale, "cosine", "c8", latC8);
    const cc16 = CONCURRENCIES[1];
    assert.ok(cc16 !== undefined, "c16 concurrency configured");
    const latC16 = await queryCosineConcurrent(vectors, queries, cc16);
...
async function main(): Promise<void> {
  ...
  console.log(
    `scales: ${SCALES.map((s) => s.toLocaleString()).join(", ")} | K=${K_QUERIES} queries | DIM=${DIM} | TOP_K=${TOP_K} | EF=${EF} | BATCH=${BATCH}`,
  );
  console.log(
    `concurrency: ${CONCURRENCIES.join(", ")} | build deadlines(ms): ${SCALES.map((s) => `${s.toLocaleString()}=${BUILD_DEADLINE_MS[s]}`).join(", ")}`,
  );
  console.log(``);
  ...
  try {
    ...
    const nsList = rootInfo?.namespaces ? Object.keys(rootInfo.namespaces) : [];
    const leftover = nsList.filter((n) => n.startsWith("bench_hnsw_"));
    if (leftover.length === 0) {
      console.log(
        `OK — no bench_hnsw_* namespaces remain (clean). All ${nsList.length} root namespaces: ${nsList.join(", ") || "(none)"}`,
      );
    } else {
