 ...
 */
import { containment, nearDupTokens } from "../src/store/near-dup.ts";
...
const NEAR_CANDIDATES: Array<[string, string, string]> = [
  [
    "gh ship waits for remote CI; use squash merge immediately after local checks pass",
    "after local checks pass squash merge immediately; gh ship blocks on remote CI",
    "paraphrase reorder",
  ],
  [
    "gh ship waits for remote CI; use squash merge immediately after local checks pass",
    "use squash merge right after local checks pass",
    "subset",
  ],
  [
    "Never top-level cd in this repo; use a subshell like cd dir and cmd or pass cwd",
    "avoid top-level cd; prefer a subshell or the cwd flag",
    "paraphrase",
  ],
  [
    "Never top-level cd in this repo; use a subshell like cd dir and cmd or pass cwd",
    "use the cwd flag or a subshell instead of top-level cd",
    "compress",
  ],
  [
    "Run bun install from bun-apps never from the repo root; bun lock is canonical",
    "bun lock is the canonical lockfile; run bun install inside bun-apps not the repo root",
    "reorder",
  ],
  [
    "Run bun install from bun-apps never from the repo root; bun lock is canonical",
  ...
  [
    "Response language is force-controlled by responseLanguage in settings json",
    "the reply language is governed by responseLanguage in settings json",
    "synonym",
  ],
  [
    "Response language is force-controlled by responseLanguage in settings json",
    "settings json responseLanguage force-controls reply language",
    "reorder",
  ],
  [
    "MLX dtypes use bfloat16 native; quantize mlx-8bit default or 4-bit; no FP8",
    "MLX uses bfloat16 natively; quantization is mlx-8bit default or 4-bit; FP8 unsupported",
    "expand",
  ],
  [
    "MLX dtypes use bfloat16 native; quantize mlx-8bit default or 4-bit; no FP8",
    "MLX dtypes bfloat16 native mlx-8bit or 4-bit quant no FP8",
    "compress",
  ],
  [
    "SurrealDB syncMemoryEntriesBatch has an OR-chain parser-recursion bug at scale",
    "syncMemoryEntriesBatch in SurrealDB recurses on OR-chains at scale",
    "reorder",
  ],
  [
    "SurrealDB syncMemoryEntriesBatch has an OR-chain parser-recursion bug at scale",
    "SurrealDB has a parser-recursion bug in syncMemoryEntriesBatch",
    "subset",
  ],
  [
    "pi wayfinder produces decisions not deliverables; plan do not do by default",
    "wayfinder is planning by default it yields decisions not deliverables",
    "paraphrase",
  ],
  [
    "pi wayfinder produces decisions not deliverables; plan do not do by default",
    "wayfinder decisions not deliverables",
    "subset",
  ],
  [
    "memory dedup is MD-layer only; SQLite and Surreal do identity-only sync dedup",
    "dedup lives in the MD layer only; SQLite and Surreal sync dedup is identity-only",
    "paraphrase",
  ],
  [
    "memory dedup is MD-layer only; SQLite and Surreal do identity-only sync dedup",
    "MD-layer dedup only; SQLite and Surreal identity-only",
    "compress",
  ],
  [
    "gh ship waits for remote CI; use squash merge immediately after local checks pass",
    "after local verification succeeds merge via squash at once; do not block on remote CI",
    "synonym rewrite",
  ],
  [
    "Never top-level cd in this repo; use a subshell like cd dir and cmd or pass cwd",
    "do not change directory at top level; invoke commands inside a subshell or supply the working directory flag",
    "synonym rewrite",
  ],
  [
    "Run bun install from bun-apps never from the repo root; bun lock is canonical",
    "install dependencies from the bun-apps workspace not the repository root; the lockfile of record is bun lock",
    "synonym rewrite",
  ],
  [
    "Response language is force-controlled by responseLanguage in settings json",
    "the assistant tongue is pinned by the responseLanguage key inside settings json",
    "synonym rewrite",
  ],
  [
    "MLX dtypes use bfloat16 native; quantize mlx-8bit default or 4-bit; no FP8",
    "MLX dtypes use bfloat16 native; quantize mlx-8bit default or 4-bit; no FP8; SDPA only no CUDA",
    "superset",
  ],
  [
    "pi wayfinder produces decisions not deliverables; plan do not do by default",
    "pi wayfinder produces decisions not deliverables; plan do not do by default; one decision per ticket",
    "superset",
  ],
];
...
function nearDupPairs(): GoldenPair[] {
  return NEAR_CANDIDATES.map(([a, b, note]) => {
    ...
    return {
      ...
      a,
      b,
      label: "near-dup" as const,
      expectedLayers: ["near-dup" as const],
      containment: round(max),
      containmentAB: round(ab),
      containmentBA: round(ba),
      note,
...
const TOPIC_CANDIDATES: Array<[string, string, string]> = [
  ...
  [
    "await merge ship squash merge after local ci passes green",
    "await merge ship never use the auto flag waiting for remote",
    "family await_merge_ship",
  ],
  [
    "await merge ship squash merge after local ci passes green",
    "await merge ship rebase onto origin main before charting map",
    "family await_merge_ship",
  ],
  [
    "await merge ship never use the auto flag waiting for remote",
  ...
  [
    "memory dedup lives in the MD layer only canonically",
    "memory dedup lives behind a warn do not block policy",
    "family memory_dedup_lives",
  ],
  [
    "memory dedup lives in the MD layer only canonically",
    "memory dedup lives outside the SQLite sync identity path",
    "family memory_dedup_lives",
  ],
  [
    "memory dedup lives behind a warn do not block policy",
  ...
  [
    "wayfinder tickets decisions not tasks by design default",
    "wayfinder tickets decisions resolve one question each session",
    "family wayfinder_tickets_decisions",
  ],
  [
    "wayfinder tickets decisions not tasks by design default",
    "wayfinder tickets decisions graduate from the fog of war",
    "family wayfinder_tickets_decisions",
  ],
  [
    "wayfinder tickets decisions resolve one question each session",
  ...
  [
    "dtypes bfloat16 native on Apple Silicon MLX pipeline",
    "dtypes bfloat16 native means no float32 fallback path",
    "family dtypes_bfloat16_native",
  ],
  [
    "dtypes bfloat16 native on Apple Silicon MLX pipeline",
    "dtypes bfloat16 native quantize to mlx-8bit by default",
    "family dtypes_bfloat16_native",
  ],
  [
    "dtypes bfloat16 native means no float32 fallback path",
  ...
  [
    "install only inside the bun-apps workspace root dir",
    "install only inside bun-apps per the monorepo SOP rule",
    "family install_only_inside",
  ],
  [
    "install only inside the bun-apps workspace root dir",
    "install only inside bun-apps to keep the lockfile canonical",
    "family install_only_inside",
  ],
  [
    "install only inside bun-apps per the monorepo SOP rule",
...
export const GOLDEN_STATS = {
  ...
  byLabel: GOLDEN_PAIRS.reduce(
    (m, p) => {
      m[p.label] = (m[p.label] || 0) + 1;
      return m;
    },
    {} as Record<DedupLabel, number>,
  ),
  nearDupContainment: GOLDEN_PAIRS.flatMap((p) =>
    p.label === "near-dup" && p.containment != null ? [p.containment] : [],
  ).sort((a, b) => a - b),
