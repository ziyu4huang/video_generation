...
import { DEFAULT_NEAR_DUP_THRESHOLD, findNearDuplicate } from "../src/store/near-dup.ts";
import { findTopicRecurrence } from "../src/store/topic-key.ts";
import { type DedupLabel, GOLDEN_PAIRS } from "./dedup-golden-corpus.ts";
...
const topicHit = (a: string, b: string) => findTopicRecurrence(b, [a]) !== null || findTopicRecurrence(a, [b]) !== null;
...
const nearDefault = sweep.find((s) => s.threshold === DEFAULT_NEAR_DUP_THRESHOLD) ?? sweep[0];
const best = sweep.reduce((a, b) => (b.f1 > a.f1 ? b : a), sweep[0]);

const byLabel = GOLDEN_PAIRS.reduce(
  (m, p) => {
...
L.push("");
L.push(
  `Corpus: **${GOLDEN_PAIRS.length} pairs** — exact-dup ${byLabel["exact-dup"]}, near-dup ${byLabel["near-dup"]}, topic-recurrence ${byLabel["topic-recurrence"]}, non-dup ${byLabel["non-dup"]}.`,
);
L.push("");
L.push(
...
L.push("|---|---|---|---|---|---|---|---|---|---|---|---|");
const rowm = (r: LayerResult) =>
  `| ${r.layer} | ${r.threshold ?? "—"} | ${f3(r.p)} | ${f3(r.r)} | ${f3(r.f1)} | ${r.tp} | ${r.fp} | ${r.fn} | ${r.tn} | ${r.pos} | ${r.neg} | ${r.crossFire} |`;
L.push(rowm(exact));
...
for (const s of sweep)
  L.push(`| ${s.threshold} | ${f3(s.p)} | ${f3(s.r)} | ${f3(s.f1)} | ${s.tp} | ${s.fp} | ${s.fn} |`);
L.push("");
L.push(
  `- **Current default (${DEFAULT_NEAR_DUP_THRESHOLD}):** P=${f3(nearDefault.p)} R=${f3(nearDefault.r)} F1=${f3(nearDefault.f1)}`,
);
...
L.push("");
L.push(
  `- **exact layer:** recall ${pct(exact.r)} on ${exact.pos} normalize-equal pairs (corpus invariant — exact pairs are normalize-equal by construction; measures the invariant, not layer capability).`,
);
L.push(
  `- **near-dup @${DEFAULT_NEAR_DUP_THRESHOLD}:** catches ${nearDefault.tp}/${nearDefault.pos} semantic near-dups (R=${pct(nearDefault.r)}); F1=${f3(nearDefault.f1)}.`,
);
L.push(
  `- **near-dup best F1:** threshold ${best.threshold} → F1=${f3(best.f1)} (Δ vs current ${f3(best.f1 - nearDefault.f1)}).`,
);
L.push(
  `- **topic-recurrence:** catches ${topic.tp}/${topic.pos} recurrence pairs (R=${pct(topic.r)}); crossFire=${topic.crossFire}.`,
);
L.push(
