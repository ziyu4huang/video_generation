...
describe("MemorySerializer (extracted §-md)", () => {
  ...
  it("serialize→deserialize is byte-identical to memory-format for one entry", () => {
    const card: Card = {
      id: "uuid-1",
      kind: "memory",
      content: "prefers MLX bf16",
      frontmatter: { id: "uuid-1", created: "2026-08-09", last: "2026-08-09" },
    };
    const frag = ser.serialize(card);
    const expected = serializeMetadataFrontmatter({
      id: "uuid-1",
      text: "prefers MLX bf16",
      created: "2026-08-09",
      last: "2026-08-09",
    });
  ...
  it("deserialize splits a multi-entry section-md file into N cards", () => {
    const file = [
      ser.serialize({
        ...
        kind: "memory",
        content: "one",
        frontmatter: { id: "a", created: "2026-08-09", last: "2026-08-09" },
      }),
      ser.serialize({
        id: "b",
        kind: "memory",
        content: "two",
        frontmatter: { id: "b", created: "2026-08-09", last: "2026-08-09" },
      }),
  ...
  it("preserves content + frontmatter through round-trip", () => {
    const card: Card = {
      ...
      kind: "memory",
      content: "body text",
      frontmatter: { id: "uuid-2", created: "2026-08-09", last: "2026-08-09" },
    };
