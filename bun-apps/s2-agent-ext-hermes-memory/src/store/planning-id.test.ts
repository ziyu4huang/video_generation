...
import { parsePlanningPath, planningCardKindFromPath, planningEffortId, planningTicketId } from "./planning-id.js";

describe("planning-id", () => {
  it("classifies effort + ticket paths", () => {
    assert.equal(planningCardKindFromPath(".planning/2026-08-08-knowledge-pipeline/map.md"), "planning-effort");
    assert.equal(
      planningCardKindFromPath(".planning/2026-08-08-knowledge-pipeline/tickets/08-planning-card-model.md"),
      "planning-ticket",
    );
  });
  it("rejects non-planning-card paths", () => {
    assert.equal(planningCardKindFromPath(".planning/specs/foo.md"), null);
    assert.equal(planningCardKindFromPath(".planning/2026-08-08-knowledge-pipeline/specs/bar.md"), null);
    assert.equal(planningCardKindFromPath(".planning/2026-08-08-knowledge-pipeline/plans/baz.md"), null);
  ...
  it("parses effort + ticketNo/slug", () => {
    ...
    assert.deepEqual(parsePlanningPath(".planning/my-effort/tickets/08-planning-card-model.md"), {
      kind: "planning-ticket",
      effort: "my-effort",
      ticketNo: "08",
      slug: "planning-card-model",
    });
  });
