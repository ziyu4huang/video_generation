...
## Question
Pin the contract. See bun-apps/s2-agent-ext-hermes-memory/src/store/card.ts
...
describe("planning-parse", () => {
  ...
  it("extracts a one-line resolution gist (matches '## Resolution (…)')", () => {
    ...
    assert.ok(body, "fixture has a body");
  ...
  it("extracts cited repo-relative paths (deduped)", () => {
    ...
    assert.ok(paths.includes("bun-apps/s2-agent-ext-hermes-memory/src/store/card.ts"));
...
describe("parseDependsOn", () => {
  it("accepts an explicit array of repo-relative paths", () => {
    assert.deepEqual(parseDependsOn(["bun-apps/x/src/a.ts", "docs/spec.md"]), ["bun-apps/x/src/a.ts", "docs/spec.md"]);
  });
  ...
  it("accepts a comma/newline list and trims + drops empties", () => {
    assert.deepEqual(parseDependsOn("src/a.ts, src/b.ts\n , docs/c.md"), ["src/a.ts", "src/b.ts", "docs/c.md"]);
  });
