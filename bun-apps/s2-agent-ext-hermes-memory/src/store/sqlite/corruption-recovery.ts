...
import { RawDatabase as Database, type DatabaseLike, quoteIdentifier } from "./sqlite-backend.js";
...
export function copySessions(source: DatabaseLike, target: DatabaseLike): number {
  ...
  for (const row of readTableRows(source, "sessions", [
    "id",
    "project",
    "cwd",
    "started_at",
    ...
    insert.run(row.id, project, row.cwd, row.started_at, nullableString(row.ended_at), integerOr(row.message_count, 0));
    copied++;
  }
...
export function copyMessages(source: DatabaseLike, target: DatabaseLike): number {
  ...
  for (const row of readTableRows(source, "messages", [
    "id",
    "session_id",
    "role",
    "content",
...
export function readTableRows(
  source: DatabaseLike,
  table: string,
  desiredColumns: readonly string[],
