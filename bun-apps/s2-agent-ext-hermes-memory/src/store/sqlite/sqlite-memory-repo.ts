...
import type { DatabaseLike, SqliteBackend } from "./sqlite-backend.js";
...
function runExclusive<T>(db: DatabaseLike, fn: () => T): T {
  ...
  try {
    ...
    try {
      ...
      /* tx may already have rolled back */
    }
    throw err;
  }
}
...
export class SqliteMemoryRepository implements MemoryRepository {
  ...
  private syncOneInTx(input: MemorySyncInput): MemorySyncResult {
    ...
    return {
      ...
      entry: this.getMemoryById(existing.id) ?? ({ id: existing.id } as never),
    };
  }
  ...
  async getMdIdByContent(content: string, options: MemoryRemoveOptions): Promise<string | null> {
    return runWithTransientRetry(() =>
      this.backend.withCorruptionRecovery(() => {
        ...
        params.push(content.trim());
        const row = this.db
          .prepare(`SELECT md_id AS mdId FROM memories WHERE ${conditions.join(" AND ")} LIMIT 1`)
          .get(...params) as { mdId?: string | null } | undefined;
  ...
  async setMdIdByContent(content: string, mdId: string, options: MemoryRemoveOptions): Promise<number> {
    return runWithTransientRetry(() =>
      this.backend.withCorruptionRecovery(() => {
        ...
        params.push(content.trim());
        const matchingIds = this.db
          .prepare(`SELECT id FROM memories WHERE ${conditions.join(" AND ")}`)
          .all(...params) as Array<{ id: number }>;
  ...
  async bumpMemoryWorth(id: number, successDelta = 0, failDelta = 0): Promise<void> {
    return runWithTransientRetry(() =>
      this.backend.withCorruptionRecovery(() => {
        ...
        if (failDelta !== 0) {
          this.db
            .prepare("UPDATE memories SET mw_fail = mw_fail + ? WHERE id = ? AND state = 'active'")
            .run(failDelta, id);
        }
      }),
    );
      ...
      this.backend.withCorruptionRecovery(() =>
        runExclusive(this.db, () => {
          this.db
            ...
            .run(newId, priorId);
          this.db
            .prepare("UPDATE memories SET supersedes = ?, parent_ids = ? WHERE id = ?")
            .run(priorId, JSON.stringify([priorId]), newId);
        }),
      ),
    );
  }
