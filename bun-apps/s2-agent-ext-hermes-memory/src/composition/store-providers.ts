 ...
 */
export function injectStoreProviders(pi: ExtensionAPI, ctx: HermesCtx, memoryToolDef: ToolDefinition): void {
  ...
  if (ctx.projectStore) {
    ctx.projectStore.setConsolidator(
      async (snapshot, signal) =>
        produceMergePlan(snapshot, {
          timeoutMs: ctx.config.consolidationTimeoutMs,
          signal,
  ...
  ctx.store.setStableIdBackfillProvider({
    ...
    setMdIdByContent: (target, content, mdId) =>
      ctx.memoryRepo.setMdIdByContent(content, mdId, { target, project: null }),
  });
  if (ctx.projectStore) {
    ctx.projectStore.setStableIdBackfillProvider({
      getMdIdByContent: (target, content) =>
        ctx.memoryRepo.getMdIdByContent(content, { target, project: ctx.projectName }),
      setMdIdByContent: (target, content, mdId) =>
        ctx.memoryRepo.setMdIdByContent(content, mdId, { target, project: ctx.projectName }),
    });
  }
  ...
  if (shouldWireHeat(ctx.config)) {
    ctx.store.setHeatForEntriesProvider(
      makeHeatProvider(ctx.config, { memoryRepo: ctx.memoryRepo, sessionRepo: ctx.sessionRepo }, null),
    );
    if (ctx.projectStore) {
      ctx.projectStore.setHeatForEntriesProvider(
        makeHeatProvider(ctx.config, { memoryRepo: ctx.memoryRepo, sessionRepo: ctx.sessionRepo }, ctx.projectName),
      );
    }
  }
  ...
  registerConsolidateCommand(
    ...
    ctx.store,
    memoryToolDef,
    ctx.config.consolidationTimeoutMs,
    ctx.projectStore,
    ctx.projectName,
    ctx.config,
  );
