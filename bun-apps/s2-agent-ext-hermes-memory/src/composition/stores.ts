...
import type { BackendBundle, MemoryRepository, SessionRepository } from "../store/repository.js";
...
export async function createStores(runtime: BackendRuntime, config: MemoryConfig): Promise<Stores> {
  ...
  const projectStoreDir =
    resolvedProjectStoreDir && !canHoldProjectStore(resolvedProjectStoreDir) ? null : resolvedProjectStoreDir;
  ...
  const inRepoProjectFile =
    config.projectMemoryDir !== null && projectStoreDir ? path.join(projectStoreDir, MEMORY_FILE) : null;
  ...
  if (initialBackend.fellBackTo) {
    ...
    console.warn(
      `[hermes-memory] configured backend "${config.dbBackend}" failed to initialize — fell back to sqlite for this session. Start the server and /memory-switch-backend ${config.dbBackend} to restore.`,
    );
  }
...
export async function initStores(runtime: BackendRuntime, config: MemoryConfig, stores: Stores): Promise<void> {
...
export function setupBackendSwitching(pi: ExtensionAPI, ctx: BackendSwitchingCtx): void {
  ...
  const switchTo = async (target: DbBackend): Promise<{ ok: boolean; message: string }> => {
    ...
    try {
      ...
      return {
        ok: false,
        message: `failed to initialize ${target}: ${err instanceof Error ? err.message : String(err)}`,
      };
    }
