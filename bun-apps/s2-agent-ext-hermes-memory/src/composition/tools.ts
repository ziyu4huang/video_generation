...
export function registerTools(pi: ExtensionAPI, ctx: HermesCtx): { memoryToolDef: ToolDefinition } {
  ...
  registerKnowledgeIngestTool(pi, {
    ...
    hierarchy: {
      ...
      embedFn: async (texts: string[]) =>
        defaultEmbedder({ baseUrl: config.lmStudioBaseUrl ?? "http://127.0.0.1:1234" })(texts, SEMANTIC_MODEL_DEFAULT),
    },
  });
...
export function registerSearchAndIndexSessions(pi: ExtensionAPI, ctx: HermesCtx, recallSet: RecallSet): void {
