...
describe("parseContradictionVerdict", () => {
  it("parses a contradicted_id", () => {
    assert.deepEqual(parseContradictionVerdict('{"contradicted_id": 42, "reason": "says npm not pnpm"}'), {
      contradictedId: 42,
    });
  });
  it("parses null (no contradiction)", () => {
    assert.deepEqual(parseContradictionVerdict('{"contradicted_id": null, "reason": "none"}'), {
      contradictedId: null,
    });
  });
