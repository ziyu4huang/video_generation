...
import { parseMetadataFrontmatter, serializeMetadataFrontmatter } from "../../src/store/memory-format.js";
import type { MemoryEntry, MemoryRepository } from "../../src/store/repository.js";
...
describe("memory sqlite sync + markdown backfill", () => {
  ...
  it("backfill command is idempotent across repeated runs", async () => {
    ...
    const userEntries = [fm("md-idem-u1", "name: Chandra", "2026-05-08", "2026-05-08")];
    const failureEntries = [fm("md-idem-f1", "[tool-quirk] npm cache stale — Failed: clear .cache/tsx")];
    ...
    fs.writeFileSync(path.join(projectDir, "MEMORY.md"), fm("md-idem-p1", "project memory entry"), "utf-8");
    ...
    assert.strictEqual(
      firstMem.length + firstUser.length + firstFail.length,
      5,
      "first run should mirror all unique entries",
    );
  ...
  it("multi-file sync reads each KIND once, not once per file (2026-08-25 kind-index fix)", async () => {
    ...
    fs.writeFileSync(
      ...
      fm("md-kindidx-f1", "[failure] kind index failure").trim(),
      "utf-8",
    );
    for (let i = 1; i <= 5; i++) {
      ...
      fs.writeFileSync(
        ...
        fm(`md-kindidx-p${i}`, `kind index project ${i}`).trim(),
        "utf-8",
      );
    }
    const inner = await makeCardStore();
  ...
  it("dirty multi-entry sync updates via ONE updateCardsBatch, not per-entry updateCard (2026-08-25)", async () => {
    ...
    assert.ok(
      cards.every((c) => c.frontmatter.last === "2026-05-20"),
      "every envelope refreshed",
    );
  });
  ...
  it("backfills legacy project memory directories from the old ~/.pi/agent/<project> layout", async () => {
    ...
    fs.writeFileSync(path.join(legacyProjectDir, "MEMORY.md"), fm("md-legacy-1", "legacy project entry"), "utf-8");
  ...
  it("backfills the in-repo project memory file (.agents/memory/) tagged with the project name (ticket 04)", async () => {
    ...
    fs.writeFileSync(path.join(inRepoDir, "MEMORY.md"), fm("md-inrepo-1", "in-repo project convention"), "utf-8");
  ...
  it("search merges legacy global + in-repo project entries for the same project (ticket 05 merge pin)", async () => {
    ...
    const inRepoDir = path.join(tmpDir, "repo", ".agents", "memory");
    fs.mkdirSync(inRepoDir, { recursive: true });
    fs.writeFileSync(path.join(inRepoDir, "MEMORY.md"), fm("md-split-inrepo-1", "in-repo project entry"), "utf-8");
    ...
    assert.deepStrictEqual(cards.map((c) => c.content).sort(), [
      "in-repo project entry",
      "legacy global project entry",
    ]);
  });
  ...
  it("backfill sets failure state by category for stateless entries + mirrors to DB (Task 6)", async () => {
    ...
    fs.writeFileSync(path.join(globalDir, "failures.md"), [failureEntry, quirkEntry].join(ENTRY_DELIMITER), "utf-8");
  ...
  it("failure-state backfill reports counts + stoppedInjecting (Task 8 dry-run)", async () => {
    ...
    assert.ok(
      fstate.stoppedInjecting.some((s) => s.includes("known quirk")),
      "tool-quirk flagged as stopped injecting",
    );
