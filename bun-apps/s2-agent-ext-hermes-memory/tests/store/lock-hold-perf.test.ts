...
import { createPerfRecorder, type PerfRecord, type TimedFn } from "../../src/perf.js";
...
describe("MemoryStore lock-hold perf (T2)", { concurrency: 1 }, () => {
  ...
  beforeEach(async () => {
    try {
      await fs.promises.rm(path.join(MEMORY_DIR, "MEMORY.md"), { force: true });
    } catch {
      /* ignore */
    }
    ...
    try {
      await fs.promises.rm(path.join(MEMORY_DIR, "MEMORY.md.lock"), { recursive: true, force: true });
    } catch {
      /* ignore */
    }
  });
