...
import {
  RawDatabase as Database,
  SQLITE_WAL_AUTOCHECKPOINT_PAGES,
  SqliteBackend,
} from "../../src/store/sqlite/sqlite-backend.js";
...
describe("SqliteBackend", () => {
  ...
  function corruptRecoverableIndexPage(dbPath: string, indexName: string): void {
    ...
    const row = db.prepare("SELECT rootpage FROM sqlite_schema WHERE type = 'index' AND name = ?").get(indexName) as
      | { rootpage: number }
      | undefined;
    db.close();
  ...
  describe("schema", () => {
    ...
    it("should add mw_success/mw_fail columns to a legacy memories table lacking them", () => {
      ...
      const row = migratedDb.prepare("SELECT mw_success, mw_fail FROM memories").get() as {
        mw_success: number;
        mw_fail: number;
      };
    ...
    it("should migrate legacy sessions table without project column", () => {
      ...
      const row = migratedDb.prepare("SELECT project FROM sessions WHERE id = ?").get("legacy-session") as {
        project: string;
      };
    ...
    it("should migrate legacy target CHECK constraint to allow failure entries", () => {
      ...
      const rows = migratedDb.prepare(`SELECT target, content FROM memories ORDER BY id ASC`).all() as Array<{
        target: string;
        content: string;
      }>;
  ...
  describe("corruption recovery", () => {
    it("repairs recoverable corruption on open and preserves readable rows", () => {
      ...
      for (let i = 0; i < 50; i++) {
        insertMessage.run(
          ...
          `message ${i}`,
          `2026-05-03T00:${String(i).padStart(2, "0")}:00Z`,
        );
      }
      ...
      const memory = repairedDb.prepare("SELECT content FROM memories WHERE content = ?").get("recoverable memory") as
        | { content: string }
        | undefined;
      assert.ok(memory);
      ...
      const card = repairedDb
        .prepare("SELECT md_id, state, severity, pin, frontmatter, graph FROM memories WHERE md_id = ?")
      ...
      assert.ok(
        fs.readdirSync(tmpDir).some((name) => name.startsWith("sessions.db.corrupt-")),
        "corrupt DB should be quarantined",
      );
    });
    ...
    it("quarantines unrecoverable files and recreates an empty database", () => {
      ...
      assert.ok(
        fs.readdirSync(tmpDir).some((name) => name.startsWith("sessions.db.corrupt-")),
        "unrecoverable DB should be quarantined",
      );
    });
  ...
  describe("stale backup pruning (fixes unbounded quarantine clutter)", () => {
    ...
    it("pruneStaleBackups keeps the N newest clutter files, deletes the rest", () => {
      ...
      const remaining = fs
        .readdirSync(tmpDir)
        .filter((n) => /^sessions\.db\.(corrupt|bak)-/.test(n))
        .sort();
