 ...
 * directive from the era when s2-agent's static import first exposed this
 * package to a type-checker (see s2-agent/src/static-extensions.ts). The errors
 * it silenced were fixed by later store refactors, but nobody came back to
 * remove it — and s2-agent's own header had been updated to claim "both
