...
**Protocol message** (`send_message` `type` envelopes, ticket 04):
A TYPED envelope on `send_message` — `shutdown_request` / `shutdown_response` / `plan_approval_request` / `plan_approval_response` (+ `approve`/`feedback` on responses) — as opposed to a plain follow-up message. One tool, matching Claude Code: the envelope changes the DELIVERY semantics (resolve a pending hold, run a two-stage stop, fire a notification), never the routing (still `to:` name/agentId/`main` through the same registry and bus). In-process only: a detached resume subprocess (marked by `SUBAGENT_DETACHED_RESUME` in its env) refuses every child-origin protocol message with a clear error — its parent lives in another OS process, and no bus or pending map it can reach is the real one.
_Avoid_: "kill"/"cancel" for the shutdown envelope (those are ABORTS — the mechanism stage 2 uses, not the protocol); inventing per-envelope tools (one tool, envelope-typed); "request" as a synonym for the whole layer (plan approval is one of the two exchanges).

**Plan approval** (`request_plan_approval` child-injected tool + `plan_approval_response` envelope, ticket 04):
The child's half: a named live agent calls the CHILD-INJECTED `request_plan_approval` tool (never parent-registered — it joins neither the active set nor the workflow gate family; children get it via the extensionTools bridge plus an allowlist append on the named-dispatch path). Its execute holds ONE entry on core-runtime's **PendingProtocolMap** (keyed by the child's self-declared name), wakes the parent through the ParentMessageBus, and awaits the verdict. The parent's half: `send_message {type:"plan_approval_response", approve}` resolves the hold. Timeout defaults to DENY (map D6 — budget-safe; never block a dispatch forever); a session end drops every hold default-deny.
_Avoid_: "permission" (ext-permission's word — a different, host-owned layer); "confirmation" (too weak — the deny has teeth: the child must not proceed); calling the hold a "lock" (nothing is locked; the child simply waits).

**Shutdown handshake** (parent→child `shutdown_request`, ticket 04):
Two-stage, mirroring the budget guard's `BUDGET_WRAP_UP_MESSAGE` semantics: stage 1 delivers the wrap-up notice (a steer when mid-flight, a final prompt when idle) bounded by the grace window (default 60s, `timeoutMs` overrides); stage 2 stops the session — registry release disposes the LiveAgent, aborting any in-flight exchange. One abort lever per request (the same grace window caps the prompt path and the backstop timer). Child→parent `shutdown_request` is NOTIFICATION ONLY: the parent "approves by stopping" — `list_subagent_runs {action:"stop"}` accepts the agent's name/agentId (ticket 04) as that lever. `shutdown_response` is the acknowledgment envelope, either direction.
_Avoid_: "kill"/"cancel" (see protocol message); "terminate" (the registry's word for a LIFETIME-CEILING release — budget/turns exhaustion — which is a different exit than a requested shutdown).
