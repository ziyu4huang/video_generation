 ...
 * `@repo/s2-agent-core-runtime` symbols that no peer package reached for.
 ...
 *
 * The facade is not simply deletable, which is the subtle part. `s2-agent`,
 * `s2-agent-ext-obsidian`, `s2-agent-ext-file2md` and `s2-agent-ext-knowledge-card`
 * do NOT declare `@repo/s2-agent-core-runtime` in package.json, so for them
...
const CORE = "@repo/s2-agent-core-runtime";
...
const FACADE_SYMBOLS: Record<string, string> = {
  ...
  setRateLimitCapResolver: "s2-agent-ext-subagent/tests/rate-limiter-cross-pkg.test.ts",
};
...
function importsThroughBarrel(file: string, name: string): boolean {
  ...
  const re =
    /(?:import|export)\s+(?:type\s+)?\{([^}]*)\}\s*from\s*["'](?:@repo\/s2-agent-ext-subagent|(?:\.\.\/)+src\/index\.js|\.\/index\.js)["']/g;
