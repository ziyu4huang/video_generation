#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for pi-agent-ext-grill-memory.
#   quick (default) — this package's existing test command (biome check + bun test), unchanged.
...
print_list() {
	cat <<EOF
$(Y "pi-agent-ext-grill-memory run-test.sh — tiers"):
  $(G quick)  biome check . && bun test (this package's existing test command)  $(Y "[default]")
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/pi-agent-ext-grill-memory-runtest.log 2>&1
	rc=$?
	...
	if [ "$rc" -ne 0 ]; then sed 's/^/      /' /tmp/pi-agent-ext-grill-memory-runtest.log | tail -n 25 >&2; fi
}

run_quick() { ( cd "$SCRIPT_DIR" && bun run test ${EXTRA[@]+"${EXTRA[@]}"} ) ; }
run_contract() { ( cd "$SCRIPT_DIR" && bun test tests/extension-contract.test.ts ) ; }

echo "$(Y "▶ pi-agent-ext-grill-memory run-test.sh — tier=$TIER")"
case "$TIER" in
