// @ts-nocheck
/**
 * test-krea2-e2e.js — canonical L2 judgment e2e for pi-agent-ext-krea2.
 *
 ...
 *   `[krea2] saved` parse, path safety) is covered by `pi-agent-ext-krea2/src/*.test.ts`.
 *   This workflow only covers the judgment layer.
 *
 * INVOCATION (unified runner)
 *   bash bun-apps/pi-agent/scripts/run-ext-e2e.sh krea2
 *   # or directly:
 *   bun-apps/pi-agent/run.sh -e workflow -p \
 *     "read bun-apps/pi-agent-ext-krea2/workflows/test-krea2-e2e.js and execute it via the workflow tool (background:false)"
 */

export const meta = {
  name: "test_krea2_e2e",
  description:
    "L2 judgment e2e for pi-agent-ext-krea2: generate via the krea2 CLI (native Swift Krea 2 Turbo), then VLM-score each output (run.py caption --style score). Pass = every output clears conservative quality thresholds.",
  phases: [
    { title: "Generate" },
    { title: "Judge" },
  ],
...
const KREA2 = REPO_ROOT + "/swift/krea2-image-director/.build/release/krea2";
const OUT_DIR = String(a.outDir || REPO_ROOT + "/../video_generation__output/wf-e2e-krea2");
const NAME = String(a.name || "wf_e2e_judge");
const SEED = Number(a.seed ?? 42);
const WIDTH = Number(a.width ?? 1024);
const HEIGHT = Number(a.height ?? 1024);
const STEPS = Number(a.steps ?? 8);
...
// complex poses — see pi-agent-ext-flux2/docs/pose-validation.md §1). When
// `poses` is absent the workflow falls back to `score` judging on `prompts`
// (the regression-tested path).
...
// Pose gate: anatomy_pass is a HARD fail; faithfulness floored at 0.8 (recomputed
// in Python by parse_pose_dsg — never trusted from the model).
...
// ─── Phase 1: generate one PNG per prompt via the krea2 CLI ─────────────────
phase("Generate");
...
const GEN_SCHEMA = {
  ...
  properties: {
    ...
    binaryReady: { type: "boolean", description: "krea2 release binary + mlx.metallib present and --help exits 0." },
    error: { type: "string", description: "Failure tail if ok=false; empty otherwise." },
  },
  additionalProperties: false,
};

log("Generating " + GEN_PROMPTS.length + " krea2 t2i output(s) at " + WIDTH + "x" + HEIGHT + (POSES.length ? " (pose_dsg mode)" : ""));
...
try {
  gen = await agent(
    [
      "Generate one krea2 t2i PNG per prompt, in order. Repo root: " + REPO_ROOT + ". Binary: " + KREA2 + ".",
      "FIRST ensure the binary is ready: " + KREA2 + " --help must exit 0. If missing, build: `swift build -c release --package-path " + REPO_ROOT + "/swift/krea2-image-director` then `bash " + REPO_ROOT + "/swift/krea2-image-director/scripts/setup-metallib.sh release`. Skip setup-metallib if .build/release/mlx.metallib already exists. Build may take minutes — use a long timeout.",
      "Create the output dir if needed: mkdir -p " + JSON.stringify(OUT_DIR),
      "Then run ONE krea2 t2i per prompt (i index 0..N-1). krea2 has NO --output-dir/--name; pass an explicit --out abspath per run:",
      "  " + KREA2 + " t2i --prompt <prompt> --seed " + SEED + " --width " + WIDTH + " --height " + HEIGHT + " --steps " + STEPS + " --out " + JSON.stringify(OUT_DIR) + "/" + NAME + "_<i>.png",
      "Each run loads the Krea 2 Turbo MMDiT — expect ~1-3 min each (native Swift, 8 steps). Use a long timeout; if your bash tool times out, run nohup ... & and poll for the PNG, then read the log.",
      "On success krea2 prints a `[krea2] saved <abspath>` line (the LAST such line is the output). Use that path — it is absolute and authoritative.",
      "Return ok=true only if EVERY prompt produced a PNG that exists with non-zero size. Return paths[] in prompt order (absolute, the `[krea2] saved` paths). Put any failure tail in `error`.",
      "Prompts (in order): " + JSON.stringify(GEN_PROMPTS),
    ].join("\n"),
    { label: "krea2 t2i generate", schema: GEN_SCHEMA },
  );
} catch (e) {
...
// holistic `score` (gate: overall/artifacts thresholds).
...
async function judgeScore(p, i) {
  try {
    const j = await agent(
      ...
      { label: "vlm score output " + (i + 1), schema: JUDGE_SCHEMA },
    );
...
// 'EOF'), then runs caption --style pose_dsg --prompt --atoms <file>.
async function judgePose(p, i, pose) {
  const atomsJson = JSON.stringify({ atoms: pose.atoms }, null, 2);
  const atomsFile = "/tmp/krea2_pose_atoms_" + i + ".json";
  try {
    const j = await agent(
      [
      ...
      { label: "vlm pose_dsg output " + (i + 1), schema: JUDGE_POSE_SCHEMA },
    );
...
// Per-pose × per-atom regression matrix — the real signal L2 is after. Stable
// atom ids let a caller diff this matrix commit-over-commit to see which atom
// regressed (e.g. dancer's-pose a4 from present→absent) even if overall is flat.
