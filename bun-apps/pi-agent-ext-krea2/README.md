...
Sister package `pi-agent-flux2` follows the same architecture for the larger Flux2 Klein CLI (18 subcommands). Krea 2's surface is tiny (2 subcommands, no gate/manifest/scene), so this is a focused, simplified port — not a copy.
...
```bash
( cd bun-apps/pi-agent-ext-krea2 && bun scripts/build-bundle.ts )
# → dist/pi-extensions/pi-agent-ext-krea2.bundle.js
```
...
```bash
( cd bun-apps/pi-agent-ext-krea2 && bun scripts/check-flags.ts )
```
