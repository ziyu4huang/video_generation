...
The ubiquitous language of pi-agent-ext-knowledge-card — Zettelkasten knowledge management: distill markdown into atomic notes, CRUD over the vault, and graph-enhanced RAG, each backed by an isolated subagent; plus a deterministic convergence sink that dissolves per-workflow knowledge silos into one shared graph.
