#!/usr/bin/env bun
/**
 * run-test.ts — tiered test launcher for s2-agent-ext-obsidian.
 *
 ...
 *                    runContract() comment).
 *
 * NOTE — obsidian's canonical `bun run test` is scoped to `bun test extensions/__tests__/` (the same scope the .sh
 ...
 * USAGE (from anywhere):
 *   bun bun-apps/s2-agent-ext-obsidian/run-test.ts              # = quick
 *   bun bun-apps/s2-agent-ext-obsidian/run-test.ts full
 *   bun bun-apps/s2-agent-ext-obsidian/run-test.ts --list       # print the tier table, exit 0
 *   bun bun-apps/s2-agent-ext-obsidian/run-test.ts quick --bail # extra flags are forwarded
...
const PKG = "s2-agent-ext-obsidian";
// extension-contract.test.ts re-asserted standalone (the `full` tier).
const CONTRACT_TEST = "extensions/__tests__/extension-contract.test.ts";
...
function printList(): void {
  process.stdout.write(
    `${Y(`${PKG} run-test.sh — tiers`)}:\n` +
      `  ${G("quick")}  bun test extensions/__tests__/ (this package's existing test command)  ${Y("[default]")}\n` +
