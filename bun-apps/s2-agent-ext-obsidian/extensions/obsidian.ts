 ...
 * — notably s2-agent's `--dry-run` (see `dryRunExclude` / `applyDryRunEnv` in
 * `bun-apps/s2-agent/src/cli/sessions/shared.ts`).
 *
...
export default function (pi: ExtensionAPI) {
	...
	// is a type-only import (no runtime symbol), and s2-agent vendors an
	...
	// became reachable via s2-agent's static import.
	const _capture = {
			...
			if (engine === "deterministic") {
				...
				try {
					...
					return {
						content: [{
							type: "text",
							text: `Deterministic health check failed (is s2-agent-ext-knowledge-card installed?): ${errMsg(e)}`,
						}],
						isError: true,
