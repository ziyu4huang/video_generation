 ...
 *
 * Lets `s2-agent` expose the web-access extension as a top-level
 * sub-command:
 *
 *   s2-agent cli research <natural-language query...>
 *   s2-agent cli --model sonnet research "RAG vs graph RAG benchmarks 2026"
 *
 ...
 * direction is s2-agent → s2-agent-ext-web-access, so the spec is typed
 ...
 * …) — s2-agent-ext-obsidian consolidated the old per-verb tool names
 ...
 * validateToolNames in s2-agent/src/cli/sessions/shared.ts).
 */
...
export const researchSubcommand: ExtensionSubcommandSpec = {
  ...
  details: `Usage:
  s2-agent cli research <natural-language query...> [options]
  s2-agent cli research <query...> --save [options]
...
Examples:
  s2-agent cli research "RAG vs graph RAG benchmarks 2026"
  s2-agent cli --model sonnet research "Apple MLX vs PyTorch MPS performance" --save
  s2-agent cli research "explain the paper at https://arxiv.org/abs/2401.00001"`,
  factory: extension,
