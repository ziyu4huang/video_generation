...
- **`dependencies` vs `peerDependencies` is correct as-is.** acorn is a functional dependency (the engine's parser), not a platform capability provided by the environment — so it is not a peer like `pi-coding-agent` / `pi-tui` / `typebox` / `pi-agent-ext-subagent`.
