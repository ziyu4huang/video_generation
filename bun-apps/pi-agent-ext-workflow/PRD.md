...
| `subagent` tool | Dispatch one isolated-context subagent outside a script; reports real usage/cost, accepts `timeoutMs`/`retryOnTransient`, `agentType` (named tool/model/prompt/isolation bindings), and `schema` (structured output) |
