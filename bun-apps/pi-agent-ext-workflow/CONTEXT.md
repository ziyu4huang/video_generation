...
The shared, **publicly-exported** wrapper over `WorkflowAgent.run` for programmatic single-subagent dispatch from peer-extension CODE (not the LLM tool path). **Moved to `@repo/pi-agent-ext-subagent`** as part of the subagent extraction; re-exported here (`pi-agent-ext-workflow` `src/index.ts`) for backward compatibility, so existing consumers importing it from the workflow package root keep compiling. New consumers SHOULD import `spawnSubagent` + `SpawnSubagentOptions` + `SpawnSubagentResult` + `AgentUsage` from `@repo/pi-agent-ext-subagent` directly. Used by `pi-agent-ext-wayfind`, `pi-agent-ext-superpowers`, `pi-agent-ext-knowledge-card` (`zk_card`/`zk_ask`). Same runner as the `subagent` tool and `agent()`; returns `{output, exitCode, stderr, timedOut, usage?}` (mirrors the old `runSubagentWithRetry` shape). `prime?` is a no-op forward-ref to ③ (experimental, NOT stable).
_Avoid_: re-implementing a child-process subagent runner in a peer extension (call this instead); importing from the workflow package root for NEW code (import from `@repo/pi-agent-ext-subagent`)
