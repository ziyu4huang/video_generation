...
The same method applies to krea2 (and any `pi-agent-ext-*`) — it lives in the
...
POSE=$(jq -r '.levels[2].poses[0]' bun-apps/pi-agent-ext-flux2/workflows/poses.json)
...
bun-apps/pi-agent/run.sh -e workflow -p \
  "read bun-apps/pi-agent-ext-flux2/workflows/test-flux2-e2e.js and execute it via the workflow tool (background:false), passing args.poses = the L3+L4 entries of bun-apps/pi-agent-ext-flux2/workflows/poses.json"
```
