...
bash bun-apps/pi-agent/scripts/run-ext-e2e.sh flux2

# or directly via the workflow tool:
bun-apps/pi-agent/run.sh -e workflow -p \
  "read bun-apps/pi-agent-ext-flux2/workflows/test-flux2-e2e.js and execute it via the workflow tool (background:false)"
```
...
combinator (`pi-agent-ext-workflow/src/workflow.ts`); the validator is pure JS
...
bash bun-apps/pi-agent/scripts/run-self-improve-loop.sh --pose-id L3-01 --attempts 5 --seed 42
bash bun-apps/pi-agent/scripts/run-self-improve-loop.sh --prompt "a red apple on a table"   # non-pose
...
**Tests.** `bun-apps/pi-agent-ext-workflow/tests/regression-self-improve-loop.test.ts`
