 ...
 *     same on-disk auth/config (`~/.pi/`, models.json) that pi-agent uses — no
 ...
 */
import { runWorkflow } from "../../pi-agent-ext-workflow/dist/workflow.js";
...
await import("../../pi-agent/src/patches/pre-load-providers.ts");
...
const WF_PATH = path.join(REPO_ROOT, "bun-apps/pi-agent-ext-flux2/workflows/self-improve-flux2.js");
...
async function main() {
  ...
  if (!mock && !wfArgs.fewShot) {
    ...
    if (targetBase) {
      const exemplarFile = path.join(REPO_ROOT, "bun-apps/pi-agent-ext-flux2/workflows/self-improve-flux2.exemplars.jsonl");
  ...
  const exemplarFile = (r.exemplarsFile as string) || path.join(REPO_ROOT, "bun-apps/pi-agent-ext-flux2/workflows/self-improve-flux2.exemplars.jsonl");
