export type FieldDef =
  ...
  | { type: "select"; key: string; label: string; options?: { value: string; label: string }[]; choicesFrom?: string; default?: string; hint?: (state: Record<string, any>, ctx: { inputDims: { w: number; h: number } | null }) => string; visible?: (state: Record<string, any>) => boolean }
