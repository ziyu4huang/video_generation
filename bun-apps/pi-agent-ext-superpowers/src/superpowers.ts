...
Pi does not ship a standard subagent tool in core. This repo's pi-agent-ext-workflow provides a 'subagent' tool (an isolated-context child via spawnSubagent). Use subagent({ task, model, tools, excludeTools, cwd, commitScope, tokenBudget, spendBudget }) for Superpowers subagent workflows - pass a self-contained 'task' (the child has no access to this session's history). For SDD implementer/fix dispatches, pass commitScope with the task's declared file scope (from the brief) so the tool flags any out-of-scope committed path - the recurring 'git add -A' sweep that stages .planning/<effort>/sdd/ scratch into a commit, which then lands on main at squash-merge - as a warning violation (detection only; you decide the revert). Use [] for a read-only subagent that should commit nothing. For expensive or open-ended dispatches (exploratory research, large refactors, generous timeoutMs), consider passing tokenBudget/spendBudget to bound runaway spend - the run aborts mid-run (status 'budget') if exceeded, catching a looping agent that wall-clock timeoutMs alone cannot. Soft guidance: a well-scoped implementer rarely needs it. If no 'subagent' tool is available, do the work in this session or explain the missing capability instead of inventing Task calls.
