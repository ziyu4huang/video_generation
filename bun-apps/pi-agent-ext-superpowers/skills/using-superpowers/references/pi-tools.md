...
Pi core does not ship a standard subagent tool. This repo's `pi-agent-ext-workflow` provides a `subagent` tool — a single-agent, isolated-context dispatch (`subagent({ task, model, tools, excludeTools, cwd, schema, agentType, timeoutMs, retryOnTransient })`) backed by `spawnSubagent()`. It covers SDD's implementer/reviewer dispatch.
