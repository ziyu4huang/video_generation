/**
 * The freshness gate: manifest.json is derived from s2-agent.registry.yaml.
...
describe("manifest.json freshness", () => {
	test("is byte-identical to what the registry generates", () => {
		const registry = parseRegistry(readFileSync(join(PKG_DIR, "s2-agent.registry.yaml"), "utf8"), {
