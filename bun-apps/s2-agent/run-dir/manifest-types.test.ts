...
describe("parseManifestEntry", () => {
	...
	test("bare string with index.ts → uses parent dir name", () => {
		const e = parseManifestEntry("s2-agent-ext-web-access/index.ts");
		expect(e.name).toBe("s2-agent-ext-web-access");
		expect(e.entry).toBe("s2-agent-ext-web-access/index.ts");
	});
...
describe("parseManifestEntries", () => {
	test("mixed bare strings + declared objects", () => {
		const entries = parseManifestEntries([
			...
			"@repo/s2-agent-ext-obsidian/extensions/obsidian.ts",
		]);
