#!/usr/bin/env bash
########################################
# run-sh-agent-e2e.sh — L2/L3 e2e for a s2-agent-sh deploy.
#
...
#
# USAGE
#   bash bun-apps/s2-agent/scripts/run-sh-agent-e2e.sh           # L2, fresh deploy to a temp dir
#   bash bun-apps/s2-agent/scripts/run-sh-agent-e2e.sh --tui     # L3 pty smoke
#   DEPLOY_DIR=~/proj/dist/s2-agent-sh/current \
#     bash bun-apps/s2-agent/scripts/run-sh-agent-e2e.sh         # reuse an existing deploy
########################################
set -uo pipefail
...
DEVOPS="$REPO_ROOT/bun-apps/s2-agent-ext-devops"
...
# ── L3: the interactive TUI under a real pty ────────────────────────────────
  ...
  # pkill targets this run's unique temp path, never another s2-agent.
