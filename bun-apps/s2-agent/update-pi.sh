...
#
# RENAME NOTE: s2-agent = renamed pi-agent (2026-08-21) — this wrapper and its
# upstream targets are UNCHANGED by the rename; only the workspace package
# dirs it cd's into are now bun-apps/s2-agent*.
#
# WHY THIS EXISTS
...
#
#   The 4 core packages are the runtime every s2-agent-* extension builds on.
...
#       ./bun-apps/s2-agent/update-pi.sh --lockstep   # offline gate, exits non-zero on drift
#
# USAGE
#   ./bun-apps/s2-agent/update-pi.sh            # upgrade all 4 to latest (unifies any drift)
#   ./bun-apps/s2-agent/update-pi.sh --check    # current vs latest + lockstep report (network)
#   ./bun-apps/s2-agent/update-pi.sh --lockstep # OFFLINE lockstep invariant gate (CI-friendly)
#   ./bun-apps/s2-agent/update-pi.sh --rebuild  # also cut a new versioned deploy (moves `current`)
#   ./bun-apps/s2-agent/update-pi.sh -h|--help  # print this header
#   The upgrade path also runs `bun run typecheck` (mirrors CI: test · s2-agent),
...
check_lockstep() {
  ...
  if [[ "$LOCKSTEP_OK" -eq 1 ]]; then
    ...
    echo "$(green '✓') lockstep OK — every s2-agent-* workspace pins all 4 core packages at ${LOCKSTEP_VER}."
  else
    echo "$(red '✗') lockstep BROKEN — s2-agent-* workspaces disagree on pi-* core version:"
    printf '%s\n' "$LOCKSTEP_DRIFT" | sed 's/^/    /'
    echo "    $(dim 'fix: ./bun-apps/s2-agent/update-pi.sh  (unifies all 4 to latest)')"
  fi
  [[ "$LOCKSTEP_OK" -eq 1 ]]
}

# Cut a new s2-agent deploy. Extracted to a function so the early-exit
...
#
# NOTE: this produces a NEW VERSIONED DEPLOY under ~/proj/dist/s2-agent-sh/ and
# REPOINTS `current` at it — a more consequential action than the old
# dist/s2-agent bundle rebuild it replaces. The four legacy deploy modes were
...
do_rebuild() {
  echo
  echo "$(green '▶') cut a new s2-agent deploy (versioned; repoints current)"
  (cd "$REPO_ROOT/bun-apps/s2-agent" && bun run deploy --force)
}

# Cross-package TypeScript preflight. Runs `bun run typecheck` (tsc --noEmit)
# from bun-apps/s2-agent — the SAME check CI's `test · s2-agent` job runs — so a
...
do_typecheck() {
  echo
  echo "$(green '▶') typecheck (cross-package — mirrors CI: test · s2-agent)"
  local log n
  log="$(mktemp)"
  if (cd "$REPO_ROOT/bun-apps/s2-agent" && bun run typecheck) >"$log" 2>&1; then
...
if [[ "$TYPECHECK_FAILED" -eq 1 ]]; then
  ...
  echo "committable), but fix the TS errors above before pushing — CI's test · s2-agent gates it."
