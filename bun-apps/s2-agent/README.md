...
## Setup
```bash
bun install          # from bun-apps/ (the workspace root; never inside s2-agent/)
```
## Usage
```bash
...
# print mode
bun bun-apps/s2-agent/src/cli.ts -p "hello"
# list models — lm-studio entries appear alongside built-ins
bun bun-apps/s2-agent/src/cli.ts --list-models
# load an EXTRA extension on top of the run-dir set (see below) without pi install
bun bun-apps/s2-agent/src/cli.ts -e bun-apps/zai-mcp/extensions/zai-mcp.ts -p "list your tools"
```
### Optional: alias in `~/.zshrc`
```sh
alias pi='bun /path/to/repo/bun-apps/s2-agent/src/cli.ts'
...
Toggle:
```bash
BUN_PI_PRE_LOAD_PROVIDERS=0 bun bun-apps/s2-agent/src/cli.ts --list-models   # custom providers hidden
BUN_PI_DEBUG_PATCHES=1      bun bun-apps/s2-agent/src/cli.ts                  # show patch status
```
...
hack: copy/deploy s2-agent elsewhere, or invoke it via the `~/.zshrc` alias from any
other directory, and every extension silently vanished.
Fix: `-e`/`--extension` and `--skill` accept **absolute paths**, which bypass `cwd`
resolution and trust-gating entirely. `s2-agent.registry.yaml` is the registry
...
The result: s2-agent loads the exact same extensions regardless of invocation `cwd`,
...
`s2-agent.registry.yaml`, then `bun run --cwd bun-apps/s2-agent regen:manifest`
...
bun bun-apps/s2-agent/src/cli.ts -p "…"
# opt in for one invocation — the alias resolves to the real factory file
bun bun-apps/s2-agent/src/cli.ts -e workflow -p "audit src/ for missing auth"
bun bun-apps/s2-agent/src/cli.ts -e dynamic-workflows -p "…"
...
- Deploy layouts still self-inject `-ne` internally (run-dir/resolve.ts) so a
  deployed s2-agent ignores whatever `.pi/` exists in your cwd. That injected
...
```bash
bun bun-apps/s2-agent/src/cli.ts cli doctor [--json]
```
## Build / Deploy

s2-agent ships as ONE artifact: a versioned, frozen tree under
`~/proj/dist/s2-agent-sh/<version>/` holding a minimal compiled core plus one
...
bun run --cwd bun-apps/s2-agent deploy --no-freeze  # skip the read-only freeze (bypasses the core cache)
```

The extension set, the host-module contract and the per-extension build
metadata all live in `s2-agent.registry.yaml`. **[`docs/deploy.md`](docs/deploy.md)
...
## Testing
`run-test.sh` (now living at `../s2-agent-ext-devops/scripts/run-test.sh`)
...
```bash
../s2-agent-ext-devops/scripts/run-test.sh                  # = medium  (~7s)   s2-agent suite incl. launcher e2e   [default]
../s2-agent-ext-devops/scripts/run-test.sh quick            # (~0.2s)   unit only, no build — pre-commit safe
../s2-agent-ext-devops/scripts/run-test.sh smoke            # (~30s)    LIVE local-LLM check vs LM Studio (skips when down)
../s2-agent-ext-devops/scripts/run-test.sh full             # (~40s)    + smoke + sibling pi-* unit baseline (whole stack)
../s2-agent-ext-devops/scripts/run-test.sh --list           # print the tier table
```
| Level | Adds | Catches |
|---|---|---|
...
| **full** | sibling pi-* unit baseline (obs/kc/cli/vlm) | the whole stack s2-agent loads as extensions |
...
(`../s2-agent-ext-devops/scripts/run-test.sh high --bail`). Numeric aliases `0-3` work too.
...
```
s2-agent/
├── package.json            # bin: s2-agent → src/cli.ts; also holds the migrated npm extension deps
├── README.md
├── s2-agent.registry.yaml  # THE extension registry (one entry per extension; edit this)
├── run-dir/
...
- **Standalone binary can't dynamically `-e`-load `.ts` extensions**
  (`./dist/s2-agent/s2-agent`). In `isBunBinary` mode, jiti feeds each
  ...
  This is a bun-compile + jiti limitation, not a s2-agent regression, and it
...
## Non-interactive CLI (`s2-agent cli`)
...
```bash
./s2-agent.sh cli <command> [options]                    # from the repo root
bun bun-apps/s2-agent/src/cli.ts cli <command> [options]  # same, no wrapper
~/proj/dist/s2-agent-sh/current/s2-agent cli <command>    # deployed sh tree
```
...
> static extension factories. Note `bun run --cwd bun-apps/s2-agent cli` is this
...
```bash
./s2-agent.sh cli chat                        # interactive multi-turn REPL
./s2-agent.sh cli agent <task...>             # free-form agentic task, broad toolset
./s2-agent.sh cli file2md <files...>          # PDF/image → Obsidian markdown (local VLM)
./s2-agent.sh cli zk-extract <files/folders>  # markdown → Zettelkasten atomic notes
./s2-agent.sh cli zk-card <add|find|update|remove|check>
./s2-agent.sh cli zk-ask <question>           # graph-enhanced RAG answer
./s2-agent.sh cli zk-ingest --source <src>    # converge .knowledge.jsonl → cards
./s2-agent.sh cli zk-query <query>            # deterministic tag-ranked digest
./s2-agent.sh cli sessions <query>            # search past session transcripts
./s2-agent.sh cli memory <query>              # search pi-hermes-memory
./s2-agent.sh cli doctor [--json] [--fix]     # cross-machine portability self-check
./s2-agent.sh cli list | list-tools | version | completions <bash|zsh|fish>
./s2-agent.sh cli help [command]
```
`./s2-agent.sh cli help` prints the authoritative list; `help <command>` prints
one command's flags.
...
```bash
./s2-agent.sh cli file2md paper.pdf --pages 1-3 --out ./vlm-out
./s2-agent.sh cli file2md scan.jpg --type image --dpi 200
```
...
```bash
./s2-agent.sh cli zk-extract ./inbox/ --folder Zettelkasten --max-notes 20
```
### Extension-backed sub-commands (NL → tool)
These wrap a per-command extension's tool behind a natural-language prompt.
```bash
./s2-agent.sh cli flux2 <request...>       # images (Flux2 Klein, Swift/MLX)
./s2-agent.sh cli krea2 <request...>       # images (Krea 2 Turbo, Swift/MLX)
./s2-agent.sh cli ltx <request...>         # video (LTX-2.3, Swift/MLX)
./s2-agent.sh cli movie <request...>       # video-production orchestrator
./s2-agent.sh cli research <query...> [--save]   # web research → digest
./s2-agent.sh cli power-tool <request...>  # runtime diagnostics
```
...
```bash
./s2-agent.sh cli pipeline pdf-to-vault <pdf> [--pages 1-3] [--delete-png]
./s2-agent.sh cli pipeline image-to-vault <image>
./s2-agent.sh cli pipeline url-to-vault <url>
./s2-agent.sh cli pipeline youtube-to-vault <url> [question]
./s2-agent.sh cli pipeline memory-to-vault
./s2-agent.sh cli pipeline status|run|dry-run|lint   # knowledge pipeline
```
### Workflow sub-command (headless engine, no agent session)
```bash
./s2-agent.sh cli workflow list                       # [pack] vs [file]
./s2-agent.sh cli workflow run <name|path> [--dry-run] [--json]
```
...
```bash
./s2-agent.sh cli -p "What files are in the current directory?"
./s2-agent.sh cli --model deepseek-v4-flash -p "Summarize this"
./s2-agent.sh cli --mode json --no-session --tools read,bash "summarize"
```
This is exactly what the `obsidian_distill` / `obsidian_garden` subagent tools
invoke internally (`process.argv[1]` + pi flags), which is why it exists: s2-agent
...
catalog is layered on top — see [ADR 0005](docs/adr/0005-provider-catalog-from-s2-agent.md).
### CLI docs
