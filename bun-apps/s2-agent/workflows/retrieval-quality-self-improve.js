 ...
 *
 * Lives in bun-apps/s2-agent/workflows/ (ENGINE dir) for the gate/pipeline/
 * journal primitives. See ../docs/workflow-cli.md (two-runtime boundary).
 *
 * INVOCATION
 *   bun --cwd bun-apps/s2-agent src/cli.ts cli workflow run retrieval-quality-self-improve \
...
export const meta = {
  ...
  description:
    "READ-side retrieval-quality loop: adversarial query-gen → zk-ask --retrieve-only in two blend modes → blind LLM judge (deterministic A/B alternation) → persist receipt + .knowledge.jsonl. Proves blend > lexical with data (mean relevance@5, win tally). Engine workflow — runnable via `s2-agent workflow run`. Needs vault-mind for the semantic seed.",
  phases: [
...
1. Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/s2-agent' src/cli.ts cli zk-card find 'error' --folder '${FOLDER}' --limit 5 2>&1 | head -40")
...
2. Bash("bun '${PROJECT_ROOT}/bun-apps/s2-agent/workflows/lib/lexical-overlap-check.mjs' --queries '${qFile}' --vault '${VAULT}' --folder '${FOLDER}'")
...
1. MODE A (${MODE_A}):
   Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/s2-agent' src/cli.ts cli zk-ask '${esc}' --retrieve-only --blend ${MODE_A} --folder '${FOLDER}' --top-k ${TOP_K} --model ${RETRIEVE_MODEL} --thinking ${THINKING} -p > '${lexFile}' 2>&1")
2. MODE B (${MODE_B}):
   Bash("OB_VAULT_PATH='${VAULT}' bun --cwd '${PROJECT_ROOT}/bun-apps/s2-agent' src/cli.ts cli zk-ask '${esc}' --retrieve-only --blend ${MODE_B} --folder '${FOLDER}' --top-k ${TOP_K} --model ${RETRIEVE_MODEL} --thinking ${THINKING} -p > '${blendFile}' 2>&1")
