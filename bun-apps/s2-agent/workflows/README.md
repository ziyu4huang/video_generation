...
```bash
bun --cwd bun-apps/s2-agent src/cli.ts cli workflow run echo --dry-run
bun --cwd bun-apps/s2-agent src/cli.ts cli workflow run args-demo --dry-run
bun --cwd bun-apps/s2-agent src/cli.ts cli workflow run sample --dry-run
```
...
- **Path B — interactive tool**: the `workflow` tool with `name: "<pack>"` in a
  pi TUI session (the workflow extension is built-in via `./s2-agent.sh`). The
...
```
              bun --cwd bun-apps/s2-agent src/cli.ts cli workflow run <name> --model <spec>
                                                 │
                                                 ▼
        ┌───────────────────────────── s2-agent cli (Command dispatch) ─────────────────────────────┐
...
- `vault` — override vault path (default `<root>/vaults_root/s2-agent-vault`).
