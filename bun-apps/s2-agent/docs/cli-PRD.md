...
> The non-interactive command namespace of `bun-apps/s2-agent` (`src/cli/**`),
> reached as `s2-agent cli <command>`. Written while it was a separate
> `s2-agent-cli` package; merged into s2-agent 2026-08-12.
...
in the workspace deps (`s2-agent-ext-*`).
...
fallback. (b) `buildBakedRegistry()` — `@repo/s2-agent`'s `PROVIDERS`
...
- baked providers sourced from `s2-agent` — one-file model catalog, no drift.
...
- `s2-agent-ext-hermes-memory` (persistent memory: memory_search, session_search, skill_manage)
- `s2-agent-ext-obsidian` (always loaded for vault access)
- `s2-agent-ext-knowledge-card` (zk-extract, zk-ask, zk-ingest, knowledge_query)
- `s2-agent-ext-knowledge-card` `zk_ingest` actions `gate`/`converge`/`status` (knowledge distillation pipeline: hermes-memory → vault → graph)
- `s2-agent-ext-file2md` (VLM describe)
- `s2-agent-ext-power-tool` (doctor diagnostics)
