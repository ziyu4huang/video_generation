...
How s2-agent adds a custom LLM provider / model, and why neither touches
...
pi-coding-agent dist). The `s2-agent cli` namespace honors those env vars
directly; the TUI path wraps the real pi, so without this bridge the same
`PI_MODEL=...` that worked for `s2-agent cli` was silently ignored. The patch splices the env value into argv
...
since 2026-07-03 — see `docs/deploy.md` § "The tree is read-only" / the `s2-agent-readonly-deploy`
