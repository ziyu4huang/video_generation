...
A portable single-exec binary (in 2026-07 that was `s2-agent-cli` built via its
own `bun scripts/build.ts --compile`; today it is `s2-agent` built via
`bun scripts/deploy.ts --exe`, invoked as `s2-agent cli workflow run …`) must run a user-supplied workflow-pack by NAME on a machine without
...
- The change lives in the engine (`s2-agent-ext-workflow/src/workflow-pack.ts`),
