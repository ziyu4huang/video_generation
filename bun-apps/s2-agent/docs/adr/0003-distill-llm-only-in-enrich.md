**ID:** `ADR-s2-agent-0003` — ADR numbers restart per context, so this number alone is ambiguous; cite this ID. Index: `bun-apps/docs/adr/INDEX.md`
...
driving agent's s2-agent session (the same SDK/model path every command resolves
