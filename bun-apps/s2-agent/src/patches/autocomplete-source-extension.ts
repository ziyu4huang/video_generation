 ...
 * from `.../bun-apps/s2-agent-ext-<name>/...`), the source is "local"/"auto" so
 ...
 * SDK renders the base `[tag] desc`, we derive the owning `s2-agent-ext-<name>`
 ...
 *   `[u:npm:@x/s2-agent-ext-hyperframes] desc` → `[e:hyperframes] desc`.
 ...
 * `s2-agent-ext-<name>` keep their original marker (user/project/temporary).
 *
 ...
 * is NOT from a s2-agent-ext package (user/project skills, etc.), in which case
...
export function owningExtension(
  ...
  const m = hay.match(/s2-agent-ext-([a-z0-9-]+)/i);
  return m ? m[1] : undefined;
}

/** True when the resource originates from a s2-agent-ext-* package, even if a
...
export function isExtensionSource(
  ...
  const hay = `${sourceInfo.path ?? ""} ${sourceInfo.source ?? ""} ${sourceInfo.baseDir ?? ""}`;
  return /s2-agent-ext-/i.test(hay);
}
