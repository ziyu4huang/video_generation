...
describe("classifySize", () => {
	test("3 files one package = T1", () => {
		expect(classifySize(["bun-apps/s2-agent/src/a.ts", "bun-apps/s2-agent/src/b.ts", "bun-apps/s2-agent/src/c.ts"], REPO_ROOT)).toBe("T1");
	});
	...
	test("package dir not on disk = T3", () => {
		expect(classifySize(["bun-apps/s2-agent-ext-newthing/src/index.ts"], REPO_ROOT)).toBe("T3");
	});
});
...
describe("runGate", () => {
	test("green T2 effort exits 0", () => {
		const r = runGate({
			...
			changedFiles: ["bun-apps/s2-agent/src/a.ts", "bun-apps/s2-agent/src/b.ts"],
			repoRoot: REPO_ROOT,
		});
