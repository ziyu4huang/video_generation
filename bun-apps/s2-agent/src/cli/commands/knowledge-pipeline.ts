 ...
 *   --vault-dir <name>    vault folder under cwd (default vaults_root/s2-agent-vault)
...
export const knowledgePipelineCommand = {
	...
	details: `Usage:
  s2-agent cli pipeline status [options]        snapshot: captures, pending, health
  s2-agent cli pipeline run [options]           converge + merge + heal (writes a receipt)
  s2-agent cli pipeline dry-run [options]       preview converge + merge (no writes)
  s2-agent cli pipeline lint [--fix] [options]  merge-duplicates + heal only
...
Examples:
  s2-agent cli pipeline status
  s2-agent cli pipeline run
  s2-agent cli pipeline dry-run
  s2-agent cli pipeline lint --fix
  s2-agent cli pipeline run --json`,
