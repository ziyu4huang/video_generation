...
describe("buildBakedRegistry — s2-agent PROVIDERS baked in", () => {
  ...
  test("baked lm-studio is registered with the s2-agent catalog + zero cost", async () => {
    const { modelRegistry } = await buildBakedRegistry();
    const m = modelRegistry.find("lm-studio", "prism-ml/bonsai-27b");
...
describe("applyObsidianSubagentFloor — inject OB_SUBAGENT_MODEL from settings", () => {
  ...
  test("a personal floor distinct from the built-in still wins over it", () => {
    applyObsidianSubagentFloor({ obsidian: { subagentModel: "zai/glm-4.7" } });
    expect(process.env.OB_SUBAGENT_MODEL).toBe("zai/glm-4.7");
  });
});
