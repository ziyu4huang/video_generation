...
// PI_MODEL env, resolveLLM falls back to the built-in lm-studio/prism-ml/bonsai-27b — giving a
...
describe("workflowRunCommand.run — pi-default model + receipt (Task 3)", () => {
	...
	it("forwards callerModel=undefined + piDefaultModel=lm-studio/prism-ml/bonsai-27b when no --model / no PI_MODEL", async () => {
		await withCleanEnv(async () => {
			resetCaptures();
			fakeModel = "lm-studio/prism-ml/bonsai-27b";
	...
	it("renders `model: <spec> [source]` in the text receipt", async () => {
		await withCleanEnv(async () => {
			...
			assert.ok(
				line.includes("model: lm-studio/prism-ml/bonsai-27b [pi-default]"),
	...
	it("forwards callerModel=lm-studio/x when --model is given (rendered source --model)", async () => {
		await withCleanEnv(async () => {
			...
			assert.equal(capturedPiDefaultModel, "lm-studio/prism-ml/bonsai-27b", "piDefault is still computed");
			const line = out.find((l) => l.includes("✓"));
