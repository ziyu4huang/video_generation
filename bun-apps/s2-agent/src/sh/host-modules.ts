 ...
 * binary (measured). s2-agent-ext-task builds TUI overlays and keybindings
...
import * as coreRuntime from "@repo/s2-agent-core-runtime";
// pi-ai is already compiled in via pi-coding-agent (it is a s2-agent dep), so
...
import * as coreInterface from "@repo/s2-agent-core-interface";
 ...
 *
 * 2 (2026-08-20): `@repo/s2-agent-ext-subagent` was REMOVED from the registry.
 ...
 * Those symbols now live in `@repo/s2-agent-core-runtime` (already a host
...
const REGISTRY: Readonly<Record<string, unknown>> = Object.freeze({
	...
	"@repo/s2-agent-core-runtime": coreRuntime,
	"@repo/s2-agent-core-interface": coreInterface,
...
export class HostModuleNotFoundError extends Error {
	constructor(spec: string) {
		super(
			`[s2-agent-sh] extension required "${spec}", which the host does not provide. ` +
