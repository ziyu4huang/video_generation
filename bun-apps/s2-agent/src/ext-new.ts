 ...
 * `s2-agent-ext-<name>/`, so plain collisions with sibling workspace dirs are
 * the hazard (a `s2-agent` scaffold would nest the s2-agent package itself).
 */
const RESERVED_DIRECTORIES = new Set(["s2-agent", "node_modules", "dist", "docs", "gui-movie-director"]);

/**
 * Valid iff kebab-case suffix (`^[a-z][a-z0-9-]*$`) without the
 * `s2-agent-ext-` prefix (the tool adds it — accepting both spellings would
 * create `s2-agent-ext-s2-agent-ext-foo`) and not a reserved dir name.
 */
export function validateName(name: string): boolean {
	if (!NAME_RE.test(name)) return false;
	if (name.startsWith("s2-agent-ext-")) return false;
...
export function parseExtNewArgs(argv: string[]): ExtNewArgs {
	...
	if (!validateName(args.name)) {
		throw new Error(
			`invalid name ${JSON.stringify(args.name)} — expected a kebab-case suffix (^[a-z][a-z0-9-]*$, no s2-agent-ext- prefix, not a reserved dir name)`,
		);
	}
	return args;
}
...
export function buildScaffoldFiles(name: string, opts: { libFace: boolean }): Record<string, string> {
	...
	files["package.json"] = opts.libFace
		? `{
	"name": "@repo/s2-agent-ext-${name}",
	"private": true,
	"version": "0.1.0",
	"description": "Pi extension: ${name} (scaffolded by \`s2-agent ext new\` — replace with a real description).",
	"license": "MIT",
...
`;
	...
	files["README.md"] = `# @repo/s2-agent-ext-${name}

Pi extension: ${name} (scaffolded by \`s2-agent ext new\` — replace with a real description).

## Develop

\`\`\`bash
bun test --cwd bun-apps/s2-agent-ext-${name}
bun run --cwd bun-apps/s2-agent-ext-${name} typecheck
\`\`\`
 ...
 * that carry each extension's rationale. src/run-dir/manifest.json is
 * regenerated by script; it is never written directly.
 */
...
async function runRegen(script: string): Promise<void> {
	...
	if ((await regen.exited) !== 0) {
		throw new Error(`\`bun run --cwd bun-apps/s2-agent ${script}\` failed (exit ${regen.exitCode})`);
	}
}
