...
describe("userSuppressFlags + userExtensionPaths + overriddenStaticExtensions", () => {
	...
	it("overriddenStaticExtensions matches whole path segments", () => {
		const names = ["s2-agent-ext-hermes-memory", "s2-agent-ext-webui"];
		expect(
			overriddenStaticExtensions(["-e", "~/dev/s2-agent-ext-hermes-memory/extensions/hermes-memory.ts"], names),
		).toEqual(new Set(["s2-agent-ext-hermes-memory"]));
		// substring must NOT match (-v2 suffix)
		expect(
			overriddenStaticExtensions(["-e", "~/dev/s2-agent-ext-hermes-memory-v2/x.ts"], names),
		).toEqual(new Set());
	});
});
