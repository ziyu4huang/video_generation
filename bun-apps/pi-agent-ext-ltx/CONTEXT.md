...
The ubiquitous language of pi-agent-ext-ltx — an agent-optimized wrapper around the `ltx-video` pure-Swift/MLX generator (LTX-2.3 image-to-video on Apple Silicon). One dispatcher tool over 15 commands; the load-bearing distinction is the **native-*** family (pure-Swift/MLX, zero `run.py`) vs the production `i2v` (which bridges `run.py`).
