...
**Procedure:**
1. Ledger medians: `bun scripts/runs-stats.ts` (s2-agent-ext-subagent; counts + per-status medians from `~/.pi/subagents/runs`; `--trend` trajectory, `--snapshot` append). Prioritize turns > tokens > time.
