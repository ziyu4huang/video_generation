...
export async function ingestRecords(
	...
	// 6. ticket 08 fold-back (D40): post-write index rebuild — after the MOC
	// regeneration, fingerprint-gated + coalesced, fire-and-forget. Opt-in
	// (`indexRebuild: true`, production callers only — a rebuild touches the
	// live Surreal service); any failure is non-fatal — the D36 freshness gate
	// serves flat until the next successful rebuild.
