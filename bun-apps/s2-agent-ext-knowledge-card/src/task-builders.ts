...
export function buildRagTask(
	...
	return [
		...
		"",
		"Use the tier ladder (L0/L1/L2) to assemble context:",
		"",
		`Tier 1 (L2 full read): for notes ranked in the top ${tier1Count}, or with score ≥ 0.7:`,
		`  Run obsidian action:"read". DEMOTE, NEVER TRUNCATE: if a note's content`,
		`  exceeds ${maxNoteTokens} tokens, demote it to L1 — its \`summary\` frontmatter`,
		"  if present, else the body lead (~600 chars). Do NOT slice the full body.",
		assembleNote,
		"",
		"Tier 2 (L0 abstract): for all other notes:",
