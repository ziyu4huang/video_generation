...
flowchart TB
  ...
  subgraph KC["📦 s2-agent-ext-knowledge-card (this package)"]
    direction TB
  ...
  subgraph fwd["⬆️ FORWARD dependency (peer, hard)"]
    OBS[" 🔌 s2-agent-ext-obsidian<br/><small>runSubagentWithRetry · resolveVault<br/>parseFrontmatter · validateZettelNote<br/>getIndex · graphDeadLinks · graphOrphans<br/>invalidateCache · ZETTEL_MAX_BYTES</small>"]:::dep
  ...
  subgraph rev["⬇️ REVERSE dependencies (needed by)"]
    direction LR
    CLI["s2-agent cli<br/><small>zk-extract · zk-card · zk-ask<br/>zk-ingest · zk-query · knowledge-pipeline<br/><b>hard workspace:*</b></small>"]:::consumer
...
<details>
<summary><b>Reverse dep — s2-agent static imports (verified <code>rg</code>)</b></summary>
...
```
src/store/vault-converge.ts:130   const kc = await import("@repo/s2-agent-ext-knowledge-card/src/ingest.ts");
...
- **Renaming an exported builder** (`buildRagTask` etc.) → s2-agent breaks at
  build time (caught by its 248-test suite).
- **Renaming a `src/` export** → s2-agent + pi-hermes-memory break; hermes
