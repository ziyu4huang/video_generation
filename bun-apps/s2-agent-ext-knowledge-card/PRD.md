# PRD — s2-agent-ext-knowledge-card
...
(subagent over `s2-agent-ext-obsidian`):
...
- **`graph_health` / `healGraph`** → registered with `s2-agent-ext-obsidian` so
...
`let zkSpawn = spawnSubagent` from `@repo/s2-agent-ext-subagent`) that the
...
(`s2-agent/src/static-extensions.ts:85`) — the four `zk_*` tools are present in
...
```
 ...
 └──────┬───────────────────────────────┘
        │
 ┌──────▼─────────────────────────────────────────────────────────────┐
 │ @repo/s2-agent-ext-subagent · spawnSubagent → createAgentSession    │
...
> Note: `s2-agent-ext-power-tool` is **no longer a dependency** —
...
```bash
# CLI
bun bun-apps/s2-agent/src/cli.ts cli zk-ask "question"
bun bun-apps/s2-agent/src/cli.ts cli zk-ingest <file.knowledge.jsonl>
bun bun-apps/s2-agent/src/cli.ts cli zk-query --tags flux2,vae        # retrieve digest
bun bun-apps/s2-agent/src/cli.ts cli zk-query --json                  # deterministic JSON
# Or via the extension (registers the 4 agent tools)
pi -e bun-apps/s2-agent-ext-knowledge-card
```
