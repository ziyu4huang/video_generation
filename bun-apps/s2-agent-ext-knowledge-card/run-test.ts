#!/usr/bin/env bun
/**
 * run-test.ts — tiered test launcher for s2-agent-ext-knowledge-card.
 *
 ...
 *                    runContract() comment).
 *
 * NOTE — knowledge-card's canonical `bun run test` is scoped to `bun test __tests__/` (the same scope the .sh
 * used for quick), so quick is byte-identical to the .sh's form. Its standalone contract step therefore runs the
 * BARE file (`bun test <file>`): forwarding the path through `bun run test` would union with the baked-in scope
 * and re-run the whole suite as the 'contract' step (the finding this fix round closed).
 * USAGE (from anywhere):
 *   bun bun-apps/s2-agent-ext-knowledge-card/run-test.ts              # = quick
 *   bun bun-apps/s2-agent-ext-knowledge-card/run-test.ts full
 *   bun bun-apps/s2-agent-ext-knowledge-card/run-test.ts --list       # print the tier table, exit 0
 *   bun bun-apps/s2-agent-ext-knowledge-card/run-test.ts quick --bail # extra flags are forwarded
...
const PKG = "s2-agent-ext-knowledge-card";
// extension-contract.test.ts re-asserted standalone (the `full` tier).
const CONTRACT_TEST = "__tests__/extension-contract.test.ts";
...
function printList(): void {
  process.stdout.write(
    `${Y(`${PKG} run-test.sh — tiers`)}:\n` +
      `  ${G("quick")}  bun test __tests__/ (this package's existing test command)  ${Y("[default]")}\n` +
