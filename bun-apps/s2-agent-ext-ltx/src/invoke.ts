/**
 * invoke.ts — spawn `ltx-video`, stream progress, honor abort.
 *
...
/** Spawn ltx-video and stream until exit/abort. Never rejects — check exitCode/aborted. */
export function invokeLtx(opts: InvokeOpts): Promise<InvokeResult> {
      ...
      stream.on("data", (chunk: Buffer) => {
        ...
        while ((nl = lineBuf[key].indexOf("\n")) >= 0) {
          ...
          if (line) {
            ...
            // contract (found by s2-agent-ext-ltx-self-improve's review
            // lane, 2026-07-05).
            try {
