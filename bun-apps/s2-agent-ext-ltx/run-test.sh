#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for s2-agent-ext-ltx.
...
print_list() {
	cat <<EOF
$(Y "s2-agent-ext-ltx run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/s2-agent-ext-ltx-runtest.log 2>&1
	rc=$?
	...
	if [ "$rc" -ne 0 ]; then sed 's/^/      /' /tmp/s2-agent-ext-ltx-runtest.log | tail -n 25 >&2; fi
}
...
echo "$(Y "▶ s2-agent-ext-ltx run-test.sh — tier=$TIER")"
case "$TIER" in
