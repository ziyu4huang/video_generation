 ...
 *
 * Lets `s2-agent` expose the ltx extension as a top-level sub-command:
 *
 *   s2-agent cli ltx <natural-language request...>
 *   s2-agent cli --model sonnet ltx generate a 5s video of ocean waves, native-i2v
 *
 ...
 * direction is s2-agent → s2-agent-ext-ltx, so the spec is typed with a
...
export const ltxSubcommand: ExtensionSubcommandSpec = {
  ...
  details: `Usage:
  s2-agent cli ltx <natural-language request...> [options]
...
--fps, --seed, …) are NOT parsed by the CLI. Two ways to pass them:
  ...
  2. Use the \`--\` end-of-options separator to pass raw flags through verbatim:
       s2-agent cli ltx -- native-i2v --prompt "waves" --duration 5
...
Examples:
  s2-agent cli ltx generate a 5s video of ocean waves at sunset, native-i2v
  s2-agent cli --model sonnet ltx upscale the last video to 2x, native-upscale
  s2-agent cli ltx -- native-i2v --image photo.png --prompt "subtle zoom in" --duration 4`,
  factory: extension,
  tools: ["ltx", "ltx_help"],
  task: (parsed) => {
