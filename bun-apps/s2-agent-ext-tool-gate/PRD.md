...
Gates are **owner-declared**: each extension declares its family once in the shared `GATE_DEFS` registry (`@repo/s2-agent-core-interface`) and its tools reference it via `gating: { gate: "<id>" }` on their `ToolDefinition`.
...
|---|---|---|---|
| flux2 | `flux2`, `flux2_help` | s2-agent-ext-flux2 | keywords + noun∧verb |
...
| zai | `zai_web_search_web_search_prime`, `zai_web_reader_webReader` | s2-agent-ext-zai-mcp *(env-gated on ZAI_API_KEY)* | keywords + noun∧verb |
| deploy_pi_agent_sh | `deploy_pi_agent_sh`, `verify_pi_agent_deploy` | s2-agent-ext-devops | keywords + noun∧verb |
| merge_pr_after_local_ci, sweep_merged_branches, run_local_ci, check_main_health, sync_default_branch, run_devops_retrospect, prepare_feature_branch, verify_merge_landed | devops single-tool gates | s2-agent-ext-devops | keywords |
| zk_card, zk_ask, zk_ingest, knowledge_query | knowledge-card on-demand | s2-agent-ext-knowledge-card | keywords + noun∧verb (ticket 02) |
| skill_manage, session_search, knowledge_search, knowledge_ingest, planning_stale, grill_decision, memory_supersede | hermes-memory on-demand | s2-agent-ext-hermes-memory | keywords ± noun∧verb (ticket 02) |
| wayfind_effort | wayfind | s2-agent-ext-wayfind | keywords + noun∧verb (ticket 02) |
| get_search_content | web-access | s2-agent-ext-web-access | keywords + noun∧verb (ticket 02) |
| obsidian | `obsidian`, `obsidian_help` | s2-agent-ext-obsidian | keywords + noun∧verb (ticket 02) |
...
todo, goal_complete                        → s2-agent-ext-task
ask_user_question                          → s2-agent-ext-task (HITL)
memory, memory_search                      → s2-agent-ext-hermes-memory
web_search, fetch_content                  → s2-agent-ext-web-access
inspect_context, inspect_agent, inspect_extensions,   → s2-agent-ext-power-tool
...
- `s2-agent` (loaded via run-dir manifest, `extensions/tool-gate.ts` is the single registration entry)
...
```bash
# Auto-loaded via s2-agent's run-dir manifest (no manual setup)
