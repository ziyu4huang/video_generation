...
describe("parseRemoteUrl", () => {
  ...
  test("https with credentials + trailing path junk", () => {
    expect(parseRemoteUrl("https://user:tok@git.example.com/o/r.git")).toEqual({
  ...
  test("ssh:// with port", () => {
    expect(parseRemoteUrl("ssh://git@git.example.com:2222/o/r.git")).toEqual({
      host: "git.example.com",
      owner: "o",
      repo: "r",
    });
...
function fakeSpawn(routes: Array<{ match: (cmd: string, args: string[]) => boolean; result: SpawnResult }>): SpawnFn {
  return async (cmd, args) => {
    ...
    return hit?.result ?? { stdout: "", stderr: "no route", exitCode: 1 };
  };
}
...
const remoteOk = (url: string) =>
  fakeSpawn([{ match: (c, a) => c === "git" && a[0] === "remote" && a[1] === "get-url", result: ok(`${url}\n`) }]);
