...
test("ci-local.ts --only filter previews just that subset (--list and --tsv)", () => {
  assertParity(CI_LOCAL, [
    ...
    {
      name: "only-tsv",
      args: ["--only", "s2-agent", "--tsv"],
      expectCode: 0,
      out: "s2-agent\tbun test && bun run typecheck",
    },
    {
      name: "only-eq-form",
      args: ["--only=s2-agent", "--tsv"],
...
test("ci-local.ts --only= (empty value) is the NO-FILTER quirk, not a usage error", () => {
  ...
  assertParity(CI_LOCAL, [{ name: "only-empty", args: ["--only=", "--tsv"], expectCode: 0, out: MATRIX_TSV_GOLDEN }]);
});
...
test("ci-local.ts usage errors exit 2 (D4 exit-code law)", () => {
  assertParity(CI_LOCAL, [
    ...
    {
      name: "jobs-matrix-forbidden",
      args: ["--jobs", "2"],
      expectCode: 2,
      out: "--jobs > 1 applies to --gates only; the matrix run is sequential by contract",
    },
    { name: "jobs-not-int", args: ["--gates", "--jobs", "x"], expectCode: 2, out: "--jobs needs an integer ≥ 1" },
    {
      name: "gate-timeout-not-int",
      args: ["--gates", "--gate-timeout-ms", "abc"],
      expectCode: 2,
      out: "--gate-timeout-ms needs an integer ≥ 1000",
    },
    {
      name: "budget-not-int",
      args: ["--gates", "--budget-ms", "1"],
      expectCode: 2,
      out: "--budget-ms needs an integer ≥ 1000",
    },
    {
      name: "only-not-a-package",
      args: ["--only", "bogus", "--list"],
      expectCode: 2,
      out: "",
      errIncludes: [
        "ci-local FAILED: --only: 'bogus' is not a package in the .github/workflows/ci.yml.disabled tests matrix (run --list)",
      ],
    },
    {
      name: "gates-plus-only",
      args: ["--gates", "--only", "x"],
      expectCode: 2,
      out: "--only filters matrix packages; it does not apply to --gates",
    },
  ]);
});
