...
import { CI_WORKFLOW_PATH, parseCiMatrix, readCiMatrix } from "../src/ci-matrix.js";
...
describe("parseCiMatrix", () => {
  ...
  test("ignores a job whose matrix has no include list (determinism-spotcheck shape)", () => {
    const yaml = ["jobs:", "  tests:", "    strategy:", "      matrix:", "        package:", "          - pkg-a"].join(
      "\n",
    );
...
describe("readCiMatrix — against this repo's real workflow", () => {
  test("reads every row, including the ones a generic derivation would get wrong", async () => {
    ...
    expect(matrix["s2-agent-ext-file2md"]).toBe("bun test --isolate");
    expect(matrix["s2-agent-ext-archify"]).toBe("bun test --isolate");
