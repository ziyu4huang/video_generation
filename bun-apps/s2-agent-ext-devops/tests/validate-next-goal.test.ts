...
describe("validateNextGoalFile — strict format", () => {
  ...
  test("filename pattern (sort key for newest-file resolution + pruning)", () => {
    ...
    writeFile(
      bad,
      mutate(good, (s) => s.replaceAll(good, bad)),
    );
  ...
  test("missing frontmatter block", () => {
    ...
    writeFile(
      bad,
      mutate(good, (s) => s.slice(s.indexOf("# Next goal")) + ""),
    );
  ...
  test("missing frontmatter key", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) => s.replace(/supersedes: .*\n/, "")),
    );
  ...
  test("self-abs-path: file: must name THIS file's absolute path (the worktree self-reference)", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) => s.replaceAll(`file: ${file}`, `file: /elsewhere/${basename(file)}`)),
    );
  ...
  test("created must be filename-derived (date AND time)", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) => s.replace("created: 2026-08-23 03:03:03", "created: 2026-01-01 03:03:03")),
    );
  ...
  test("created: date-only (legacy shape) is NOT enough — time is required", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) => s.replace("created: 2026-08-23 03:04:04", "created: 2026-08-23")),
    );
    expect(failedChecks(validateNextGoalFile(file))).toEqual(["created-matches-filename"]);
  });
  ...
  test("unknown frontmatter keys are a violation (strict key set)", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) => s.replace("supersedes: none", "supersedes: none\nmood: ambitious")),
    );
  ...
  test("missing section heading", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) => s.replace("## Honest gaps", "## Reflection")),
    );
  ...
  test("Ranked next goals: fewer than 3 fails", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) => s.replace("3. **Third** — first step.\n", "")),
    );
  ...
  test("Immediate steps: a bare-pointer step fails the detail bar (what's next must be explained)", () => {
    ...
    writeFile(
      file,
      mutate(file, (s) =>
        s.replace(/^1\. Sync main.*\n2\. Branch via.*$/m, "1. Fix it.\n2. Ask the user what to do next, then do that."),
      ),
    );
