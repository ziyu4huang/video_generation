...
import type { CiOptions, CiOutcome } from "../src/ci-recipe.js";
import type { WorktreeRecord } from "../src/gh.js";
import { type MainHealthClient, runMainHealth } from "../src/main-health-recipe.js";
...
describe("runMainHealth — where it runs", () => {
  test("runs the FULL matrix in the worktree holding the default branch, not the caller's cwd", async () => {
    ...
    const out = await runMainHealth({
      client: fakeClient({
        worktrees: [
          { worktree: "/repo/feature-wt", branch: "feat/x" },
          { worktree: MAIN_WT, branch: "main" },
        ],
      }),
      spawn: noSpawn,
      runCi: ci.fn,
