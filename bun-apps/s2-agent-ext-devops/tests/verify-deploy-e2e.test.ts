...
describe("runDeployE2e", () => {
  ...
  test("a FAST provider failure on the model call is a SKIP, never a fail", async () => {
    ...
    const r = await runDeployE2e({
      versionDir,
      spawn: fakeSpawn({
        modelCall: { exitCode: 1, stdout: "", stderr: "provider lm-studio: connection refused — no api key" },
      }),
    });
...
describe("parseHermesStartupRoundTrips (pure)", () => {
  ...
  test("returns the MAX across multiple startup ops", () => {
    expect(
      parseHermesStartupRoundTrips(
        [BANNER("startup.syncMarkdownMemories", 60), BANNER("startup.other", 90)].join("\n"),
      ),
    ).toBe(90);
  });
  ...
  test("ignores non-startup ops and ms-only breaches", () => {
    ...
    expect(
      parseHermesStartupRoundTrips("[hermes-memory] slow startup.syncMarkdownMemories: 2596ms (backend=surrealdb)."),
    ).toBeNull();
  });
    ...
    const r = await runDeployE2e({
      ...
      spawn: fakeSpawn({
        modelCall: {
          stdout: "ok",
          stderr:
            "[hermes-memory] slow startup.syncMarkdownMemories: 240 HTTP round-trips (backend=surrealdb). See perf.jsonl.\n",
        },
      }),
    });
    expect(r.verdict).toBe("fail");
    ...
    const r = await runDeployE2e({
      ...
      spawn: fakeSpawn({
        modelCall: {
          stdout: "ok",
          stderr:
            "[hermes-memory] slow startup.syncMarkdownMemories: 114 HTTP round-trips (backend=surrealdb). See perf.jsonl.\n",
        },
      }),
    });
    const mc = r.probes.find((p) => p.id === "model-call")!;
    ...
    const r = await runDeployE2e({
      ...
      spawn: fakeSpawn({
        modelCall: {
          ...
          stdout: "",
          stderr:
            "[hermes-memory] slow startup.syncMarkdownMemories: 240 HTTP round-trips (backend=surrealdb).\nprovider lm-studio: connection refused — no api key",
        },
      }),
    });
    const mc = r.probes.find((p) => p.id === "model-call")!;
...
describe("providers-catalog probe (the pre-load-providers patch, verified in the FINAL tree)", () => {
  ...
  test("patch dead: a baked lane missing from the ON run FAILS naming the pair", async () => {
    ...
    const on = listModelsStdout([
      ...
      ...bakedModelPairs()
        .filter((p) => p.provider !== "lm-studio")
        .map((p) => [p.provider, p.model] as [string, string]),
    ]);
    const r = await runDeployE2e({
      versionDir,
  ...
  test("bakedModelPairs mirrors PROVIDERS §1 (catalog-true, not hardcoded)", () => {
    ...
    expect(pairs.length).toBe(Object.values(PROVIDERS).reduce((n, e) => n + e.models.length, 0));
...
const visionAsk =
  (r: Partial<VisionAskOutcome>): ((p: string) => Promise<VisionAskOutcome>) =>
...
describe("vision-call probe", () => {
  ...
  test("a reply containing the fixture text passes and names the lane evidence", async () => {
    ...
    expect(
      r.probes
        .filter(
          (p) =>
            p.id !== "file2md-ocr" && p.id !== "tool-gate-fire" && p.id !== "standalone-import" && p.id !== "parity",
        )
        .every((p) => p.verdict === "pass"),
    ).toBe(true);
  });
  ...
  test("a provider-down error is a SKIP that degrades the verdict, never a fail", async () => {
    ...
    expect(
      r.probes.filter((p) => p.id !== "file2md-ocr" && p.id !== "standalone-import").every((p) => p.verdict !== "fail"),
    ).toBe(true);
  });
  ...
  test("--skip-model-call skips the vision probe without degrading the verdict", async () => {
    ...
    expect(
      r.probes
        .filter((p) => p.id !== "file2md-ocr" && p.id !== "tool-gate-fire" && p.id !== "standalone-import")
        .every((p) => p.verdict !== "fail"),
    ).toBe(true);
  });
...
describe("isBunShellChildSignature (win32 upstream-block classification)", () => {
  ...
  test("a genuine launcher defect does NOT classify (bun-direct broken too)", () => {
    expect(isBunShellChildSignature({ "bun-direct": 0, "cmd-echo": 22, "cmd-bun": 0, "cmd-shim": 0 })).toBe(false);
  });

  test("a cmd-only defect does NOT classify (cmd's own echo lost as well)", () => {
    expect(isBunShellChildSignature({ "bun-direct": 1299, "cmd-echo": 0, "cmd-bun": 0, "cmd-shim": 0 })).toBe(false);
  });

  test("the launcher suddenly delivering output does NOT classify (bug fixed → probes pass)", () => {
    expect(isBunShellChildSignature({ "bun-direct": 1299, "cmd-echo": 22, "cmd-bun": 1299, "cmd-shim": 1299 })).toBe(
      false,
    );
  });
});
...
describe("isNoConsoleRelayWorkaround (win32 ticket-02 relay-route signature)", () => {
  ...
  test("relay delivers through cmd while the direct shim loses → WORKS", () => {
    expect(isNoConsoleRelayWorkaround({ "cmd-shim": 0, "cmd-ps1-nw-relay": 1299, "ps1-nw-relay": 1299 })).toBe(true);
  });

  test("relay losing through cmd does NOT classify (shipped-shim rewrite would be blind)", () => {
    expect(isNoConsoleRelayWorkaround({ "cmd-shim": 0, "cmd-ps1-nw-relay": 0, "ps1-nw-relay": 1299 })).toBe(false);
  });
  ...
  test("a fixed launcher does NOT need the route (shim already speaks)", () => {
    expect(isNoConsoleRelayWorkaround({ "cmd-shim": 1299, "cmd-ps1-nw-relay": 1299 })).toBe(false);
  });
});
...
describe("isBunRelayWorkaround (win32 ticket-02 bun-relay signature, diag iteration 2)", () => {
  ...
  test("relay file nonzero while the direct shim loses → WORKS", () => {
    expect(isBunRelayWorkaround({ "cmd-shim": 0, "cmd-bun-relay-file": 0, "cmd-bun-relay-file-file": 1299 })).toBe(
      true,
    );
  });

  test("empty relay file does NOT classify", () => {
    expect(isBunRelayWorkaround({ "cmd-shim": 0, "cmd-bun-relay-file": 0, "cmd-bun-relay-file-file": 0 })).toBe(false);
  });
  ...
  test("a fixed launcher does NOT need the route (shim already speaks)", () => {
    expect(isBunRelayWorkaround({ "cmd-shim": 1299, "cmd-bun-relay-file-file": 1299 })).toBe(false);
  });
});
