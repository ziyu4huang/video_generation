...
const base = (over: Partial<ParityFingerprint> = {}): ParityFingerprint => ({
  ...
  skills: [
    {
      n: "collect-youtube-llm",
      p: "/w/bun-apps/s2-agent-ext-research-tool/skills/collect-youtube-llm/SKILL.md",
      ch: "5",
    },
  ],
  ...over,
...
describe("diffFingerprints", () => {
  ...
  test("FAIL: skill content drift / deploy-only skill / unattributed dev skill", () => {
    const d = deploy();
    d.skills = [
      { n: "collect-youtube-llm", p: "/dist/ext/research-tool/skills/collect-youtube-llm/SKILL.md", ch: "555" },
    ];
    ...
    const v = base({
      skills: [
        ...base().skills,
        { n: "stray-skill", p: "/w/bun-apps/s2-agent-ext-devops/skills/stray-skill/SKILL.md", ch: "2" },
      ],
    });
    ...
    const v = base({
      tools: [
        ...base().tools,
        {
          n: "collect_videos",
          s: "extension",
          p: "/w/bun-apps/s2-agent-ext-research-tool/extensions/research-tool.ts",
          dh: "7",
          sh: "8",
        },
      ],
  ...
  test("inline-factory tools attribute via <inline:<name>> / <inline:<package>> paths", () => {
    ...
    const withExcluded = [
      ...EXCLUDED,
      { name: "web-access", package: "s2-agent-ext-web-access", reason: "inline factory ext" },
    ];
    ...
    expect(
      bad.findings
        .filter((f) => f.kind === "unattributed-dev-tool")
        .map((f) => f.item)
        .sort(),
    ).toEqual(["read_page", "web_search"]);
  });
