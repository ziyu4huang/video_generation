...
import { PARITY_PROBE_SOURCE, type ParityFingerprint, parseParityFpLine } from "../src/parity-probe.js";
...
describe("parseParityFpLine", () => {
  ...
  test("wrong marker version → ok:false", () => {
    const r = parseParityFpLine(
      `[PARITY-FP-START]${JSON.stringify({ ...GOOD_FP, marker: "PARITY_FP_v0" })}[PARITY-FP-END]`,
    );
    expect(r.ok).toBe(false);
  });
});
