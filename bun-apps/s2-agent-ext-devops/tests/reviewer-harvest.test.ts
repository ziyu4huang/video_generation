...
import { REVIEWER_HARVEST_USAGE, runReviewerHarvestCli } from "../scripts/reviewer-harvest.js";
import {
  findTranscripts,
  type HarvestIo,
  harvest,
  ...
  test("resume after end_turn: a user line / working assistant line voids the stale verdict", () => {
    ...
    resumed.push(
      JSON.stringify({
        ...
        message: { role: "user", content: "follow-up nudge" },
      }),
    );
    ...
    resumed.push(
      JSON.stringify({
        ...
        message: {
          role: "assistant",
          stop_reason: "end_turn",
          content: [{ type: "text", text: "REVISED verdict after resume" }],
        },
      }),
    );
...
describe("harvest — pi-runs fallback (claude-glm absent)", () => {
  ...
  test("terminal-failure pi status maps to errored (never a fake still-running spin)", async () => {
    const piRunsRoot = tempPiRuns([
      { file: "mtfake03.json", run: piRun("rev-pi-3", { status: "failed", output: "" }) },
    ]);
    const result = await harvest({
      name: "rev-pi-3",
      harnessRoot: noClaudeRoot(),
