  ...
  test("healthy stub tree: exit 0, verdict pass, pure JSON on stdout", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        config: {
          extensions: [
            { name: "stub-a", enabled: true },
            { name: "stub-b", enabled: true },
          ],
        },
      }),
    );
    ...
    const r = run(
      "bun",
      [
        join(PKG, "src/verify-deploy-e2e-cli.ts"),
        "--deploy-root",
        root,
        "--dev-launcher",
        join(versionDir, "s2-agent.sh"),
...
describe.skipIf(!!process.env.CI)("s2-agent runtime: the wrapper still boots with this package wired in", () => {
  ...
  }, 30_000); // not a perf gate; the spawn's own 60s cap still bounds a hang. // parallel, so 5s flakes under load. The intent is "wrapper boots", // imported) — ~3.2s in isolation, and local-ci runs packages in // but this probe boots the whole s2-agent CLI (all static extensions // Explicit 30s harness budget: bun's default per-test timeout is 5s,
});
