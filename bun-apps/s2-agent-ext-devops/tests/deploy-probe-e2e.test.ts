...
const agentDirEnv: Record<string, string> = {
  ...
  PI_MODEL:
    ...
    (process.env.DEEPSEEK_API_KEY || !process.env.ZAI_API_KEY ? "deepseek-v4-flash-vision-exp" : "glm-5.3-flash"),
  PI_THINKING: "off",
};
...
const configuredNames = shConfig({ bunAppsDir: BUN_APPS_DIR })
...
const realHistoryRoot = join(process.env.PI_CODING_AGENT_DIR ?? join(homedir(), ".pi", "agent"), "prompt-history");
const realHistoryBefore: string[] = existsSync(realHistoryRoot) ? readdirSync(realHistoryRoot).sort() : [];
...
afterAll(() => {
  ...
  expect(added, `probe leaked per-user writes into the REAL ${realHistoryRoot} — agent-dir isolation broken`).toEqual(
    [],
  );
  rmTree(outRoot);
});
...
async function run(argv: string[], opts: { cwd?: string; env?: Record<string, string> } = {}): Promise<Run> {
  ...
  try {
    ...
    if (out.eof && err.eof) {
      ...
      code = await Promise.race([proc.exited, new Promise<null>((r) => setTimeout(() => r(null), 5_000))]);
      if (code === null) {
        deadline();
        code = 124;
      }
...
`,
    ...
    const p = payload(r, "[REG]") as {
      tools: { name: string; path: string }[];
      cmds: { name: string; path: string }[];
    };
...
`,
  ...
  test("boots and starts a session with ALL networking denied (sandbox-exec)", async () => {
    ...
    const boot = Bun.spawn(
      ["sandbox-exec", "-p", DENY_NETWORK_SBPL, "bash", join(target, "s2-agent.sh"), "--ext-list"],
      {
        cwd: target,
        env: { ...process.env, ...agentDirEnv },
        stdout: "pipe",
    ...
    let bootErr = "";
    try {
      [bootOut, bootErr] = await Promise.all([new Response(boot.stdout).text(), new Response(boot.stderr).text()]);
    } finally {
    ...
    const session = Bun.spawn(["sandbox-exec", "-p", DENY_NETWORK_SBPL, binary, "-e", probePath, "-p", "hi"], {
      cwd: target,
      env: { ...process.env, ...agentDirEnv },
      stdout: "pipe",
      stderr: "pipe",
