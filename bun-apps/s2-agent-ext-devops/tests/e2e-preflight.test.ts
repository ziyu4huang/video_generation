...
import { E2E_RC_FILES, preflightE2eLane, resolveRcApiKey } from "../src/e2e-preflight.ts";
...
describe("resolveRcApiKey — bash resolve_gate_api_key parity", () => {
  test("ambient env wins over rc files", () => {
    ...
    expect(resolveRcApiKey({ DEEPSEEK_API_KEY: "from-env" }, read, "DEEPSEEK_API_KEY")).toBe("from-env");
  });
...
describe("preflightE2eLane — the four scenarios", () => {
  ...
  test("key present (env) → ok, with the unset-pin advisory", () => {
    const result = preflightE2eLane({ DEEPSEEK_API_KEY: "sk-123" }, rcReaderFrom({}));
  ...
  test("key present only in an rc file → ok (the historical operator shape)", () => {
    const result = preflightE2eLane({}, rcReaderFrom(HOME_FILES({ zshrc: "export ZAI_API_KEY=zai-key" })));
    expect(result.ok).toBe(true);
  ...
  test("malformed VERIFY_E2E_MODEL pin → fail, naming the provider/model-id form", () => {
    const result = preflightE2eLane({ DEEPSEEK_API_KEY: "sk", VERIFY_E2E_MODEL: "just-a-model-id" }, rcReaderFrom({}));
    expect(result.ok).toBe(false);
    if (!result.ok) {
