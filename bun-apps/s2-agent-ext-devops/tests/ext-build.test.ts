...
describe("buildExtPackage", () => {
  ...
  test("fails with an actionable message when a declared copy dir is missing", async () => {
    ...
    mkdirSync(join(pkgDir, "extensions"), { recursive: true });
    writeFileSync(join(pkgDir, "package.json"), JSON.stringify({ name: "fake-ext", version: "0.0.0", type: "module" }));
    writeFileSync(join(pkgDir, "extensions", "fake.ts"), `export default () => {};`);
    await expect(
      buildExtPackage({
        ext: {
          name: "fake-ext",
  ...
  test("rewrites an allowed (host) dynamic import through require — the /websearch defect", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: "dynimp-ext", version: "0.0.0", type: "module" }),
    );
    writeFileSync(
  ...
  test("vendors the dependency CLOSURE of a vendor: root, recording pruned optionals", async () => {
    ...
    writeFileSync(
      join(pkgDir, "package.json"),
      JSON.stringify({ name: "closure-ext", version: "0.0.0", type: "module" }),
    );
...
describe("rewriteAllowedDynamicImports", () => {
  test("routes a HOST-module dynamic import through require — the /websearch defect shape", () => {
    ...
    expect(rewriteAllowedDynamicImports(code, [...HOST_MODULES, "@earendil-works/pi-ai/compat"])).toBe(
      `const { getModel } = await Promise.resolve(require("@earendil-works/pi-ai/compat"));`,
    );
  });
...
describe("assetFoldRegex / rewriteAssetImportMetaFolds (cross-host fold spellings)", () => {
  ...
  test("percent-encoded folds match when the store dir has spaces/unicode", () => {
    ...
    expect(
      ("file:///C:/Users/runner%20admin/cache/tesseract-wasm@5/t.js".match(assetFoldRegex(wdir)) ?? []).length > 0,
    ).toBe(true);
  });

  test("no fold from any asset package still throws the shape assert", () => {
    const { root } = fixturePkg();
    expect(() => rewriteAssetImportMetaFolds("var x = 1;", ["fixture-asset-pkg"], root)).toThrow(/none found/);
  });
});
