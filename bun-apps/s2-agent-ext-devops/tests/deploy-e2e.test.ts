...
describeE2E("s2-agent-sh deploy e2e", () => {
  test("full deploy produces a working core, extensions, and current symlink", async () => {
    ...
    // Consumer-declared piped-ness — bun's isTTY lies in the
    ...
    const v = Bun.spawnSync([join(r.target, "bin", "bun"), join(r.target, CORE_FILENAME), "--version"], {
      stdout: "pipe",
      stderr: "pipe",
    });
  ...
  test("the shipped tree is offline-contained (Gate 5 static half)", async () => {
    ...
    expect(
      scanBinaryForeignPaths(join(target, CORE_FILENAME), target).foreign,
      "core bakes build-machine path(s)",
    ).toEqual([]);
  }, 60_000);
...
describeE2E("legacy flat-layout housekeeping", () => {
  ...
  test("a deploy removes a stale pre-t05 top-level current", async () => {
    ...
    expect(
      existsSync(join(r.target, "..", "current")) || existsSync(join(legacyRoot, hostTargetName(), "current")),
    ).toBe(true);
  }, 300_000);
...
describeE2E("cross-target deploy (win32-x64 from a darwin host)", () => {
  ...
  beforeAll(() => {
    ...
    const tar = Bun.spawnSync(
      ["tar", "-cf", zip, "--format", "zip", "-C", join(crossRoot, "payload"), "bun-windows-x64"],
      {
        stdout: "pipe",
        stderr: "pipe",
      },
