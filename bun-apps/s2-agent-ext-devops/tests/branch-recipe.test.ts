...
import { buildSweepPlan, executeSweep, resolveProtected, runSweep, type SweepClient } from "../src/branch-recipe.js";
...
describe("executeSweep", () => {
  ...
  test("a failed LOCAL delete → skipped with reason, NOT in deletedLocal", async () => {
    ...
    expect(out.skipped).toContainEqual({
      name: "feat/merged",
      reason: expect.stringMatching(/delete failed.*branch not found/),
    });
  ...
  test("a failed REMOTE delete → skipped with reason, NOT in deletedRemote", async () => {
    ...
    expect(out.skipped).toContainEqual({
      name: "rmt/merged",
      reason: expect.stringMatching(/delete failed.*remote rejected/),
    });
  });
