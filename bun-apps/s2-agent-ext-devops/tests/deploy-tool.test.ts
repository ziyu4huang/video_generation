...
import { type DeployResult, deployViaCli, parseCliJson, runDeploy } from "../src/deploy-tool.ts";
...
describe("deployViaCli", () => {
  ...
  test("a CLI failure maps `error` onto errorTail and keeps the rest", async () => {
    const out = JSON.stringify(
      { ok: false, error: "bundle references specifier(s) the host does not provide: foo" },
      null,
      2,
    );
    const r = await deployViaCli(
    ...
    const out = JSON.stringify(
      {
        ...
        version: "0.1.0+gabc1234",
        target: "/dist/x",
        message: "exists — pass --force",
        e2e: { verdict: "skip" },
      },
      null,
      2,
    );
    const r = await deployViaCli({}, { resolveDir: () => PI_AGENT_DIR, spawn: fakeSpawn({ output: out }) });
