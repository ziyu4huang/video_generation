...
import { deriveExcludedExtensions, runDeployE2e } from "../src/deploy-e2e-recipe.js";
...
const DEV_FP = {
  ...
  tools: [
    ...
    {
      n: "merge_pr_after_local_ci",
      s: "extension",
      p: "/w/bun-apps/s2-agent-ext-devops/extensions/devops.ts",
      dh: "3",
      sh: "4",
    },
  ],
...
function fakeSpawn(
  ...
  return async (_cmd: string, args: string[]): Promise<SpawnResult> => {
    ...
    if (argv.includes("-e")) {
      ...
      const fp = isDev
        ? variants.devFp === undefined
          ? DEV_FP
          : variants.devFp
        : variants.deployFp === undefined
          ? DEPLOY_FP
          : variants.deployFp;
    ...
    if (argv.includes("--ext-list")) {
      return {
        stdout: JSON.stringify({ loaded: ["task"], loadedCount: 1, skillPaths: [], skipped: [] }),
        stderr: "",
        exitCode: 0,
      };
    }
...
describe("runDeployE2e parity probe", () => {
  ...
  test("fail when a shared tool hash drifts", async () => {
    ...
    const drifted = {
      ...DEPLOY_FP,
      tools: DEPLOY_FP.tools.map((t) => ({ ...t, sh: t.n === "merge_pr_after_local_ci" ? "999" : t.sh })),
    };
  ...
  test("fail when dev-only tool is unattributed (real registry exclusion list applies)", async () => {
    ...
    const stray = {
      ...DEV_FP,
      tools: [...DEV_FP.tools, { n: "stray", s: "builtin", p: "<builtin:stray>", dh: "9", sh: "9" }],
    };
  ...
  test("I1: both fingerprints EMPTY-but-valid (marker present) → fail citing symmetric degradation, no providers comparison", async () => {
    ...
    const r = await runDeployE2e({
      versionDir,
      spawn: fakeSpawn({ devFp: emptyDev, deployFp: emptyDep }),
      devLauncher,
    });
    const p = r.probes.find((x) => x.id === "parity")!;
    expect(p.verdict).toBe("fail");
