...
describe("parsePrView", () => {
  ...
  test("missing base/head refs → empty strings (defensive)", () => {
    expect(parsePrView({ state: "OPEN", mergeStateStatus: "CLEAN" })).toMatchObject({
      baseRefName: "",
      headRefName: "",
    });
  });
  ...
  test("malformed/empty input → OPEN/UNKNOWN defaults + empty refs", () => {
    expect(parsePrView(null)).toEqual({
      state: "OPEN",
      mergeState: "UNKNOWN",
      mergeSha: undefined,
      baseRefName: "",
...
describe("parseChecks", () => {
  ...
  test("an unknown state defaults to pending (never claim success)", () => {
    expect(parseChecks([{ name: "x", state: "WAT", completedAt: "z" }])).toEqual({ pass: 0, fail: 0, pending: 1 });
  });
});
...
describe("createGhClient (glue)", () => {
  ...
  test("prStatus parses view + checks JSON into the domain shape (incl. base/head refs)", async () => {
    const { fn, calls } = rec([
      {
        match: (c, a) => c === "gh" && a.includes("view"),
        result: {
      ...
      {
        match: (c, a) => c === "gh" && a.includes("checks"),
        result: {
          stdout: JSON.stringify([{ name: "a", state: "SUCCESS", completedAt: "x" }]),
          stderr: "",
          exitCode: 0,
        },
      },
  ...
  test("prList issues gh pr list --state <s> --json number,headRefName,mergedAt --limit N", async () => {
    const { fn, calls } = rec([
      {
        ...
        result: {
          stdout: JSON.stringify([{ number: 7, headRefName: "feat/x", mergedAt: "2026-01-01T00:00:00Z" }]),
          stderr: "",
          exitCode: 0,
        },
      },
    ...
    expect(calls[0]).toEqual({
      cmd: "gh",
      args: ["pr", "list", "--state", "merged", "--json", "number,headRefName,mergedAt", "--limit", "50"],
    });
  });
  ...
  test("mergeNow THROWS on a non-zero exit (surfaces as a recipe block)", async () => {
    const { fn } = rec([
      {
        match: (c, a) => c === "gh" && a.includes("merge"),
        result: { stdout: "", stderr: "merge queue: not your turn", exitCode: 2 },
      },
    ]);
...
describe("parseRemoteBranches", () => {
  test("strips origin/, drops HEAD -> line", () => {
    expect(
      parseRemoteBranches(["  origin/HEAD -> origin/main", "  origin/main", "  origin/feat/x"].join("\n")),
    ).toEqual(["main", "feat/x"]);
  });
...
describe("parseWorktrees", () => {
  test("extracts branch refs/heads/<name>, ignores detached", () => {
    const out = parseWorktrees(
      ["worktree /a", "HEAD abc", "branch refs/heads/feat/x", "", "worktree /b", "HEAD def", "detached"].join("\n"),
    );
...
describe("parseSubmoduleStatus", () => {
  ...
  test("all four flags parse verbatim: ' ' (in-sync), '+' (drifted), '-' (not initialized), 'U' (conflict)", () => {
    const rows = parseSubmoduleStatus(
      ` ${sha("a")} sub-a\n+${sha("b")} sub-b\n-${sha("c")} sub-c\nU${sha("d")} sub-d\n`,
    );
    expect(rows).toEqual([
...
describe("createBranchClient (glue)", () => {
  ...
  test("branchVv issues git branch -vv", async () => {
    const { fn, calls } = rec([
      {
        match: (c, a) => c === "git" && a.includes("-vv"),
        result: { stdout: "  x abc [origin/x]\n  y def [origin/y: gone]\n", stderr: "", exitCode: 0 },
      },
    ]);
  ...
  test("remoteBranches issues git branch -r", async () => {
    const { fn } = rec([
      {
        match: (c, a) => c === "git" && a.includes("-r"),
        result: { stdout: "  origin/HEAD -> origin/main\n  origin/main\n", stderr: "", exitCode: 0 },
      },
    ]);
  ...
  test("worktrees issues git worktree list --porcelain", async () => {
    const { fn, calls } = rec([
      {
        match: (c, a) => c === "git" && a.includes("worktree"),
        result: { stdout: "worktree /a\nbranch refs/heads/feat/x\n", stderr: "", exitCode: 0 },
      },
    ]);
  ...
  test("containedBranches issues git branch --merged <default>", async () => {
    const { fn, calls } = rec([
      {
        match: (c, a) => c === "git" && a.includes("--merged"),
        result: { stdout: "  feat/old\n* main\n", stderr: "", exitCode: 0 },
      },
    ]);
  ...
  test("defaultBranch parses symbolic-ref origin/HEAD", async () => {
    const { fn } = rec([
      {
        match: (c, a) => c === "git" && a.includes("symbolic-ref"),
  ...
  test("currentBranch issues git rev-parse --abbrev-ref HEAD", async () => {
    const { fn } = rec([
      {
        match: (c, a) => c === "git" && a.includes("rev-parse"),
        result: { stdout: "feat/cur\n", stderr: "", exitCode: 0 },
      },
    ]);
  ...
  test("deleteLocalBranch THROWS on a non-zero exit (surfaces stderr)", async () => {
    const { fn } = rec([
      {
        match: (c, a) => c === "git" && a.includes("-D"),
        result: { stdout: "", stderr: "branch not found", exitCode: 1 },
      },
    ]);
    await expect(createBranchClient(fn).deleteLocalBranch("feat/x")).rejects.toThrow(
      /git branch -D feat\/x failed .*1.*branch not found/,
    );
  });
  ...
  test("deleteRemoteBranch THROWS on a non-zero exit (surfaces stderr)", async () => {
    const { fn } = rec([
      {
        match: (c, a) => c === "git" && a.includes("--delete"),
        result: { stdout: "", stderr: "remote rejected", exitCode: 1 },
      },
    ]);
    await expect(createBranchClient(fn).deleteRemoteBranch("feat/x")).rejects.toThrow(
      /git push --no-verify origin --delete feat\/x failed .*1.*remote rejected/,
    );
  });
  ...
  test("remoteName scoping: defaultBranch/deleteRemoteBranch follow a non-origin remote", async () => {
    const { fn, calls } = rec([
      {
        match: (c, a) => c === "git" && a.includes("symbolic-ref"),
        result: { stdout: "refs/remotes/upstream/main\n", stderr: "", exitCode: 0 },
      },
  ...
  test("dirtyPaths issues git -C <dir> status --porcelain=v1", async () => {
    const { fn, calls } = rec([
      {
        match: (c, a) => c === "git" && a.includes("status"),
        result: { stdout: " M .agents/memory/MEMORY.md\n?? skip.txt\n", stderr: "", exitCode: 0 },
      },
    ]);
    ...
    const { fn, calls } = rec([
      {
        ...
        result: {
          stdout:
            "100644 616e2b4b375d47d0a5f6c11f4e2b6421 1\t.agents/memory/MEMORY.md\n100644 8a5f0e13d9c7f2b4a1c6e3d2f9b8a7c6 3\t.agents/memory/MEMORY.md\n",
          stderr: "",
          exitCode: 0,
        },
      },
  ...
  test("isClean stays available on the full BranchClient (true on exit 0)", async () => {
    const { fn } = rec([
      { match: (c, a) => c === "git" && a.includes("diff"), result: { stdout: "", stderr: "", exitCode: 0 } },
    ]);
