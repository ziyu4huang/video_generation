...
- `src/forge/` — the **forge abstraction** (git-host backends):
  ...
  - `select.ts` — backend selection: GitHub hosts — `GITHUB_TOKEN`/`GH_TOKEN` env → `gh auth token` → gh on PATH → abort with remediation (never anonymous REST); Gitea/Forgejo hosts (naming heuristic, or forced via `DEVOPS_FORGE=gitea`) → the Gitea adapter, which requires `GITEA_TOKEN` (no CLI to harvest one; `GITEA_API_BASE` overrides the API base for http instances). Unknown hosts refused.
  - `gitea.ts` — Gitea/Forgejo REST adapter (`…/api/v1`, `Authorization: token <PAT>`): pure mappers `mapGiteaPullRequest` / `mapGiteaStatuses` / `toGiteaMergeStyle` + thin client; boolean `mergeable` → CLEAN/BLOCKED/UNKNOWN (no BEHIND on Gitea — the local-CI gate is the correctness gate), commit-statuses-only checks, `{Do: "rebase-merge"}` merge mapping, `DELETE /branches/{name}`.
