 ...
 */
export async function computeChangedPackages(opts: ComputeChangedPackagesOptions): Promise<ChangedPackagesMap> {
  ...
  const allTrue = (): ChangedPackagesMap => Object.fromEntries(packages.map((p) => [p, true] as const));
