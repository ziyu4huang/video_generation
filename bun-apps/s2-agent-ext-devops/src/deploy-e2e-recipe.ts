...
export function resolveOneShotBudgetMs(
  ...
  if (!Number.isInteger(n) || n <= 0) {
    return {
      ok: false,
      message: `--model-call-budget-ms must be a positive integer (milliseconds), got ${JSON.stringify(raw)}`,
    };
  }
...
export async function runDeployedVisionAsk(
  ...
  await applyPatches();
  const r = (await executeExtTool(
    join(ctx.versionDir, "ext", "file2md", "ext.cjs"),
    "vision_ask",
    {
      image: imagePath,
...
export function parseHermesStartupRoundTrips(stderr: string): number | null {
  const clean = stderr.replace(new RegExp("\\x1b\\[[0-9;]*m", "g"), "");
...
export function bakedModelPairs(
  ...
  return Object.entries(providers).flatMap(([provider, entry]) => entry.models.map((m) => ({ provider, model: m.id })));
}
 ...
 */
export function parseDeployJson(
  raw: string,
): { ok: true; value: DeployJson; enabled: string[] } | { ok: false; message: string } {
  let parsed: unknown;
  try {
...
export function parseExtListPayload(
  stdout: string,
): { ok: true; value: ExtListPayload } | { ok: false; message: string } {
  try {
...
async function win32LayerDiag(
  ...
  const layers: Array<[string, string, string[]]> = [
    ...
    [
      "ps1-direct",
      "powershell.exe",
      ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", join(versionDir, "s2-agent.ps1"), ...args],
    ],
  ...
  for (const [label, cmd, argv] of layers) {
    try {
      ...
      parts.push(
        `${label}: exit ${r.exitCode}${r.timedOut ? " TIMEOUT" : ""}, ${r.stdout.length}B stdout — ${head || "<no output>"}`,
      );
    } catch (e) {
  ...
  try {
    ...
    const r = await spawn("cmd", ["/c", bunExe, core, ...args, ">", outFile], { cwd: versionDir, timeoutMs: 30_000 });
  ...
  try {
    ...
    const relayLayers: Array<[string, string, string[]]> = [
      ...
      ["cmd-ps1-echo", "cmd", ["/c", "powershell.exe", "-NoProfile", "-Command", "Write-Output diag-ps1-echo-marker"]],
    ];
    ...
    parts.push(
      `relay-route (cmd→ps1 no-console spawn of bun): ${isNoConsoleRelayWorkaround(stdoutBytes) ? "WORKS" : "BROKEN"}`,
    );
    ...
    for (const [label, cmd, argv, env, probeFile] of [
      ["bun-relay-pipe", bunExe, [relayJs], { RELAY_MODE: "pipe" }, null as string | null],
      ["cmd-bun-relay-pipe", "cmd", ["/c", bunExe, relayJs], { RELAY_MODE: "pipe" }, null],
      [
        "cmd-bun-relay-file",
        "cmd",
        ["/c", bunExe, relayJs],
      ...
      try {
        ...
        parts.push(
          `${label}: exit ${r.exitCode}${r.timedOut ? " TIMEOUT" : ""}, ${r.stdout.length}B stdout${fileNote} — ${head || "<no output>"}`,
        );
      } catch (e) {
        parts.push(`${label}: spawn error ${(e as Error).message}`);
      }
    ...
    parts.push(
      `bun-relay-route (cmd→bun relay→direct core spawn): ${isBunRelayWorkaround(stdoutBytes) ? "WORKS" : "BROKEN"}`,
    );
    ...
    try {
      const r = await spawn("cmd", ["/c", bunExe, shippedRelay, ...args], {
        ...
        env: {
          ...
          DIAG_ARGS: args.join(" "),
          DIAG_CWD: versionDir,
          S2_RELAY_OUT: shippedOut,
          S2_RELAY_ERR: shippedErr,
        },
      });
...
export async function runDeployE2e(opts: DeployE2eOptions): Promise<DeployE2eOutcome> {
  ...
  {
    const t0 = now();
    const r = await opts.spawn(launcher.command, [...launcher.prefix, "--help"], {
      cwd: opts.versionDir,
      timeoutMs: BOOT_CAP_MS,
      env: launcherEnv,
    });
    const ms = now() - t0;
    probes.push(
  ...
  {
    const t0 = now();
    const r = await opts.spawn(launcher.command, [...launcher.prefix, "--ext-list"], {
      cwd: opts.versionDir,
    ...
    if (r.timedOut || r.exitCode !== 0) {
      ...
      if (!p.ok) {
        ...
        const skippedNote = p.value.skipped.length ? `; skipped=${JSON.stringify(p.value.skipped)}` : "";
        probes.push(
          missing.length
            ? {
                id: "ext-load",
  ...
  // paths, relative ext resolution) fails HERE, not in a user's script.
  {
    ...
    const foreignCommand = launcher.command.startsWith(".")
      ? join(opts.versionDir, launcher.command)
      : launcher.command;
    ...
    try {
      ...
      if (extLoadLoaded === null) {
        ...
        const same =
          extLoadLoaded.length === p.value.loaded.length &&
          [...p.value.loaded].sort().join(",") === [...extLoadLoaded].sort().join(",");
        probes.push(
          same
            ? {
                id: "cwd-independence",
                verdict: "pass",
                ms,
                note: `same ${p.value.loadedCount} extension(s) loaded from a foreign cwd`,
              }
            : {
                id: "cwd-independence",
                verdict: "fail",
    ...
    if (!opts.devLauncher) {
      ...
      if (!excl.ok) {
        ...
        if (!devCap.ok || !depCap.ok) {
          verdict = "fail";
          lines.push(
            `fingerprint capture failed — dev: ${devCap.ok ? "ok" : devCap.error} · deploy: ${depCap.ok ? "ok" : depCap.error}`,
          );
        } else if (
          ...
          const devIds = devModels.stdout
            .split("\n")
            .map((l) => l.trim())
            .filter(Boolean)
            .sort();
          const depIds = depModels.stdout
            .split("\n")
            .map((l) => l.trim())
            .filter(Boolean)
            .sort();
          ...
          if (provFail.length > 0) {
            ...
            lines.push(
              `providers: model id lists differ — dev-only=[${onlyDev.join(",")}] deploy-only=[${onlyDep.join(",")}]`,
            );
          }
          if (verdict === "pass")
            note = `surfaces identical: ${depCap.fp.toolCount} tools, ${depCap.fp.skillCount} skills, ${depIds.length} models`;
        }
        if (verdict === "fail") {
          note =
            `parity FAIL (${lines.length} finding(s)):\n` +
        ...
        if (onRows === null) {
          ...
          if (missing.length) {
            pcNote = `${missing.length}/${baked.length} baked catalog pair(s) missing from --list-models (e.g. ${missing
              ...
              .join(
                ", ",
              )}) — the ModelRuntime.create wrap is not registering the baked PROVIDERS (the pre-0.80 loadModels precedent: hook target gone upstream?)`;
            pcDetail = tail(on.stdout, on.stderr);
  ...
  if (opts.skipModelCall) {
    ...
    probes.push({
      ...
      verdict: "skip",
      ms: 0,
      note: "skipped by caller (--skip-model-call flag or S2_AGENT_E2E_SKIP_MODEL_CALL=1)",
    });
  } else {
  ...
  if (!enabled.includes("file2md")) {
    ...
    try {
      ...
      const ask =
        opts.visionAsk ??
        ((imagePath: string) => runDeployedVisionAsk({ versionDir: opts.versionDir, hostModules }, imagePath));
