  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (flag === "--config") {
      return {
        ok: false,
        error:
          "--config is retired: the registry is bun-apps/s2-agent/src/registry-config.ts (registry-code-as-config t03)",
      };
    }
    ...
    if (flag === "--list") list = true;
      ...
      continue; // JSON is always on; accepted for symmetry
    else {
      sawDeployFlag = true;
