...
export async function runSync(opts: SyncOptions): Promise<SyncOutcome> {
  ...
  if (mode === "hands-on") {
    ...
    const callerAtTip =
      !b?.aborted &&
      (b
        ? true // phase B rebased the caller onto <remote>/<D> successfully
    ...
    if (!callerAtTip) {
      warnings.push(
        `hands-on: calling worktree is NOT at ${remote}/${D} — fix the abort above and re-run before executing.`,
      );
    }
    if (dry) {
    ...
    return outcome(
      b?.aborted ? { ...b.aborted, message: `[phase B: caller reconcile] ${b.aborted.message}` } : undefined,
    );
  }
  ...
  if (detached) {
    ...
    const co = existing ? await git(repoRoot, ["checkout", name]) : await git(repoRoot, ["checkout", "-b", name]);
    if (!dry && co.exitCode !== 0) {
      warnings.push(`git checkout ${existing ? "" : "-b "}${name} failed: ${trim(co.stderr || co.stdout)}`);
      return outcome({
        aborted: true,
        reason: "checkout_failed",
        message: `checkout ${existing ? "" : "-b "}${name} failed`,
      });
    }
    current = name;
    warnings.push(
      `detached HEAD — ${existing ? `attached existing branch` : `created branch`} '${name}' at HEAD (${head.slice(0, 12)}); proceeding with ${mode} mode.`,
    );
  }
  ...
  if (mode === "rebase") {
    ...
    if (r.exitCode !== 0) {
      ...
      advanceAborted = {
        ...
        message: `rebase onto ${remote}/${D} failed (resolve conflicts, then re-run).`,
      };
    }
  } else {
    ...
    if (r.exitCode !== 0) {
      ...
      advanceAborted = {
        aborted: true,
        reason: "merge_failed",
        message: `merge ${remote}/${D} failed (resolve conflicts, then re-run).`,
      };
    }
  }
