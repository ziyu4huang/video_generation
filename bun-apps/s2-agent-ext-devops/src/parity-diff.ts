...
export function diffFingerprints(
  ...
  for (const [n, ds] of depSkills) {
    ...
    if (!vs) {
      findings.push({
        kind: "deploy-only-skill",
        item: n,
        detail: `deploy ships skill "${n}" (${ds.p}); dev does not`,
      });
    } else if (vs.ch !== ds.ch) {
      findings.push({
        kind: "hash-drift-skill",
        item: n,
        detail: `skill content hash differs — dev ch=${vs.ch} vs deploy ch=${ds.ch}`,
      });
    }
  }
