...
function popConflictPaths(pop: SpawnResult, fallback: string[]): string[] {
  for (const out of [pop.stderr, pop.stdout]) {
    const paths = [...(`${out ?? ""}`.matchAll(/CONFLICT \(content\): [Mm]erge conflict in (.+)/g) ?? [])].map((m) =>
      m[1].trim(),
    );
...
export function createPreserveStash(opts: { git: PreserveGit; spawn: SpawnFn; dry: boolean }) {
  ...
  const restorePreserve = async (park: PreservePark): Promise<{ outcome?: PreserveOutcome; warnings: string[] }> => {
    ...
    if (apply.exitCode !== 0) {
      ...
      return {
        outcome: { paths: park.paths, restored: false, conflict: trim(apply.stderr || apply.stdout) },
        warnings,
      };
    }
    ...
    if (idx >= 0) {
      ...
      warnings.push(
        `preserve restore: applied the parked stash (${park.sha.slice(0, 12)}), but its entry is no longer in the stash list — nothing to drop.`,
      );
    }
