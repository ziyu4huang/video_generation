...
export function bumpVersion(
  ...
  if (!m)
    return {
      ok: false,
      message: `current version is not plain x.y.z: "${current}" (bump manually if you need prerelease forms)`,
    };
...
export function rewritePkgJson(
  raw: string,
  from: string,
  to: string,
...
export function rewriteDispatch(
  raw: string,
  from: string,
  to: string,
): { ok: true; value: string } | { ok: false; message: string } {
...
export async function runVersionBumpCli(argv: string[], deps: { repoRoot?: string } = {}): Promise<CliResult> {
