...
export async function runVerifyMerge(opts: VerifyMergeOptions): Promise<VerifyMergeOutcome> {
  ...
  const defaultBranch = (await safe("defaultBranch", () => client.defaultBranch(), undefined, warnings)) ?? "";
  if (defaultBranch && status.headRefName) {
    ...
    if (!branchSpent && status.headRefOid) {
      ...
      if (!headSha) {
        ...
        warnings.push(
          `head ref '${status.headRefName}' does not resolve locally — treating as spent (nothing to delete).`,
        );
        branchSpent = true;
      } else {
