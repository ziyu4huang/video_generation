...
import { BUNS_DIR, type CachedBun, computeBunHash, ensureCachedBunFrom } from "./bun-cache.ts";
...
async function acquireFromNpm(opts: {
  ...
  if (actual !== expected) {
    throw new Error(
      `npm tarball checksum mismatch for ${pkg}@${bunVersion}: registry ${expected}, downloaded ${actual}`,
    );
  }
