...
describe("patchOfflinePackageLoader", () => {
  ...
  test("replaces the npm-install bootstrap with a fail-fast offline throw", () => {
    ...
    expect(patched).toContain(
      'throw new Error("package not vendored in the offline s2-agent-sh dist: " + missing.join(", "));',
    );
  });
...
describe("copyDeployAssets", () => {
  ...
  test("copies a file payload byte-for-byte under <outDir>/<to>", () => {
    ...
    expect(new Uint8Array(readFileSync(join(outDir, "vendored", "tesseract-wasm", "tesseract-core.wasm")))).toEqual(
      wasm,
    );
  });
    ...
    expect(() =>
      copyDeployAssets(
        [
          {
            pkg: "tesseract-wasm",
            from: "dist/tesseract-core.wasm",
            to: "vendored/tesseract-wasm/tesseract-core.wasm",
          },
        ],
        outDir,
        root,
