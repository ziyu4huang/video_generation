...
export async function standaloneImportProbe(
  ...
  try {
    ...
    const shippedBun =
      opts.shippedBun ??
      (existsSync(binDir) ? join(binDir, readdirSync(binDir).find((n) => n.startsWith("bun")) ?? "") : "");
    ...
    if (existsSync(join(versionDir, "ext", "file2md", "ext.cjs"))) {
      const { loadExt } = (await import(shimPath)) as {
        loadExt: (name: string) => { tool: (n: string) => { execute: (...a: unknown[]) => Promise<unknown> } };
      };
