...
import { BUILTIN_MODEL_DEFAULT, PROVIDERS } from "../../../../s2-agent/src/pre-load-providers.ts";
...
function gateRow(g: GateRecord): string {
  ...
  return (
    `<tr><td>${esc(g.id)}</td><td><code>${esc(g.title)}</code></td><td>${esc(g.scope)}</td>` +
...
function extRow(e: ReportExtension): string {
  ...
  return (
    `<tr><td>${esc(e.name)}</td><td>${e.order}</td><td>${fmtBytes(e.bytes)}</td><td>${closureDetail}</td>` +
  ...
  const rows = entries
    .map((e) => {
      const report = e.hasReport
        ...
        : "—";
      return (
        `<tr><td><code>${esc(e.version)}</code></td><td>${esc(e.builtAt)}</td><td><code>${esc(e.sourceSha.slice(0, 8))}</code></td>` +
