...
- **Submodules** (e.g. `vaults_root/s2-agent-vault`): `git add` only when the recorded pointer (commit SHA) actually changed — "modified content" in the submodule's own working tree is **not** a parent-repo change. Check `git diff --submodule`.
...
`devops-merge-pr-after-ci` bin (`bun-apps/s2-agent-ext-devops/src/merge-pr-after-ci-cli.ts`).
