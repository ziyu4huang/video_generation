...
import { type MainHealthOutcome, runMainHealth } from "../src/main-health-recipe.js";
import { type PrepareOutcome, runPrepare } from "../src/prepare-recipe.js";
import { runMergeRecipe } from "../src/recipe.js";
import { resolveRemoteName } from "../src/remote.js";
import { type RetrospectOutcome, runRetrospect } from "../src/retrospect-recipe.js";
...
function formatSweep(o: SweepOutcome): string {
  ...
  L.push(
    `  review (${o.review.length}, human decides): ${o.review.map((p) => `${p.name} [${p.confidence}: ${p.reason}]`).join(", ") || "—"}`,
  );
  ...
  if (o.executed) {
    const sk = o.executed.skipped.length
      ...
      : "";
    L.push(
      `Deleted local: ${o.executed.deletedLocal.join(", ") || "none"} | Deleted remote: ${o.executed.deletedRemote.join(", ") || "none"}${sk}`,
    );
  }
  return L.join("\n");
}
...
function formatCiOutcome(o: CiOutcome): string {
  ...
  if (o.schemaCost) {
    L.push(
      `Schema-cost: ${o.schemaCost.exitCode === 0 ? "✓" : `✗ exit ${o.schemaCost.exitCode}`} (${o.schemaCost.note}).`,
    );
  }
  return L.join("\n");
}
...
function formatSync(o: SyncOutcome): string {
  ...
  for (const a of o.advanced) {
    L.push(
      `  advanced ${a.branch} @ ${a.worktree}: ${a.from.slice(0, 7)} → ${a.to.slice(0, 7)} (${a.count} commit(s))`,
    );
...
export const SYNC_REPO_PROBES = {
  ...
  controls: [
    ...
    "bring main up to date with origin",
  ],
};
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      strategy: Type.Optional(
        Type.String({
          description:
            "Merge strategy: 'squash' (default, matches the repo's gh-ship convention), 'merge', or 'rebase'.",
        }),
      ),
  ...
  pi.registerTool({
    ...
    promptSnippet:
      "One-shot PR state + check tally (pass/fail/pending). Use instead of a bash gh loop when you only need to inspect.",
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      execute: Type.Optional(
        Type.Boolean({ description: "Delete the high-confidence set. Default false (dry-run: plan only)." }),
      ),
      confirm: Type.Optional(
        Type.Array(Type.String(), {
          description:
            "Branches the human reviewed + approved (must have appeared in `review`). Re-guarded; cannot bypass evidence.",
        }),
      ),
      ...
      protected: Type.Optional(
        Type.Array(Type.String(), {
          description: "Extra protected names. main/master + the repo default branch are always protected.",
        }),
      ),
      prune: Type.Optional(
        Type.Boolean({ description: "Run `git fetch --prune` first so [gone] hints are fresh (default true)." }),
      ),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      baseRef: Type.Optional(
        Type.String({
          description:
            "Base ref to diff against (default <remote>/main; remote = DEVOPS_REMOTE > git config devops.remote > origin). Must exist locally — runLocalCi stays offline (never auto-fetches).",
        }),
      ),
      packages: Type.Optional(
        Type.Array(Type.String(), {
          description: "Explicit package list (bun-apps/<name>); skips change detection entirely.",
        }),
      ),
      all: Type.Optional(
        Type.Boolean({ description: "Run every bun-apps/* package (computeChangedPackages all:true)." }),
      ),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      mode: Type.Optional(
        Type.String({
          description:
            "Sync mode: 'full' (default — advance default branch + submodules), 'rebase' (rebase current onto origin/<default>), 'pull' (merge origin/<default> into current), or 'hands-on' (SOP one-shot prelude — full advance + calling worktree reconciled to the tip; outcome carries handsOn.callerAtTip).",
        }),
      ),
      dryRun: Type.Optional(
        Type.Boolean({
          description: "Compute + return the exact git commands without mutating anything (default false).",
        }),
      ),
      force: Type.Optional(
        Type.Boolean({
    ...
    async execute(_id, params, signal) {
      ...
      const outcome = await runSync({
        ...
        preserve: params.preserve as string[] | undefined,
        remoteName,
        signal,
      });
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      expectedScope: Type.Optional(
        Type.Array(Type.String(), {
          description:
            "Optional scope prefixes; recent touched paths outside ALL prefixes surface as a scope-drift anomaly.",
        }),
      ),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      base: Type.Optional(
        Type.String({
          description:
            "Rebase/create base. Default: <remote>/<defaultBranch> (remote = DEVOPS_REMOTE > git config devops.remote > origin).",
        }),
      ),
      ...
      forcePush: Type.Optional(
        Type.Boolean({
          description: "Force-push the branch (--force-with-lease). Default false (never force-pushes by accident).",
        }),
      ),
      dryRun: Type.Optional(
        Type.Boolean({ description: "Compute + record commands; spawn ZERO mutations (default false)." }),
      ),
    }),
    async execute(_id, params, signal) {
      const spawn = createLiveSpawn(process.cwd());
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      expectedScope: Type.Optional(
        Type.Array(Type.String(), {
          description:
            "Optional scope entries; touched files outside ALL entries → CONTAMINATED. Entry forms: `x/**` any depth, `x/*` one segment, `x/` prefix, bare `x` exact-or-directory.",
        }),
      ),
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      tier: Type.Optional(
        StringEnum(["quick", "medium", "full"] as const, {
          description: "run-test.ts tier. Default: medium.",
          default: "medium",
        }),
      ),
      bail: Type.Optional(
        Type.Boolean({ description: "Stop on first failure (--bail). Default: false.", default: false }),
      ),
    }),
    async execute(_id, params) {
      try {
