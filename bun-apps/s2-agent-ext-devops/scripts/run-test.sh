#!/usr/bin/env bash
########################################
# run-test.sh — multi-effort-level test launcher for s2-agent.
#
...
#   medium  (1)  + the s2-agent package suite, incl. the run.sh/s2-agent.sh   ~7s
...
PI_AGENT_DIR="$(cd "$SCRIPT_DIR/../../s2-agent" && pwd)"
...
step() {
	...
	if [ "$rc" -ne 0 ]; then
		sed 's/^/      /' /tmp/s2-agent-runtest.log | tail -n 60 >&2
	fi
}

echo "$(Y "▶ s2-agent run-test.sh — effort=$EFFORT")"
