...
function parseMatrix(): MatrixRow[] {
  ...
  for (const entry of requireArray(doc?.jobs?.tests?.strategy?.matrix?.include, MATRIX_PARSE_ERR(WORKFLOW))) {
...
if (listOnly) {
  ...
  process.stdout.write(`${D(`${total} entr${total === 1 ? "y" : "ies"}; each runs in its directory with CI=true`)}\n`);
  process.stdout.write("\n");
...
async function runGate(r: (typeof rows)[number], idx: number): Promise<void> {
  ...
  if (budgetMs > 0 && Date.now() - RUN_START_MS > budgetMs) {
    ...
    process.stdout.write(
      "  skipped, NOT passed. Raise --budget-ms or fix the slow gate (per-gate times in the summary).\n",
    );
...
if (gates && jobs > 1) {
  ...
  const ordered = [
    ...rows.filter((r) => HEAVY_GATE_RE.test(r.label)),
    ...rows.filter((r) => !HEAVY_GATE_RE.test(r.label)),
  ];
...
if (gates) {
  ...
  process.stdout.write(
    `${G("ci-local: PASS")} ${D("— matrix only; the non-matrix jobs listed above were NOT run.")}\n`,
  );
}
process.exit(0);
