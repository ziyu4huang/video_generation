...
- **One-shot, ANY worktree state (preferred)**: `bun
  ...
  worktree to that tip (claims `<D>` here when it is free — the caller's
  current branch is left behind, un-rebased — / rebases the current branch
  onto it / recovers a detached HEAD via `--branch`, default `auto`).
...
- **Detached worktree** (post-merge detach / fresh agent worktree — routine
  here): `--branch <slug>` works in `rebase`/`pull`/`hands-on` (phase B) modes. It is IGNORED
