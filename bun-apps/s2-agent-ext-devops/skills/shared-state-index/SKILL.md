---
name: shared-state-index
description: Use before changing or assuming shared config/resolution consumed by more than one package — active vault root, MLX models dir, Python venv, Bun workspace root, sibling-fork deps. Each row carries a verified resolver (file#symbol) and its silent-drift risk.
---
...
> **Why this is a separate doc.** The root `CONTEXT.md` is deliberately
...
> glossary. See the `domain-docs` skill
> (`bun-apps/s2-agent-ext-devops/skills/domain-docs/SKILL.md`) for the
> `CONTEXT.md` convention this complements.
...
- `domain-docs` skill (`bun-apps/s2-agent-ext-devops/skills/domain-docs/SKILL.md`) —
  the `CONTEXT.md` + ADR convention this cross-cutting view complements.
