...
3. Write the spec using the template below, then write it to a local file — `.planning/<effort>/spec.md`. Tell the user the path. (Single home — this is the same `spec.md` superpowers' `brainstorming` writes; do not offer `docs/specs/` or any other location.)
