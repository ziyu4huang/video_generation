...
**Tech Stack:** TypeScript (Bun workspace, `node:child_process` `spawnSync`), `bun:test`, Biome. Source runs under pi's jiti runtime → use `node:child_process` (not the `Bun` global) — matches `pi-agent-ext-movie-director/src/providers.ts`.
...
- **Design ref:** `bun-apps/pi-agent-ext-wayfind/docs/specs/2026-07-21-fact-freshness-guard-design.md`.
...
**Files:**
- Create: `bun-apps/pi-agent-ext-wayfind/src/freshness.ts`
- Test: `bun-apps/pi-agent-ext-wayfind/tests/freshness.test.ts`
...
Run: `( cd bun-apps/pi-agent-ext-wayfind && bun test tests/freshness.test.ts )`
...
```bash
( cd bun-apps/pi-agent-ext-wayfind && bun run check && bunx tsc )
git add bun-apps/pi-agent-ext-wayfind/src/freshness.ts bun-apps/pi-agent-ext-wayfind/tests/freshness.test.ts
...
**Files:**
- Modify: `bun-apps/pi-agent-ext-wayfind/src/commands.ts` (import + top of `handleWayfinderChart` + both steer messages)
- Test: `bun-apps/pi-agent-ext-wayfind/tests/commands.test.ts` (new describe block)
...
```bash
( cd bun-apps/pi-agent-ext-wayfind && bun run check && bunx tsc )
git add bun-apps/pi-agent-ext-wayfind/src/commands.ts bun-apps/pi-agent-ext-wayfind/tests/commands.test.ts
...
**Files:**
- Modify: `bun-apps/pi-agent-ext-wayfind/skills/wayfinder/SKILL.md`
...
Run: `( cd bun-apps/pi-agent-ext-wayfind && bun test tests/skills.test.ts )`
...
Run: `( cd bun-apps/pi-agent-ext-wayfind && bun run check && bunx tsc && bun test )`
Expected: PASS — Biome clean, `tsc` exit 0, all tests green (freshness + commands + skills + the rest).

- [ ] **Step 5: Commit**

```bash
git add bun-apps/pi-agent-ext-wayfind/skills/wayfinder/SKILL.md bun-apps/pi-agent-ext-wayfind/skills/grilling/SKILL.md
