...
const out = join(import.meta.dir, "..", "..", "..", ".planning", "2026-09-15-kcard-retrieval-lift-2", "receipts", `production-mrr-${label}.json`);
writeFileSync(out, JSON.stringify({ label, alpha: alpha ?? 0.18, mrr: r.mrr, hitAt3: r.hitAt3, questions: r.questions, neverRanked: r.neverRanked, langSplit: { zhMrr: mrrOf(zh), enMrr: mrrOf(en), zhN: zh.length, enN: en.length }, detail: r.detail }, null, 1));
