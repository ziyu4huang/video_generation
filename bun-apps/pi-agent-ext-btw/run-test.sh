#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for pi-agent-ext-btw.
...
print_list() {
	cat <<EOF
$(Y "pi-agent-ext-btw run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/pi-agent-ext-btw-runtest.log 2>&1
	rc=$?
