...
/**
 * Run an inline TypeScript script in a child process.
 *
 * Under Bun, `--eval` has a parser bug (bun v1.3.x) where multiline scripts
 * with embedded brace blocks cause `import { X }` to be misinterpreted.
 * The workaround is to write the script to a temp file and execute it directly.
 * Under Node, we keep the original `--experimental-transform-types` + loader.
 */
export function execChildScript(
	script: string,
	options: { cwd?: string; env?: NodeJS.ProcessEnv } = {},
): string {
	if (process.isBun) {
		// Write the script to a temp file at the project root (where relative
		// imports like ./src/extension/index.ts resolve correctly), then execute
		// it directly. Bun natively handles TypeScript and .ts import resolution.
		const cwd = options.cwd ?? _projectRoot;
		const tmpFile = path.join(cwd, `__pi-subagent-eval-${process.pid}-${Date.now()}.ts`);
		...
		return execFileSync(
			...
			{ cwd: options.cwd ?? _projectRoot, env: options.env, stdio: "pipe" },
		).toString().trim();
	}
}
