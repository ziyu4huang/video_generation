...
const GENERIC_GLOBAL_CONFIG_PATH = path.join(homeDir(), ".config", "mcp", "mcp.json");
const IMPORT_PATHS = {
	...
	windsurf: [path.join(homeDir(), ".windsurf", "mcp.json")],
...
function resolveConfigPath(value: string | undefined): string | undefined {
	...
	if (resolved.startsWith("~/") || resolved.startsWith("~\\")) return path.join(homeDir(), resolved.slice(2));
	return resolved;
}
