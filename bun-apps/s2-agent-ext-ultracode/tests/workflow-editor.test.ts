...
describe("hasTrigger", () => {
  ...
  it('arms on "ultracode" by default (Claude Code alignment)', async () => {
    const { hasTrigger } = await load();
    assert.equal(hasTrigger("ultracode audit every route"), true);
    assert.equal(hasTrigger("ULTRACODE now"), true);
    assert.equal(hasTrigger("run this with ultracode"), true);
  });

  it('does NOT plural-special-case "ultracode"', async () => {
    const { hasTrigger } = await load();
    assert.equal(hasTrigger("ultracodes everywhere"), false);
  });

  it("a custom trigger word replaces the whole default list", async () => {
    const { hasTrigger } = await load();
    assert.equal(hasTrigger("run pi-workflow now", "pi-workflow"), true);
    assert.equal(hasTrigger("run ultracode now", "pi-workflow"), false);
    assert.equal(hasTrigger("run workflow now", "pi-workflow"), false);
  });
});
...
describe("endsWithTrigger", () => {
  ...
  it('returns true when text ends with "ultracode"', async () => {
    const { endsWithTrigger } = await load();
    assert.equal(endsWithTrigger("run this with ultracode"), true);
  });
