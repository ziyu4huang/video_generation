...
describe("installWorkflowEditor - tool availability", () => {
  ...
  it("should work with different keyword variations: 'workflow', 'workflows', 'WORKFLOW', 'ultracode'", async () => {
    const { installWorkflowEditor } = await import("../src/workflow-editor.js");

    for (const keyword of ["workflow", "workflows", "WORKFLOW", "WorkFlows", "ultracode", "ULTRACODE"]) {
      const mockPi = createMockPi();
