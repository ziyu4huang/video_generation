...
// When no custom word is configured, EVERY default word arms (workflow +
// ultracode — the latter mirrors Claude Code's ultracode keyword).
...
function triggerRegex(triggerWord: string = DEFAULT_KEYWORD_TRIGGER_WORD, flags = "i", atEnd = false): RegExp {
...
/** A still-default word means the user hasn't overridden → ALL defaults arm;
 *  a custom word replaces the whole default list. */
function effectiveTriggerWords(triggerWord: string): string[] {
  const word = normalizeKeywordTriggerWord(triggerWord) ?? DEFAULT_KEYWORD_TRIGGER_WORD;
  return (DEFAULT_KEYWORD_TRIGGER_WORDS as readonly string[]).includes(word.toLowerCase())
    ? [...DEFAULT_KEYWORD_TRIGGER_WORDS]
    : [word];
}
...
export function hasTrigger(text: string, triggerWord: string = DEFAULT_KEYWORD_TRIGGER_WORD): boolean {
  return effectiveTriggerWords(triggerWord).some((word) => triggerRegex(word).test(text));
}

export function endsWithTrigger(textBeforeCursor: string, triggerWord: string = DEFAULT_KEYWORD_TRIGGER_WORD): boolean {
  return effectiveTriggerWords(triggerWord).some((word) => triggerRegex(word, "i", true).test(textBeforeCursor));
}
...
export function colorizeWorkflow(
  ...
  triggerWord: string = DEFAULT_KEYWORD_TRIGGER_WORD,
): string {
  ...
  for (const word of effectiveTriggerWords(triggerWord)) {
    const globalTrigger = triggerRegex(word, "gi");
...
function triggerDisplayName(keywordTriggerWord: string | undefined): string {
  ...
  return word.toLowerCase() === DEFAULT_KEYWORD_TRIGGER_WORD ? "workflow/workflows or ultracode" : `"${word}"`;
}
