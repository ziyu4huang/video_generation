...
**[Website](https://quintinshaw.github.io/s2-agent-ext-ultracode/) · [npm](https://www.npmjs.com/package/@quintinshaw/pi-dynamic-workflows) · [Pi package](https://pi.dev/packages/@quintinshaw/pi-dynamic-workflows) · [GitHub](https://github.com/QuintinShaw/pi-dynamic-workflows)**

![s2-agent-ext-ultracode demo](https://raw.githubusercontent.com/QuintinShaw/pi-dynamic-workflows/main/docs/media/demo.gif)
...
If another Pi extension has already installed a custom editor component, s2-agent-ext-ultracode leaves it in place and keeps the submit-time workflow trigger active. In that compatibility mode, the animated keyword highlight and Backspace one-shot disarm affordance are skipped because the existing editor remains responsible for rendering and input handling; use `/workflows-trigger off` or `/workflows-trigger set <word>` when you need to discuss workflow/workflows without auto-triggering, including in future sessions. Editor composition is load-order dependent: whichever extension installs a visual editor last owns the editor surface, while s2-agent-ext-ultracode still keeps its submit-time hook registered.
...
| Claude Code dynamic workflows | s2-agent-ext-ultracode (on Pi) |
| --- | --- |
...
/workflows-trigger set <word>|reset
                            customize or reset the keyword trigger word (defaults "workflow" and
                            "ultracode"; "workflow" also matches "workflows"; custom words match
                            exactly, case-insensitive and replace the whole default list)
...
The defaults are `"workflow"` (legacy behavior, also matches `"workflows"`) and `"ultracode"` — the same arming keyword Claude Code uses for its Workflow tool. Custom trigger words are literal, case-insensitive terms with no spaces and no leading slash; setting one replaces the whole default list, so `"pi-workflow"` does not match `"workflow"`, `"workflows"`, `"ultracode"`, or `"pi-workflows"`.

## Reference

The full guide — every global, agent option, `agentType` definitions, structured output, and determinism — lives on the **[website](https://quintinshaw.github.io/s2-agent-ext-ultracode/)**. The essentials:
