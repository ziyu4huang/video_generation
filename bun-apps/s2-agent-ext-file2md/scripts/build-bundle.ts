 ...
 *   on the target machine. This mirrors what the s2-agent bundle already does
 ...
 *   the output AND, when the s2-agent bundle is present, a live load test that
...
// Where the s2-agent bundle lives, for the live load test. Resolved from the
// script's cwd (bun-apps/pi-file2md/) → repo root → dist/s2-agent/.
const PI_AGENT_BUNDLE = resolve(process.cwd(), "..", "..", "dist", "s2-agent", "s2-agent.js");
...
// abs-resolved thin bundle loads via `s2-agent.js -e <thin>.js`, registers
...
// This mirrors what the s2-agent bundle already does (bakes PI_PKG_DIR, symlinks
...
//
//   (C) JITI / AGENT LIVE BOOT (when dist/s2-agent/s2-agent.js exists): boot
//   the real s2-agent bundle with `-ne -e <bundle>`. This is the ONLY tier
  ...
  if (existsSync(PI_AGENT_BUNDLE)) {
    try {
      ...
      if (crashSig.test(stdout + stderr)) {
        ...
        console.log(`  ✓ live load: s2-agent booted, file2md registered`);
      }
    } catch (e: any) {
      warnings.push(`live load test threw: ${String(e?.message || e).slice(0, 120)}`);
    }
  } else {
    warnings.push(`live load test skipped — ${PI_AGENT_BUNDLE} not built (run s2-agent build first)`);
  }
...
await stageVerify();
console.log("▶ done");
console.log(`  load with:  bun ../../dist/s2-agent/s2-agent.js -ne -e ${OUTFILE} -p "list your tools"`);
