 ...
 * `s2-agent.js` — where no build stage ever put it:
 *
 *     failed to asynchronously prepare wasm: ENOENT … dist/s2-agent/mupdf-wasm.wasm
 *
 * Any command in the `s2-agent cli` namespace tripped this, not just `file2md`:
 ...
 * regression — the old standalone `dist/s2-agent-cli/cli.js` bundle had the
 * exact same hole.)
 *
 * The fix
 * -------
