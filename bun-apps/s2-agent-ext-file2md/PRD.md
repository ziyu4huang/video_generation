...
Two VLM "subagents" (single-turn, disposable s2-agent sessions):
...
`s2-agent-ext-flux2` (image gate verification) but is **not yet exposed as a
tool** (see roadmap T1).
...
Consumers today: `s2-agent` (`file2md` + `pdf-to-vault` commands),
`s2-agent-ext-flux2` (`askImage` for gate verification).
...
- `s2-agent-ext-obsidian` (vault output)
- `s2-agent` (hosts file2md command)

## Use

```bash
pi -e bun-apps/s2-agent-ext-file2md
# Then: file2md({input: "paper.pdf"})
# Or CLI:
bun bun-apps/s2-agent/src/cli.ts cli file2md paper.pdf
```
...
- **De-fork `sessions.ts`** — it duplicates `s2-agent`'s
