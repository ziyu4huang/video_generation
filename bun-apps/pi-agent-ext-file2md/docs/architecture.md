...
| 4 | Session factory | `src/session-factory.ts` → `createSharedSession()` | Builds `AgentSessionServices` (resolves `~/.pi/agent/models.json` via `getAgentDir()`), looks the model up in `ModelRegistry`, returns a real pi-agent session. |
...
one-turn pi-agent session is spun up, fed the image+question, and disposed. The
