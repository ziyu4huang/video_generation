#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for pi-agent-ext-file2md.
...
print_list() {
	cat <<EOF
$(Y "pi-agent-ext-file2md run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/pi-agent-ext-file2md-runtest.log 2>&1
	rc=$?
	...
	if [ "$rc" -ne 0 ]; then sed 's/^/      /' /tmp/pi-agent-ext-file2md-runtest.log | tail -n 25 >&2; fi
}
...
echo "$(Y "▶ pi-agent-ext-file2md run-test.sh — tier=$TIER")"
case "$TIER" in
