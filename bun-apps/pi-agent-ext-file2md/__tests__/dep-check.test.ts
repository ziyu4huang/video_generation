...
import { missingDeps } from "../extensions/file2md.ts";
...
describe("missingDeps", () => {
  test("returns [] when probing from a $bunfs virtual path (compiled binary)", () => {
    expect(missingDeps(["@earendil-works/pi-coding-agent"], "/$bunfs/root")).toEqual([]);
    expect(missingDeps(["@earendil-works/pi-coding-agent"], "/~BUN/root")).toEqual([]);
  });

  test("returns [] when probing from undefined", () => {
    expect(missingDeps(["@earendil-works/pi-coding-agent"], undefined)).toEqual([]);
  });

  test("finds installed deps from the real package dir", () => {
    const pkgDir = new URL("..", import.meta.url).pathname;
    expect(missingDeps(["@earendil-works/pi-coding-agent"], pkgDir)).toEqual([]);
  });
});
