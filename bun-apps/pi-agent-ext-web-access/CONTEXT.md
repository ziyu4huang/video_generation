...
The ubiquitous language of pi-agent-ext-web-access — web access for Pi: multi-provider search, content extraction (URL / YouTube / GitHub / video), and a browser-curator fallback for bot-blocked sites. Eight search providers behind one unified interface, with SSRF protection on every fetch.
