...
describe("summaryModelValue + modelMatchesEnabledPatterns", () => {
	test("summaryModelValue composes provider/id", () => {
		expect(summaryModelValue(model("lm-studio", "google/gemma-4-26b"))).toBe(
			"lm-studio/google/gemma-4-26b",
		);
	});
	...
	test("exact provider/id pattern matches (case-insensitive)", () => {
		const patterns = ["lm-studio/google/gemma-4-26b"];
		expect(modelMatchesEnabledPatterns(model("LM-Studio", "google/gemma-4-26b"), patterns)).toBe(true);
