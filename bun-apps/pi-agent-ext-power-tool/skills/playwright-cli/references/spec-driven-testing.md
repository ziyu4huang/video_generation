...
All three lean on the same mechanic: run `bunx playwright test --debug=cli` in the background, then `bunx playwright-cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics and [test-generation.md](test-generation.md) for how every `playwright-cli` action emits Playwright TypeScript.
...
```bash
PLAYWRIGHT_HTML_OPEN=never bunx playwright test tests/seed.spec.ts --debug=cli
# wait for "Debugging Instructions" and the session name tw-XXXX
bunx playwright-cli attach tw-XXXX
```

Resume so the seed runs, then probe the app:

```bash
bunx playwright-cli resume                   # resume so that seed test runs fully
bunx playwright-cli snapshot                 # inventory of interactive elements
bunx playwright-cli click e5                 # follow a flow
bunx playwright-cli eval "location.href"     # read URL / state
bunx playwright-cli show --annotate          # ask the user to point at something
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never bunx playwright test <seed-file> --debug=cli   # background
bunx playwright-cli attach tw-XXXX
# resume
```
...
```bash
bunx playwright-cli snapshot                         # find refs
bunx playwright-cli fill e3 "John Doe"               # -> page.getByRole('textbox', {...}).fill(...)
bunx playwright-cli press Enter
bunx playwright-cli click e7
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never bunx playwright test tests/<group>/<scenario>.spec.ts
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never bunx playwright test tests/<group>/<scenario>.spec.ts:<line> --debug=cli
...
```bash
bunx playwright-cli snapshot                # did the element change / move / rename?
bunx playwright-cli console                 # app-side errors?
bunx playwright-cli requests                # failed request? wrong payload?
bunx playwright-cli show --annotate         # ask the user to point somewhere
```
