...
How persistent and transient user-facing state is surfaced in this repo's pi-agent
extensions (`bun-apps/pi-agent-ext-*`). This doc exists so every new persistent
...
The `footer-extension-status-notify` patch in `bun-apps/pi-agent/src/patches/`
