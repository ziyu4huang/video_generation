...
# ── concurrency: warn (not block) if other pi-agent sessions are live ──
...
# Match BOTH legacy `pi-coding-agent` AND the current runtime `bun …/pi-agent/src/cli.ts`
...
pi_count=$( { ps aux | grep -iE 'pi-coding-agent|pi-agent/src/cli' | grep -v grep | grep -v "dedup.sh" || true; } | wc -l | tr -d ' ')
echo "▸ pi-agent processes: $pi_count  (race risk if another session writes the DB mid-run)"
echo "▸ store dir: $MEM_DIR"
if [ "$COMMIT" = 1 ] && [ "$pi_count" -gt 1 ]; then
  echo "  ℹ multiple pi-agent processes detected — the .md-trim below is now cross-process-locked (Workstream B), so a live session's write can't clobber it. DB hydration / a session's in-memory cache may still lag until it reloads." >&2
fi
