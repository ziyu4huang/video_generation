...
```bash
DEDUP=bun-apps/pi-agent-ext-hermes-memory/skills/pi-memory-bulk-dedup/dedup.sh
...
- Never run `--commit` while another pi-agent session is live — it races the harness and re-fragments (a root cause of the bloat). The `ps` check warns; `--commit` still proceeds but the `.md` trim is cross-process-locked so it can't lose a live write.
