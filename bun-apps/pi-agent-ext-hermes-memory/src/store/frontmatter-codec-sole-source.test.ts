...
// @repo/pi-agent-core-interface/src/embedding-leaf.ts — the ONLY non-test file
...
const CORE_LEAF = join(here, "..", "..", "..", "pi-agent-core-interface", "src", "embedding-leaf.ts");
...
describe("frontmatter-codec sole-source gate (C1)", () => {
  it("no non-test src file hand-rolls a yaml-parse-based fence split (must delegate to core-interface splitFencedYaml)", () => {
    ...
    assert.deepEqual(
      offenders,
      [],
      `non-test src files parsing yaml directly (must delegate to @repo/pi-agent-core-interface splitFencedYaml): ${offenders.join(", ")}`,
    );
  });
