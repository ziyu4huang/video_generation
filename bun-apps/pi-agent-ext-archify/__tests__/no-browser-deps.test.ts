 ...
 * over CDP — no download, no skip-gate. `pi-agent-ext-power-tool` in this repo
 ...
 *
 * **Scope**: these two packages only. `pi-agent-ext-power-tool` and
...
const WEBUI_ROOT = join(PKG_ROOT, "..", "pi-agent-ext-webui");
...
for (const [name, root] of [
  ["pi-agent-ext-archify", PKG_ROOT],
  ["pi-agent-ext-webui", WEBUI_ROOT],
] as const) {
