...
submodule and consumed by `s2-agent`'s CLI and `s2-agent-ext-tool-gate`.
...
```bash
# Auto-loaded via s2-agent's static extension set — just run a session.
bun bun-apps/s2-agent/src/cli.ts

# Standalone, one-shot:
bun bun-apps/s2-agent/src/cli.ts \
  -e bun-apps/s2-agent-ext-power-tool/extensions/power-tool.ts \
...
`s2-agent cli agent-trends` replays the pathology detectors over historical session
