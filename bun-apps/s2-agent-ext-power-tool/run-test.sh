#!/usr/bin/env bash
########################################
# run-test.sh — multi-effort-level test launcher for s2-agent-ext-power-tool.
#
# Mirrors bun-apps/s2-agent/run-test.sh's tier names. power-tool has no
...
print_list() {
	cat <<EOF
$(Y "s2-agent-ext-power-tool run-test.sh — effort tiers"):
...
echo "$(Y "▶ s2-agent-ext-power-tool run-test.sh — effort=$EFFORT")"
