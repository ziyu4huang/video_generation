**ID:** `ADR-wayfind-0007` — ADR numbers restart per context, so this number alone is ambiguous; cite this ID. Index: repo-root `CONTEXT-MAP.md`
...
`s2-agent-ext-wayfind` had grown into two things at once: Matt Pocock's
...
vocabulary already owned by the sibling `s2-agent-ext-superpowers` port. Two
...
scripts, tests, and golden fixtures moved to `@repo/s2-agent-ext-archify`
...
- **Do NOT blow the merged sections away with a naive upstream re-port.** The
  ...
  (`s2-agent-ext-superpowers/tests/__fixtures__/upstream-skills/UPSTREAM.ref`)
