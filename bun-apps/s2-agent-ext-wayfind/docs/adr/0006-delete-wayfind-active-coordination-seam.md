...
`s2-agent-ext-wayfind` published a process-singleton reader on `globalThis` under
...
- **The plan-coordinator yield is fiction in code.** No such plan coordinator
  ...
  after `s2-agent-ext-task` was built as the plan coordinator, it never
...
M9 `__piGoalActive` prose (`s2-agent-ext-task` CONTEXT/goal.ts/extensions)
