**ID:** `ADR-wayfind-0004` — ADR numbers restart per context, so this number alone is ambiguous; cite this ID. Index: repo-root `CONTEXT-MAP.md`
...
ADR-0002 made `s2-agent-ext-wayfind` take a `workspace:*` dependency on
`s2-agent-ext-task` to call `getSharedStatusWidget()` and register a
...
2. **The coupling is asymmetric across the family.** `s2-agent-ext-superpowers`
