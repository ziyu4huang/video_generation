**ID:** `ADR-wayfind-0003` — ADR numbers restart per context, so this number alone is ambiguous; cite this ID. Index: repo-root `CONTEXT-MAP.md`
...
The coordinator this ADR parked as "designed, not built" was subsequently built as **`s2-agent-ext-task`** (the `goal-todo` package, renamed). It publishes all three `__piPlan*` seams on `globalThis`:

- `s2-agent-ext-task/extensions/task.ts:56-58` — `__piPlanPhases` / `__piPlanIncomplete` / `__piPlanSummary`
- contract-pinned by `s2-agent-ext-task/__tests__/extension-contract.test.ts` ("publishes `__piPlan*` coordination seams on globalThis")
...
- No package publishes the `__piPlan*` seams — zero publishers across ts+js, including `s2-agent-cli` and `s2-agent-ext-power-tool`; wayfind (`chain.ts`, `coordination.ts`) and ext-task (`goal.ts`) only read them.
