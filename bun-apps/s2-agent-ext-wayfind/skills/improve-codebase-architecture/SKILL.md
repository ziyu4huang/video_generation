...
- For this repo's distinct clusters, you may optionally target a specific package: `bun-apps/s2-agent-ext-*`, `python/mlx-movie-director`, `bun-apps/gui-movie-director`, or any other directory the user names.
...
```bash
( cd bun-apps/s2-agent-ext-wayfind && bun run architecture:render ../../.planning/<effort>/architecture-review-<date>.md [out.html] )
```
