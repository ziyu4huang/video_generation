...
describe("tidyNextGoals — Phase 1: filename normalization", () => {
  it("converts an underscore separator to a dash (dash is the sole canonical form)", () => {
    const dir = tmpEffort();
    try {
      writeFileSync(join(dir, "output", "next-goal-20260706_0531.md"), "x");
      const r = tidyNextGoals(dir);
      expect(r.normalized).toBe(1);
      expect(r.removed).toBe(0);
      expect(basenames(dir)).toEqual(["next-goal-20260706-053100.md"]);
    } finally {
  ...
  it("pads missing seconds with trailing '0's (4-digit and 5-digit times)", () => {
    const dir = tmpEffort();
    try {
      writeFileSync(join(dir, "output", "next-goal-20260705_1100.md"), "a"); // -> -110000
      writeFileSync(join(dir, "output", "next-goal-20260705_22334.md"), "b"); // -> -223340
      const r = tidyNextGoals(dir);
      expect(r.normalized).toBe(2);
      expect(basenames(dir)).toEqual(["next-goal-20260705-110000.md", "next-goal-20260705-223340.md"]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("is idempotent on already-canonical names (second run is a no-op)", () => {
    const dir = tmpEffort();
    try {
      writeFileSync(join(dir, "output", "next-goal-20260705-110000.md"), "a");
      ...
      expect(basenames(dir)).toEqual(["next-goal-20260705-110000.md"]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("does not clobber when a normalized name collides with an existing file", () => {
    const dir = tmpEffort();
    try {
      // canonical dash target already present; the un-padded one should NOT overwrite it.
      writeFileSync(join(dir, "output", "next-goal-20260705-110000.md"), "canonical");
      writeFileSync(join(dir, "output", "next-goal-20260705_1100.md"), "short");
      const r = tidyNextGoals(dir);
      expect(r.normalized).toBe(0); // collision → skipped, not renamed
      expect(readFileSync(join(dir, "output", "next-goal-20260705-110000.md"), "utf-8")).toBe("canonical");
...
describe("tidyNextGoals — Phase 2: retention (keep N newest)", () => {
  it("trims to keepN newest by lexicographic (timestamp) order and keeps the right ones", () => {
    ...
    try {
      const files = [
        "next-goal-20260101-000000.md",
        "next-goal-20260303-000000.md",
        "next-goal-20260202-000000.md",
        "next-goal-20260404-000000.md",
        "next-goal-20260505-000000.md",
      ];
      ...
      expect(basenames(dir)).toEqual(["next-goal-20260404-000000.md", "next-goal-20260505-000000.md"]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("keeps everything when total <= keepN (default 10)", () => {
    const dir = tmpEffort();
    try {
      writeFileSync(join(dir, "output", "next-goal-20260101-000000.md"), "a");
      writeFileSync(join(dir, "output", "next-goal-20260202-000000.md"), "b");
...
describe("tidyNextGoals — edge cases", () => {
  ...
  it("preserves non-next-goal files in output/", () => {
    const dir = tmpEffort();
    try {
      writeFileSync(join(dir, "output", "next-goal-20260101-000000.md"), "goal");
  ...
  it("combines normalize + trim in one run", () => {
    const dir = tmpEffort();
    try {
      // mix of un-normalized (underscore / short) + canonical dash; keep the 2 newest after normalization
      writeFileSync(join(dir, "output", "next-goal-20260101-0000.md"), "old-short"); // -> -000000
      writeFileSync(join(dir, "output", "next-goal-20260202_120000.md"), "feb"); // -> -120000
      writeFileSync(join(dir, "output", "next-goal-20260303_235959.md"), "mar"); // -> -235959
      writeFileSync(join(dir, "output", "next-goal-20260404_0600.md"), "apr-short"); // -> -060000
      const r = tidyNextGoals(dir, 2);
      expect(r.normalized).toBe(4); // every legacy/short name folded onto dash-canonical
      expect(r.removed).toBe(2); // 4 total, keep 2
      // newest two: 20260404-060000 (was short), 20260303-235959 (was underscore)
      expect(basenames(dir)).toEqual(["next-goal-20260303-235959.md", "next-goal-20260404-060000.md"]);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
