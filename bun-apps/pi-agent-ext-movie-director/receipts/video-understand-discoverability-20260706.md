...
a pi-agent init failure — the routing layer (#305) and the CLIP subprocess
...
> The box was contended (3 concurrent pi-agent processes queueing on the one
> local gemma in LM Studio — the `pi-agent-headless-p-hang` contention
> scenario), so wall-clock was long, but the *routing signal* is turn-1
> deterministic and independent of the queue depth.

## Test gate

`bun test` in `bun-apps/pi-agent-ext-movie-director` → **166 pass, 1 skip,
