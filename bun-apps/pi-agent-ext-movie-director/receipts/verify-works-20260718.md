...
**Goal:** Verify the pi-agent-ext-movie-director agent+tool genuinely builds a story and
...
- `bun test` in `bun-apps/pi-agent-ext-movie-director`: **679 pass / 0 fail / 8 skip**.
...
Three `pi-agent` runs (`BUN_PI_LOAD_RUN_DIR=FALSE --no-structions -e ...movie-director`,
...
4. **Precise command-mapping retry** (stage→exact-command checklist, bash/run.py forbidden,
   ...
   environmental contention.** The box had 3–5 other concurrent pi-agent sessions (deploy /
   ...
   killed. Matches [[pi-agent-headless-p-hang]] — "works on a quiet box; 'hang' = contention."
...
MLX_MODELS_DIR=$(pwd)/mlx-models \
  bun bun-apps/pi-agent-ext-movie-director/scripts/verify-tool-video.ts
