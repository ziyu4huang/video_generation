# Workflow Integration — movie-director × pi-agent-ext-workflow
...
| workflow `dist/` was stale (predated the `call()` global feature) | **prerequisite:** run `( cd bun-apps/pi-agent-ext-workflow && bun run build )` whenever workflow src changes. dist is gitignored |
...
bun bun-apps/pi-agent-ext-workflow/samples/run.ts \
  bun-apps/pi-agent-ext-movie-director/workflows/scene-assets.js \
...
package.json                         # + @repo/pi-agent-ext-workflow workspace dep
```
