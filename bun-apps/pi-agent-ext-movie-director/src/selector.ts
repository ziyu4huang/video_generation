...
/**
 * Select a provider for a capability. Deterministic: the configured candidates
 * are ranked by backend preference then by REGISTRY declaration order (stable).
 * Throws NoConfiguredProviderError if nothing is wired.
 */
...
/** All configured providers for a capability, ranked best-first (for menu/UI). */
export function rankedProviders(capability: Capability): ProviderEntry[] {
  return REGISTRY.filter((p) => p.capability === capability && p.configured).sort(
