...
/**
 * adaptRunPyMusic — normalize a run.py music (MusicGen) result. The single
 * artifact is the produced music audio (kind:"audio"); ok = run.py exited 0 AND
 * the requested file landed with real content (mirrors runPyMusic's own "0-exit
 * but nothing written is NOT success" stance). Fully local MLX → honest $0
 * marginal cost, same stance as the other local-silicon providers.
 */
export function adaptRunPyMusic(
  ...
  return {
    ...
    artifacts,
    error: details.ok ? null : `${summary}\n${stderrTailStr}`.trim(),
    // local MLX MusicGen — honest $0 marginal cost (same stance as local-silicon providers).
    cost_usd: details.ok ? costFor(req.capability, null, env) : 0,
    duration_seconds: null,
    seed: null,
    model: details.model ?? "facebook/musicgen-small",
  };
}
...
async function realRunPyMusic(req: GenerateRequest, env?: Record<string, string | undefined>): Promise<ToolResult> {
  ...
  const outputDir = req.outputDir ?? process.cwd();
  const output = opts.output ?? join(outputDir, "music_musicgen.wav");
