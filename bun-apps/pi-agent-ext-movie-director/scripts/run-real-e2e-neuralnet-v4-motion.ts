 ...
 *    --from-image` (LTX-2.3 I2V, via runPyVideo from @repo/pi-agent-ext-ltx —
 ...
 *   bun run --cwd bun-apps/pi-agent-ext-movie-director scripts/run-real-e2e-neuralnet-v4-motion.ts
 * Re-run after the first (cached, ~seconds):
 *   SKIP_ASSETS=1 bun run --cwd bun-apps/pi-agent-ext-movie-director scripts/run-real-e2e-neuralnet-v4-motion.ts
 */
import { existsSync, mkdirSync, copyFileSync } from "node:fs";
import { join } from "node:path";
import { runPyVideo } from "@repo/pi-agent-ext-ltx";
import {
  getStageOrder,
