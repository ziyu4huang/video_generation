#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for pi-agent-ext-movie-director.
...
print_list() {
	cat <<EOF
$(Y "pi-agent-ext-movie-director run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/pi-agent-ext-movie-director-runtest.log 2>&1
	rc=$?
	...
	if [ "$rc" -ne 0 ]; then sed 's/^/      /' /tmp/pi-agent-ext-movie-director-runtest.log | tail -n 25 >&2; fi
}

run_quick() { ( cd "$SCRIPT_DIR" && bun test ${EXTRA[@]+"${EXTRA[@]}"} ) ; }
run_contract() { ( cd "$SCRIPT_DIR" && bun test extensions/extension-contract.test.ts ) ; }

echo "$(Y "▶ pi-agent-ext-movie-director run-test.sh — tier=$TIER")"
case "$TIER" in
