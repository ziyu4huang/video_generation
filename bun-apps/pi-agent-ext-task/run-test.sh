#!/usr/bin/env bash
########################################
# run-test.sh — tiered test launcher for pi-agent-ext-task.
...
print_list() {
	cat <<EOF
$(Y "pi-agent-ext-task run-test.sh — tiers"):
...
step() {
	...
	start=$(date +%s)
	"$@" >/tmp/pi-agent-ext-task-runtest.log 2>&1
	rc=$?
	...
	if [ "$rc" -ne 0 ]; then sed 's/^/      /' /tmp/pi-agent-ext-task-runtest.log | tail -n 25 >&2; fi
}

run_quick() { ( cd "$SCRIPT_DIR" && bun test ${EXTRA[@]+"${EXTRA[@]}"} ) ; }
run_contract() { ( cd "$SCRIPT_DIR" && bun test __tests__/extension-contract.test.ts ) ; }

echo "$(Y "▶ pi-agent-ext-task run-test.sh — tier=$TIER")"
case "$TIER" in
