 ...
 *   - optional deps whose `os`/`cpu` do not match the deploy target are pruned
 *     (sharp's @img/* platform binaries: only the darwin-arm64 pair belongs in
 *     an Apple-Silicon deploy). Hard deps are never platform-filtered — an
 *     install that satisfies a hard dep for the wrong platform is broken
 *     regardless of what we copy.
 *
