...
| Deploy the versioned sh core + ext set (Pipeline B, registry `pi-agent.registry.yaml`) | `deploy` — `bun run --cwd bun-apps/pi-agent deploy` (CLI: `bun bun-apps/pi-agent-ext-devops/src/deploy-cli.ts [--list]`) |
...
- **Every other owned phase now has one too.** All take `--help`, print the
  ...
  bun bun-apps/pi-agent-ext-devops/src/deploy-cli.ts [--list]
  ```
