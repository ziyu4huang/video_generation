...
- `src/gh.ts` — the real `GhClient` wrapping the `gh` / `git` CLI via `Bun.spawn`; pure JSON parsers (`parsePrView`, `parseChecks`).
...
```bash
( cd bun-apps/pi-agent-ext-devops && bun test )   # 31 tests: logic + recipe + parsers + entry smoke
```
