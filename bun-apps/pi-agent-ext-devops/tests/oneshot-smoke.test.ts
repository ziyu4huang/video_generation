...
function mkFakeRepo(): string {
	...
	const files = [
		...
		"bun-apps/pi-agent/src/cli-argv.ts",
		"bun-apps/pi-agent/src/static-extensions.ts",
		"bun-apps/pi-agent-ext-hermes-memory/extensions/hermes-memory.ts",
		"bun-apps/pi-agent-ext-hermes-memory/src/index.ts",
		"bun-apps/pi-agent-ext-devops/src/oneshot-smoke.ts",
	];
	for (const rel of files) {
		mkdirSync(join(repo, rel, ".."), { recursive: true });
		writeFileSync(join(repo, rel), `// fake ${rel}\n`);
	}
	writeFileSync(join(repo, "bun-apps/pi-agent-ext-devops/package.json"), '{"name":"@repo/pi-agent-ext-devops"}');
	return repo;
}
...
describe("runOneshotSmoke — runner via fake SpawnFn + fake repo", () => {
	...
	test("cached pass (hash match, <6h) → skip, ZERO spawns", async () => {
		...
		await writeState(join(repo, "bun-apps/pi-agent-ext-devops/.cache/oneshot-smoke.state.json"), {
			version: STATE_VERSION,
			inputHash: hash,
	...
	test("first run: fast probe uses `-ne -ns` (90s cap), canary keeps `-ns` only (180s cap)", async () => {
		...
		const state = await readState(join(repo, "bun-apps/pi-agent-ext-devops/.cache/oneshot-smoke.state.json"));
	...
	test("timeout → FAIL with the incident diagnostic; state NOT written", async () => {
		...
		expect(existsSync(join(repo, "bun-apps/pi-agent-ext-devops/.cache/oneshot-smoke.state.json"))).toBe(false);
		rmRepo(repo);
	});

	test("missing hashed input (rename) → structured FAIL naming the file, never a throw", async () => {
		const repo = mkFakeRepo();
		rmSync(join(repo, "bun-apps/pi-agent-ext-hermes-memory/src/index.ts"));
		const { fn, calls } = mkSpawn([pass()]);
