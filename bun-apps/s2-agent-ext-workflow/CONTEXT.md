...
The ubiquitous language of s2-agent-ext-workflow — Claude Code–style dynamic workflows for Pi: the model writes a small JavaScript orchestration script that fans out many isolated subagents in parallel, keeps intermediate work in script variables (not the chat), and returns only the result.
...
The shared, **publicly-exported** wrapper over `WorkflowAgent.run` for programmatic single-subagent dispatch from peer-extension CODE (not the LLM tool path). **Owned by `@repo/s2-agent-ext-subagent`** since the subagent extraction, and NOT re-exported from this package — import `spawnSubagent` + `SpawnSubagentOptions` + `SpawnSubagentResult` + `AgentUsage` from `@repo/s2-agent-ext-subagent`. Used by `s2-agent-ext-wayfind`, `s2-agent-ext-superpowers`, `s2-agent-ext-knowledge-card` (`zk_card`/`zk_ask`), `s2-agent-ext-hermes-memory`, `s2-agent-ext-file2md`. Same runner as the `subagent` tool and `agent()`; returns `{output, exitCode, stderr, timedOut, usage?}` (the shape mirrors the old child-process `runSubagentWithRetry` — see the note on the `exitCode`/`stderr` vocabulary in that package's CONTEXT).
