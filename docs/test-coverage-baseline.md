...
`pi-agent-ext-power-tool` (added in the verification-hygiene pass — PR for goal
...
| `pi-agent-ext-power-tool` | 286 pass / 8 skip / 0 fail | **55.05** | **62.89** | new wiring; many `todo/*` leaves untested (see below) |
...
- **`pi-obsidian` real-vault snapshot drifts by design.** `baseline.test.mjs`
  ...
  `vaults_root/pi-agent-vault` submodule (598 notes at capture). It drifts
