...
|-------|-------------------|-----------|------------|
| **Active vault root** | `bun-apps/pi-agent-ext-research-tool/lib/vault.ts#resolveVaultRoot` — tiers: `OB_VAULT_PATH` env (1a) → `run-dir/obsidian_config.json` `vault_path` when mode≠app (1b) → `<cwd>/.pi/obsidian_config.json` `vault_path` legacy (1c) → `<cwd>` fallback | all `pi-agent-ext-research-tool` tools: `collect_videos`, `arxiv_fetch2md`, `organize_vault_notes` (write targets `weekly-news/`, `papers/`) | `obsidian_config.json` absent **and** `OB_VAULT_PATH` unset → **silently** falls back to `<cwd>`; writes land in the repo instead of the intended vault. *(Incident: 2026-07-18, arxiv_fetch2md — had to pass explicit `output_path`.)* |
...
- Per-package `CONTEXT.md` files (e.g. `bun-apps/pi-agent-ext-research-tool/CONTEXT.md`) —
