...
7. **Branch hygiene** — run `./scripts/stale-branches.sh`; expect **0 stale**.
...
one command — run it after every merge and at the start of each cycle:

```bash
./scripts/stale-branches.sh      # lists branches outside the keep-set, each w/ PR state
./scripts/stale-branches.sh --prune   # delete them
```
...
./scripts/stale-branches.sh              # expect 0 stale
```
...
- `branch-cleanup` project skill + `./scripts/stale-branches.sh` (SOP #320)
