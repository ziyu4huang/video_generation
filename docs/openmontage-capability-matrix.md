# OpenMontage capability matrix (dated 2026-07-08, updated 2026-07-10 — SyncNet LSE-D/LSE-C lip_sync re-measurement + reference_to_video same-frame-0 multi-anchor test + ambient_sound verification + offline/local_gpu/reference_image coverage-gap fix)
...
| `offline` | **yes — shipped 2026-07-09/10, PR #402** | `run.py --offline image t2i` / `video generate` / `video t2i2v` | not yet ported (Swift has no network calls to gate in the first place — its model loads are always local file reads, so the flag is moot there rather than missing) | Found this pass rescanning `ltx_video_local.py`/`wan_video.py`/`hunyuan_video.py`/`cogvideo_video.py` `supports` dicts for flags this matrix hadn't rowed yet (`offline`, `local_gpu`, `reference_image` were all present since OpenMontage's initial-release commit, confirmed via `git log`/`git blame` in `../OpenMontage`, so this is a matrix coverage gap, not a moving target). `--offline` sets `HF_HUB_OFFLINE=1`/`TRANSFORMERS_OFFLINE=1`, runs a weight-presence preflight (`check-model --preflight`) that aborts loud instead of silently fetching, and gates network-dependent stages (VLM caption, pose-model download, storyboard brain) unless `--vlm-online` is passed. 19/19 offline tests pass with `--run-gpu`. See `docs/offline-egress-map.md`. |
| `local_gpu` | **yes — trivially true, whole stack** | n/a (always true) | n/a (always true) | The entire generation stack (Python MLX + Swift MLX) runs on local Apple Silicon GPU/ANE via MLX; there is no remote-API code path to fall back from. Rowed for completeness after finding it in the same `supports` dicts as `offline`/`reference_image` above — no dedicated verification needed beyond "this repo has never had cloud inference." |
| `reference_image` | **yes — same mechanism as `image_to_video`, verified 2026-07-10** | `video generate --input-image X --prompt ...` | `native-i2v --input-image X` | Confirmed (not assumed) by reading `reference_image_url`/`reference_image_path` param-schema entries alongside the `reference_image: True` flag in `wan_video.py`/`ltx_video_local.py` — it is the same single conditioning-image input already tracked by the `image_to_video` row, just named differently in the `supports` dict vocabulary; not a distinct capability requiring separate coverage. |
