...
| **MacBook 拆解** (ours) | `browser-demo/src/macbook/` | Exploded-view teardown in space grey: assembled it's an open MacBook (live wallpaper with menu bar / dock dots / notch, backlit keyboard, trackpad, glowing logo); an explode slider (or 自動拆解 ping-pong) slides ~130 instanced parts apart into a teardown stack with per-group stagger — bottom shell → battery → logic board (gold-edge PCB, traces) + spinning fans + copper heat pipe → keyboard deck → display lid, which flattens from open to flat while lifting (per-instance rotX animated by group in the vertex shader). Look: two-light studio rig, fresnel env reflection + cool rim on the anodized aluminum, edge-crease darkening, fake contact shadow. Hinge trick: rotate around the instance origin with a `localOffset` applied pre-rotation. |
