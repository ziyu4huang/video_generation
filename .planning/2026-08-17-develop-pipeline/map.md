...
## Not yet specified
- Q1 Handoff contracts: what exactly freezes at map->spec, spec->plan,
  plan->execute? (artifact checklist per edge; who enforces?)
- Q2 Verify-child protocol: skill-level rule (superpowers) or per-plan
  convention?
- Q3 Task coordinator extension (reads .planning/plans directly): a third
  executor in the diagram, or subsumed by executing-plans?
- Q4 Budget policy: per-stage/per-child token+turn accounting; ledger format?
- Q5 Long-term home for the diagram of record: this map, CONTEXT-MAP.md, or
  both extension READMEs?
