...
**Notes:** the template is a COPY source for the human-in-the-loop loop scaffold. **Adjudicated 2026-08-23 (review finding, verified): the original brief premise was wrong — the deleted template (44 lines, verified back to its 2026-08-08 port commit) contained only `step()` + `capture()`; the helper list below is the wizard template's library, not this template's.** Executed as: a wizard-style Bun library port (from wayfind/skills/wizard/template.sh) that satisfies the binding structural golden (STAGES / `stage(` / `ask_secret` / `open_url` / failure-exit), with the old template's substance (comment header, reproduction example, `--- Captured ---` KEY=VALUE dump) preserved in the authoring section. Future readers: the old file's substance is the floor; the library is gold-plating per the (wrong) brief. Convert each library helper (`step`/`capture`/`stage`/`say`, `open_url`, `ask`/`ask_secret`, `write_env`, `set_secret`/`set_var`, `pause`/`confirm`) to its Bun equivalent; keep `STAGES` markers and the identical stage shape; the emitted artifact becomes `bun hitl-loop.ts`-runnable with the same stage semantics. Golden = structural: the template file contains `// STAGES` (exact marker text, NOT merely `TOTAL_STAGES`), `stage(`, `ask_secret`, `open_url`, `exit` on failure — asserted by reading the file (no execution).
