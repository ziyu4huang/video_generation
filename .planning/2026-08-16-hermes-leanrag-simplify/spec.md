...
4. **Search tools 3→1**: `memory_search` + `session_search` merge into one search tool with a `mode` param (memory|session|knowledge); knowledge_search stays separate (zk graph retrieve is a different contract, ADR-pi-agent-0004 two-path decision honored).
