...
Itemized strategies the loop has EARNED, each citing its evidence. Boundary:
learnings = toolchain FACTS; this file = operational STRATEGIES (supersede-
able, capped; itemized deltas only). Read at arc open; curated at close.
...
- **Confidence:** high
- **Status:** active · added 2026-09-11
...
- **Scope:** receipts
- **Strategy:** Make attribution passive (record per-leg model via flags + env override + JSONL-derived receipt field; mismatch voids the leg), and when a prior map's narrative disagrees with artifacts, re-attribute from the session JSONL, not prose. When a behavioral verdict looks bad, first re-adjudicate the preserved bytes under a corrected detector — a "PARTIAL" can be a double detector artifact and the model was compliant all along. Suspect the ORACLE too: an endpoint's model list is a CATALOG, not residency (use `lms ps` for loaded models) — a precondition keyed to a catalog count can be unsatisfiable by construction (A/B finding, 2026-09-11).
- **Evidence:** .planning/2026-09-10-spwf-drive-ab/map.md (§0 re-attribution table, D1); .planning/2026-09-10-spwf-ab-closing/map.md (§0); .planning/2026-09-11-selfimprove-playbook/evidence/ab-arc-open/ (PLAYBOOK-DISADVANTAGE adjudication)
- **Confidence:** high
- **Status:** active · added 2026-09-11
...
- **Confidence:** high
- **Status:** active · added 2026-09-11
### PB-21 — Test model-visible strings by CONTENT, not by type-checking
- **Scope:** receipts
- **Strategy:** Line-splice typos in concatenated model-visible strings (`"…" +` newline `+"…"` → NaN) pass tsc and biome silently and can delete whole steering sentences from tool descriptions. Any description that carries routing or safety weight gets a content test: assert the load-bearing sentence is present and no splice artifacts (`NaN`, dangling quotes) remain.
- **Evidence:** .planning/2026-09-14-spwf-improve/ (evidence/pre-fix/runtime-description.txt — "NaNUse this…" shipped in every boot; description-integrity test in effort-tool.test.ts; PR #2281)
- **Confidence:** high
- **Status:** active · added 2026-09-15
