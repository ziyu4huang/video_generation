...
- **8 × ADR header line 1**: `Index: \`bun-apps/docs/adr/INDEX.md\`` → `Index: repo-root \`CONTEXT-MAP.md\`` (its header declares "This file is the index"; no INDEX.md exists anywhere — verified 2026-08-25). `**ID:**` lines untouched (`bun-apps/tests/adr-citation.test.ts` validates those only; `test:adr` green is the proof).
