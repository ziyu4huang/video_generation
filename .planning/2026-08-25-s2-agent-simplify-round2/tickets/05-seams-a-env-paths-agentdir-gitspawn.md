...
- **envFlag ×3 → 1 leaf `src/env-flag.ts`** (zero imports): patches/index.ts now imports + re-exports it (force-response-language.ts and both test importers unchanged); shared.ts `isSkipModelsJson` and e2e-harness `E2E_ENABLED` migrated. **Reviewer correction**: the chart's ×3 census missed a FOURTH hand-roll — `e2e-image-agent.test.ts` `PI_AGENT_E2E_IMAGE` — migrated in the same PR (finding Important-1). **Flagged widening**: the former hand-rolls were case-sensitive "1"/"true"(/"yes") — envFlag also accepts "TRUE"/"Yes". Opt-in flags only; run-test.ts sets the literal "1".
