...
**Build status:** 08 shipped (#1208); 09 shipped; 10 shipped (#1242); 14 shipped (HNSW index — T1/T2/T3/T5a/T6; tickets 17/18 spawned for T4/T5b); 15-Phase1 shipped (#1168); 19 shipped (#1282 — LeanRAG selective port, dedup-first slice: contentHash dedup seam + survivingK cap). Still unblocked but not picked: 07 (image cards), 13 (memory-card graduation).
...
- Build status / next-pick: Spine [12 + 06a + 06b] shipped; **08 shipped (#1208), 09 shipped, 10-impl shipped (#1242), 14 shipped (HNSW index — T1/T2/T3/T5a/T6; tickets 17/18 spawned for T4/T5b), 15-Phase1 shipped (#1168)**. **Open build set = {03 (two-layer typed graph), 07 (image card + extractor), 13 (memory-card graduation), 20 (LeanRAG multi-signal frequency-vote — blocked by 03)}**. Ticket 19 spawned from the LeanRAG evaluation (ADR-0001). 07/13 remain unblocked but not picked; 10 shipped; 14 shipped (T1/T2/T3/T5a/T6; 17/18 spawned for T4/T5b). The full 3-tier-drift impl behind closed 05 (06b stubbed Tier-1 only) is a candidate fresh ticket (pending — NOT created). **Next-pick: TBD (HITL) — 19 shipped (#1282); open set {03, 07, 13, 20}.** (05 closed — migrate-at-graduation + 3-tier drift policy → task 13; spine milestone done — see top of map.)
