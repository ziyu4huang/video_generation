...
- 06 (closed) → impl SPLIT into **06a (card-agnostic store)** + **06b (spine orchestrator, TBD)**. 06a spec: `specs/2026-08-08-hermes-card-store.md`; 06a plan: `plans/2026-08-08-hermes-card-store.md`. 06a generalizes `MemoryStore` → `Card{kind}` + pluggable serializer (memory+knowledge) + pluggable dedup strategy; knowledge corpus round-trips SQLite; zk unchanged. 06b = `ingestPath`/`walkAndIngest` + zk→store seam wiring + embed (04) + drift (05) + migration (13).
