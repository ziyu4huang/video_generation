...
Ticket 07 (hierarchical retrieval) — Phase A build (D4–D8, PR #1885) + ticket 04's typed model (D15–D18) make 05/06/07 all unblocked. 07 goes first: it is the SurrealDB-heavy core of the OpenViking-clone goal (directory-recursive search with score propagation on the `card`/`kind` index, D18's type filter, client-side BFS per ticket 03) and everything else (05's virtual type dirs, 08's hotness feed, 09's eval gate) consumes its retrieval surface. 06 (session extraction → `event` writer, D16) can proceed in parallel once 07 claims its lane; D14 governs all build work (A/B vs the 17/20 baseline + independent reviewer).
