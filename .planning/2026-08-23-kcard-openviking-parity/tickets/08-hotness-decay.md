...
D14 A/B (receipts `output/recall-audit/receipt-ticket08-hotness-{baseline,on,noise}.json` + post-review `receipt-ticket08-postreview-{baseline,on,noise}.json`, live bge-m3 + SurrealDB, 2026-08-24):
- **baseline** (α=0, empty ledger): 17/20 hit@5, hit@1 12/20, MRR 0.725 — byte-identical to the ticket-05 shipped numbers, reproduced again post-review (default OFF is a true no-op).
- **noise control** (α=0.10, non-target usage seeded at equal event count): hit@5 17/20 identical to baseline; MRR 0.725 pre-review / 0.717 post-review (one in-set rank shift — the F2/F3 determinism fix seeds a DIFFERENT non-target set, sorted order). No miss introduced: usage noise does not move recall (the D8 ≤±10% bound holds; synthetic events cleared after every run, `--reset-usage`).
- **mechanism arm** (α=0.10, target usage): 18/20 hit@5, hit@1 18/20, MRR 0.900 (pre- and post-review identical) — the bounded blend lifts recently-used cards as designed. CIRCULAR BY CONSTRUCTION (seeds = answer keys): a mechanism demonstration, NOT a production recall claim. Per D39 the default stays **0 = OFF**; any default flip routes through the ticket 09 gate with REAL ledger data once the injector feeds it.

Independent reviewer (D14): **FIX-FIRST first pass** — F1 MAJOR (semantic-path hotness double-count: flat pre-blend perturbed lexRankNorm before trySemanticBlend blended again) + F2–F5 minor (non-deterministic seeding order, ambiguous target-substring match, receipt missing reset/seed state, halfLifeDays ≤ 0 guard). ALL FIXED in-session: flat blend relocated after the semantic attempt (semantic path now applies hotness exactly once, over unblended union scores), sorted-list + exact-stem-first seeding, receipt records `{mode, reset, cards, events}`, hotnessScore guards halfLifeDays. Regression tests added for both gaps (F1 single-application pin; hier-lane blend unit test — fake-client dispatch, pool-wide values + lane-failure parity). **Second reviewer pass: GO** (all five findings + the test gap verified fixed, 591/591 + tsc re-run by the reviewer).

Gates: kcard `bun run test` 591/591 (+19 hotness) + typecheck clean; hermes `scripts/recall-audit.test.ts` fixture green against the modified harness; local_ci PASS.
