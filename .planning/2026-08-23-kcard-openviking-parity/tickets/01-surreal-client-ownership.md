...
## Build receipt (2026-08-23, same session — the D4–D8 build landed)

- `SurrealClient` + unit tests + `SURREAL_DEFAULTS` now at `bun-apps/s2-agent-core-interface/src/surreal-client.ts` (exported from the package index); `bumpRoundTrips` became the injectable `onRoundTrip` option, hermes injects it from `surreal-backend.ts`. core-interface tsconfig gained `DOM`/`DOM.Iterable` (the client's fetch/Response surface) and one `noUncheckedIndexedAccess` fix (`statements.at(-1)!`, caught by s2-agent-ext-subagent's stricter tsc via the boot-smoke prebuild).
- D7: `backend-factory.ts` default flipped to `surrealdb` (both `createBackendBundle` and `createBackendBundleWithFallback`); `createBackendBundleWithFallback` remains the surrealdb-down → sqlite startup guard, so sqlite stays the permanent escape hatch.
- D8: `EMBEDDING_CONFIG` §4 in `src/pre-load-providers.ts` (base DERIVED from the lm-studio provider entry `/v1`-stripped = `http://localhost:1234`, model re-exported from `SEMANTIC_MODEL_DEFAULT`); published at startup by `patches/pre-load-providers.ts` via `publishSeam("__piEmbeddingConfig", …)`; new seam key registered `crossPackage:false` (publisher = host, outside the seam-contract scanner set — `__piRateLimitState` precedent); `resolveSemanticEmbedConfig` order = seam → env → defaults. Runtime-verified: after importing the patch, seam publishes and resolve returns `http://localhost:1234` + `text-embedding-bge-m3`.
- Known trap from the Resolution section (config does not reach `getCardEmbeddings`) remains kcard-side — deliberately NOT touched here (this build is non-kcard by scope).
- Gates at receipt: core-interface `check`+`test` green (56), hermes-memory `check`+`test` green (1553), s2-agent `typecheck`+`test` green (1030, incl. real-CLI-boot boot-smoke canary), `bun run test:seam` green (8).
