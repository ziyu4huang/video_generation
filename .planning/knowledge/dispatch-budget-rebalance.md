...
- **proposed procedure**:
  1. Run `bun scripts/runs-stats.ts` in pi-agent-ext-subagent (per-status counts + token/turns medians from the runs DB) + read the empirics — prioritize turns > tokens > time. `--trend` shows the snapshot trajectory + gate state; `--snapshot` appends one after every bounds change.
     Records now self-tag budget cohort (`budget.source` on new runs, 2026-08-18 forward-fix) — cohort medians replace raw aggregates for recalibration.
