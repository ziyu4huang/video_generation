...
- **Deploy layout**: outRoot `~/proj/dist/s2-agent-sh` (registry `bun-apps/s2-agent/src/registry-config.ts`); deploy from the `__deploy` worktree; frozen versioned dirs + `current` symlink; `ext/<name>/{ext.cjs,ext.json,wasm/}`; verify-deploy-e2e probes = boot / ext-load (17 extensions) / model-call.
