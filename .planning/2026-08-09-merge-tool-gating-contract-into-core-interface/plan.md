...
5. `pi-agent-ext-core-task/extensions/core-task.ts`: REMOVE the dead `/// <reference types="@repo/pi-tool-gating-contract" />` line (verify no gating usage via grep first).
