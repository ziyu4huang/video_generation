...
- **Feedback / calibration loop**: once candidates are captured + promoted (or rejected), does the agent learn to calibrate "skill-worthy" judgment over time (e.g. a rejected-pattern memory)? Ticket 03 (recognition criteria) is now closed; the signal still depends on ticket 05 (promotion path) landing before it can be ticketed precisely.
