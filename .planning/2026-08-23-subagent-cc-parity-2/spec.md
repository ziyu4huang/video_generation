...
| `/loop` dynamic (`schedule_wakeup`) | **PASS (final, 2026-08-23 night — B4 fixed, ticket 03)**: TOOL half — the model called `schedule_wakeup {delaySeconds: 5, reason …}` headless and got the correct no-active-loop text back; clean exit 0. COMMAND half — post-fix live smoke: `/loop status` and `/loop:2 dynamic <prompt>` both dispatch with ZERO model turns and bounded exits; plain `/loop dynamic …` hits ext-task's guard pointer (no mis-schedule). The interim defect (B4): ext-task + ext-ultracode both register `loop`, the SDK suffixes both to `loop:1`/`loop:2`, `getCommand("loop")` missed and the literal text fell to the model — fixed by the `colliding-command-dispatch` patch (plain-name miss retries `name:1`) + an ext-task parser guard for `dynamic`/`off`. Plain `/loop` = ext-task's scheduler (D5); ultracode's subagent-side = `/loop:2`. Full write-up: `.planning/2026-08-23-headless-dispatch-hang/tickets/03-*.md` |
