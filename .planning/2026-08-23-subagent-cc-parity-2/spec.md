...
| `/loop` dynamic (`schedule_wakeup`) | SPLIT **RE-RUN 2026-08-23 evening**: TOOL half PASS — the model called `schedule_wakeup {delaySeconds: 5, reason …}` headless and got the correct no-active-loop text back; run completed, clean exit 0. COMMAND half DEFECT (B4) — `-p '/loop dynamic …'` never dispatches: ext-task and ext-ultracode both register a `loop` command, the SDK suffixes both to `loop:1`/`loop:2`, `getCommand("loop")` finds neither, and the literal text falls to the model (runaway exploration, watchdog-bounded at 300s). The original "BLOCKED (B1)" reading was this defect plus B1. Ticketed: `.planning/2026-08-23-headless-dispatch-hang/tickets/03-*.md` |
