...
- **Task 1 probe digests (live-smoke find)**: the snippet hashed into raw BigInt fields — `JSON.stringify(fp)` threw inside before_agent_start (non-fatal extension error → session exit 0, marker absent). Fixed by String()-wrapping digests, types → string (dcb3a45e).
- **Task 2 attribution (live-smoke find)**: inline-registered extensions carry `path=<inline:s2-agent-ext-X>`, which the slash-only attribution rule never matches. Fixed by also matching `<inline:<name>>`/`<inline:<package>>` (e1e15536).
