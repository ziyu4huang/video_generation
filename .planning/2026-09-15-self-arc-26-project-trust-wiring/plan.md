...
`self-arc-26-project-trust-wiring` at `ff6998bd`, clean). Queue head =
...
- **Deploy mechanics (arc-25 precedent, to re-run verbatim):** deploy from
  ...
  `@earendil-works/pi-coding-agent` is bundler-EXTERNAL
  (`s2-agent-ext-devops/src/deploy-run.ts:41` idiom) — `ProjectTrustStore` /
  `getAgentDir` resolve from the deployed tree's node_modules at runtime, and
  the require-destructure PROPERTY NAMES are the **PB-09** grep targets
  (minification keeps property names, not locals).
...
≥1 (require-destructure names of the externalized peer import). A miss = stale
bundle = STOP and investigate the cache (learning #1/#2), never re-receipt.
...
(drift check — the planner read them at `ff6998bd`).
