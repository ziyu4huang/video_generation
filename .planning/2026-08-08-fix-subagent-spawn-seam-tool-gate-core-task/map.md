...
**Status:** IN PROGRESS — discovery complete (2026-08-08). Shipping optimizations #1 (active-set threading) + #2 (tool-gate seeding) first. #3 (state isolation = open ticket #16) and #4–#6 deferred.
...
|---|---|---|---|---|
| **1** | **Thread the parent's gated active set into the child** — when the `subagent`/`subagents` caller omits `tools`, default the child's tool set to `pi.getActiveTools()` instead of the full definition universe. Caller's explicit `tools` still overrides. | **Highest** (kills the ~8k tok/child re-pay) | Safe, independent | SHIPPING |
| **2** | **Seed the child's tool-gate** — idempotent `ensureSeeded()` in `before_agent_start` that seeds `sticky` from `effectiveCore` (and builds `measuredTokens` once) when `sticky` is empty. | Med (correctness + per-turn cost) | Safe, independent | SHIPPING |
...
## Tickets
- `tickets/01-thread-active-set-into-subagent.md` (#1) — IN PROGRESS
- `tickets/02-tool-gate-ensure-seeded.md` (#2) — TODO
- `tickets/03-state-isolation-sessionid.md` (#3, = core-task ticket #16) — DEFERRED
