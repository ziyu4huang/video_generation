...
- [Shared `.planning/` concurrency](tickets/04-shared-planning-concurrency.md) — **accepted as last-write-wins**, documented in wayfind ADR-0005 + a sharpened wayfinder-skill caveat. No lock: the collision is rare (fresh effort dir per `/wayfind` isolates by default), git is the recovery net, and any code guard has incomplete coverage (can't reach the agent's `edit`-tool writes). Rejects atomic-write / detect-and-warn / `proper-lockfile` with rationale.
- [Routing-discriminator drift guard](tickets/05-routing-discriminator-drift-guard.md) — repo-level guard `bun-apps/tests/routing-contract.test.ts` (`test:routing`, `regression gates`): the 3 wayfind skills superpowers' bootstrap names (grilling/wayfinder/to-spec) must exist in `wayfind/skills/` AND appear in the bootstrap source (no-orphans/no-dead, mirroring ticket 03). Closes the wayfind-side gap (superpowers-side was already pinned by `bootstrap.test.ts`). **All 5 tickets closed — destination reached; frontier empty.**
