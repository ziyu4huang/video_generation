...
class RoPEModule(nn.Module):
    def __init__(
        ...
        if freqs_for == "pixel":
            ...
            self.freqs = 1.0 / (
                theta
                ** (
                    mx.arange(0, self.freq_dim, 2, dtype=mx.float32)[: self.freq_dim // 2]
                    / self.freq_dim
                )
            )
    ...
    def __call__(
        ...
        if txt_q is None or txt_k is None or txt_shape is None:
            return RoPEModule._apply_rope_3d(
                vid_q, vid_k, vid_shape, self.freqs, self.rope_dim, self.freqs_for,
            )
        return RoPEModule._apply_mm_rope_3d(
            vid_q, vid_k, vid_shape, txt_q, txt_k, txt_shape,
            self.freqs, self.rope_dim, self.freqs_for,
        )
    ...
    def _apply_rope_3d(
        ...
        if freqs_for == "pixel":
            for b in range(vid_shape.shape[0]):
                ...
                vid_freq = RoPEModule._get_axial_freqs(
                    freqs, f, h, w, freqs_for=freqs_for
                ).reshape(-1, freqs.size * 2 * rope_dim)
    ...
    def _apply_mm_rope_3d(
        ...
        if freqs_for == "pixel":
            ...
            txt_freqs_1d = RoPEModule._get_axial_freqs(
                freqs, min(max_txt_len + 16, 1024), freqs_for=freqs_for
            )
            for b in range(vid_shape.shape[0]):
                f, h, w = int(vid_shape[b, 0]), int(vid_shape[b, 1]), int(vid_shape[b, 2])
                txt_len = int(txt_shape[b, 0])
                vid_freq = vid_freqs_full[txt_len : txt_len + f, :h, :w].reshape(
                    -1, vid_freqs_full.shape[-1]
                )
    ...
    def _get_axial_freqs(cls, freqs: mx.array, *dims: int, freqs_for: str = "lang") -> mx.array:
        ...
        return mx.concatenate(broadcasted, axis=-1)
    ...
    def _apply_rotary_emb(freqs: mx.array, t: mx.array) -> mx.array:
        ...
        t_transformed = (t_middle_f * cos_freqs) + (
            RoPEModule._rotate_half(t_middle_f) * sin_freqs
        )
