...
class WindowPartitioner:
    ...
    def _make_windows(
        ...
        wh = math.ceil(resized_h / num_windows[1])
        ww = math.ceil(resized_w / num_windows[2])
        wt = math.ceil(min(t, 30) / num_windows[0])
            ...
            for ih in range(nh):
                ...
                for it in range(nt):
                    ...
                    if t_end <= t_start:
                        continue
                    windows.append(
                        (slice(t_start, t_end), slice(h_start, h_end), slice(w_start, w_end))
                    )
        return windows
    ...
    @staticmethod
    def _unflatten_list(tensor: mx.array, shapes: mx.array) -> list[mx.array]:
    ...
    def _window_partition(
        ...
        for x in unflattened:
            t_v, h_v, w_v = x.shape[0], x.shape[1], x.shape[2]
            slices = window_fn((t_v, h_v, w_v))
            window_counts.append(len(slices))
            for st, sh, sw in slices:
                windowed.append(x[st, sh, sw])
    ...
    def _create_window_indices(
        ...
        windowed_idx, window_shapes, window_counts = WindowPartitioner._window_partition(
            idx, shape, window_fn
        )
