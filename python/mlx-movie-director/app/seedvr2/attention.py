...
class MMAttention(nn.Module):
    ...
    def __call__(self, vid, txt, vid_shape, txt_shape):
        ...
        qkv_vid = self.proj_qkv_vid(vid.reshape(-1, vid.shape[-1])).reshape(
            -1, 3, self.heads, self.head_dim
        )
        qkv_txt = self.proj_qkv_txt(txt.reshape(-1, txt.shape[-1])).reshape(
            -1, 3, self.heads, self.head_dim
        )
        ...
        # 2. Normalize
        q_vid = self.norm_q_vid(qkv_vid[:, 0])
        k_vid = self.norm_k_vid(qkv_vid[:, 1])
        v_vid = qkv_vid[:, 2]
        q_txt = self.norm_q_txt(qkv_txt[:, 0])
        k_txt = self.norm_k_txt(qkv_txt[:, 1])
        v_txt = qkv_txt[:, 2]

        # 3. Repeat text for windows
        counts, txt_len = partitioner.window_counts, txt_shape[:, 0]
        qkv_t_rep = self._repeat_text_for_windows(
            mx.stack([q_txt, k_txt, v_txt], axis=1), txt_len, counts
        )
        ...
        if self.rope_on_text:
            ...
            q_vid, k_vid = self.rope(
                vid_q=q_vid, vid_k=k_vid, vid_shape=partitioner.window_shapes,
            )
