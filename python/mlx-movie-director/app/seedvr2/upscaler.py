"""SeedVR2 upscale utilities — image preprocessing, LAB color correction, wavelet reconstruction.

Ported from mflux's seedvr2_util.py.
"""
...
def preprocess_image(
    ...
    """Resize image to target resolution, apply softness, return MLX array + true dims."""
    w, h = image.size

    # Parse resolution — can be int (pixels for shortest side) or float (scale factor)
    if isinstance(resolution, float) and resolution < 100:
        # Scale factor like 2.0, 3.0
        scale = resolution
    else:
    ...
    # Softness: map 0.0-1.0 to downscale factor 1.0-8.0
...
def apply_color_correction(
    ...
    """Apply LAB color transfer + wavelet reconstruction for faithful color correction."""
    return _lab_color_transfer_exact(content, style, luminance_weight=luminance_weight)
...
def _lab_color_transfer_exact(
    ...
    content_np = _wavelet_reconstruction(content_np, style_np)
    ...
    matched_a = _hist_match(c_lab[..., 1], s_lab[..., 1])
    matched_b = _hist_match(c_lab[..., 2], s_lab[..., 2])

    if luminance_weight < 1.0:
        matched_L = _hist_match(c_lab[..., 0], s_lab[..., 0])
...
def _wavelet_decomposition(image: np.ndarray, levels: int = 5) -> tuple[np.ndarray, np.ndarray]:
    ...
    for i in range(levels):
        radius = 2**i
        low_freq = _wavelet_blur(cur, radius)
...
def _wavelet_reconstruction(content: np.ndarray, style: np.ndarray) -> np.ndarray:
    ...
    content_high, _ = _wavelet_decomposition(content, levels=5)
    _, style_low = _wavelet_decomposition(style, levels=5)
...
def _lab_to_rgb(lab: np.ndarray) -> np.ndarray:
    ...
    return _linear_to_srgb(rgb_lin).astype(np.float32)
