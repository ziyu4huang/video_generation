...
def run(args: "argparse.Namespace") -> None:
    ...
    try:
        ...
        raise FileNotFoundError(f"run config not found: {args.file}")
    except Exception as exc:
        raise ValueError(f"loading run config: {exc}") from exc
    ...
    if run_config.command in ("generate", "refine", "text2img", "img2img"):
        ...
        raise ValueError(f"replay not supported for command '{run_config.command}'")
