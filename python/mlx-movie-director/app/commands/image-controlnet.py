...
def _execute_generation(prompt, ref_image_path, ctrl_type, strength,
    ...
    # NOTE: VAE.encode() ALREADY applies Flux shift+scale internally
    # (vendor/mflux/src/mflux/models/z_image/model/z_image_vae/vae.py:24):
    #   latent = (mean - shift_factor) * scaling_factor
    # where shift_factor=0.1159, scaling_factor=0.3611.
    # Do NOT re-apply _FLUX_SHIFT_FACTOR/_FLUX_SCALE_FACTOR here —
    # doing so would double-normalize and squash the control signal to 36% range,
    # causing raw/continuous control signals to vanish (only canny survived).
...
_SELF_TEST_PROMPT = (
    "A young asian woman standing, arms at her sides in a natural relaxed pose, "
    "facing the camera, wearing casual clothes, clean white background, "
    "studio lighting, high quality portrait photography."
)

# Reference image is GENERATED by the pipeline (not downloaded).
# Using a different subject than the test prompt ensures we can distinguish
# ControlNet pose transfer from style/image copying.
_SELF_TEST_REF_PROMPT = (
    "full body portrait of a caucasian man in his 30s, both arms raised high above "
    "his head in a Y-pose (like victory pose), facing the camera, wearing "
    "white t-shirt and blue jeans, plain gray background, soft neutral lighting"
)
...
def _run_self_test(args):
    ...
    Steps:
      1. Generate Y-pose reference image via pipeline (caucasian man, distinct from prompt)
    ...
    # ── Step 1: Generate reference image (Y-pose, caucasian man) ──────────
    ref_path = os.path.join(cfg.OUTPUT_DIR, "selftest-ref-standing.jpg")
    if not os.path.exists(ref_path):
        print(f"\n[Self-Test] Generating reference image via pipeline (caucasian man, natural pose)...")
        ref_pil = _generate_baseline(
            prompt=_SELF_TEST_REF_PROMPT,
            out_w=640, out_h=960,
            steps=9, seed=42,
        )
        ref_pil.save(ref_path)
        print(f"  Saved: {ref_path} ({ref_pil.size[0]}×{ref_pil.size[1]})")
    else:
...
def _generate_self_test_html(html_path, results, ref_image):
    ...
    # Embed reference image as base64 data URI so the HTML is self-contained
    ref_path = Path(html_path).parent / ref_image
    ref_data_uri = ""
    if ref_path.exists():
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .ref-section {{ background: var(--card); border: 1px solid var(--border); border-radius: 12px;
    margin-bottom: 16px; overflow: hidden; }}
  .ref-section.collapsed .ref-body {{ display: none; }}
  .ref-header {{ padding: 10px 16px; cursor: pointer; display: flex; justify-content: space-between;
    align-items: center; user-select: none; }}
  .ref-header:hover {{ background: rgba(255,255,255,0.03); }}
  .ref-header h2 {{ font-size: 1em; }}
  .ref-chevron {{ transition: transform 0.2s; font-size: 0.8em; }}
  .ref-section.collapsed .ref-chevron {{ transform: rotate(-90deg); }}
  .ref-body {{ padding: 0 16px 16px; }}
  .ref-img-wrap {{ width: 100%; max-height: 400px; overflow: hidden; border-radius: 8px;
    background: #111; text-align: center; }}
  .ref-img-wrap img {{ max-width: 100%; max-height: 400px; object-fit: contain; }}
  .ref-caption {{ font-size: 0.82em; color: var(--muted); margin-top: 8px; line-height: 1.5; }}
...
<div class="ref-section">
  <div class="ref-header" onclick="this.parentElement.classList.toggle('collapsed')">
    <h2 data-en="📷 Reference Image (Pose Source)" data-zh="📷 參考圖（姿勢來源）"></h2>
    <span class="ref-chevron">▼</span>
  </div>
  <div class="ref-body">
    <div class="ref-img-wrap">
      <img src="{ref_data_uri}" alt="Reference">
    </div>
    <p class="ref-caption" data-en="This reference image (caucasian man) is used as ControlNet conditioning. The test prompt generates an asian woman — compare how each setting transfers the pose without copying the subject's identity."
       data-zh="此參考圖（白人男性）作為 ControlNet 條件輸入。測試 prompt 生成亞洲女性 — 比較各設定如何傳遞姿勢而不複製主體身份。"></p>
  </div>
