...
// Load colocated per-workflow knowledge (committed .knowledge.jsonl) — model-manifest
// gotchas/patterns. Injected into the inventory prompt so recurring manifest issues
// surface during the scan.
const _ma_knowledge = await loadKnowledge(KB_FILE)
const _ma_knowledgeDigest = _ma_knowledge?.digest || ""
const kbBlock = _ma_knowledgeDigest ? `## Prior knowledge (from past runs)\n${_ma_knowledgeDigest}\n\n` : ""
...
const inventory = await agent(
  `${kbBlock}Scan the model directory at ${MODELS_DIR} and build a complete inventory.
...
await extractKnowledge(KB_FILE, TIMESTAMP, _ma_histEntry.result, null);
