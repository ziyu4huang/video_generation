...
4. CROSS-FILE DATA CONTRACT: if the finding claims a reader/writer, producer/consumer, or serialization asymmetry ACROSS files (e.g. "X reads key K but the writer stores it under Y"), open BOTH sides before flagging:
   ...
   (c) SAMPLE REAL DATA — but BOUNDED: identify the EXACT data directory the reader opens (read its code for the path constant — e.g. an OUTPUT_DIR / MODELS_DIR), then sample AT MOST a few files with: find <that-exact-dir> -maxdepth 3 -name '*.json' | head -5. HARD RULE: NEVER run find/grep/ls on the repo root, the parent dir (..), the home dir, or any directory containing a models/, node_modules/, or venv subtree — this repo has a ~135GB content-addressed model store + thousands of output files, and an unbounded find HANGS the whole run (confirmed 2026-06-21: a routine scan ran 2h because verify agents ran 'find ..'). If you cannot cheaply locate the exact data dir from the reader's code, SKIP the data sample — do NOT guess with a broad find.
...
4. CROSS-FILE DATA CONTRACT: if the finding claims a reader/writer or serialization asymmetry ACROSS files (reader reads key K, writer stores under Y), open BOTH sides. Grep the reader for the EXACT data-source path it opens (a specific OUTPUT_DIR/MODELS_DIR constant), then sample AT MOST a few REAL files with a BOUNDED command: find <that-exact-dir> -maxdepth 3 -name '*.json' | head -5   (then cat one) and check whether the reader ACTUALLY mishandles them. HARD RULE: NEVER find/grep/ls on the repo root, the parent dir (..), home, or any dir containing a models/, node_modules/, or venv subtree — this repo has a ~135GB model store and an unbounded find HANGS the run (2026-06-21: a routine scan ran 2h from verify agents running 'find ..'). If the exact dir isn't cheaply locatable, SKIP the sample. REJECT (upheld=false) unless you can show a CONCRETE real file that fails. If the writer deliberately keeps a compat field (flat mirror / shim / fallback) OR the reader normalizes (JSON.parse on a stringified value, a ?? fallback, a string-vs-object branch), the contract is INTACT — that is a FALSE POSITIVE. Contract claims backed only by code-shape inference (no real sample) are FALSE POSITIVES.
