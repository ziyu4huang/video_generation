// MLX Movie Director Image Review & Optimize — scoped code review + argparse integrity + auto-fix for the run.py "image" command
//
...
//
//   FULL REVIEW (default):
//     Workflow({ name: "mlx-movie-director-image-review-optimize" })
...
export const meta = {
  name: "mlx-movie-director-image-review-optimize",
  description: "Multi-dimensional code review + adversarial verification + auto-fix for the run.py 'image' command (t2i/i2i/controlnet/faceswap/anime2real + flux2/lens/seedvr2 pipelines), scoped to the image-command surface",
  whenToUse: "Review and harden the run.py image command code: app/commands/image*.py + image generation pipelines (flux2/lens/seedvr2). Run before committing image-command changes, after adding an image sub-action, or for periodic image-pipeline health review",
  phases: [
...
const WORKFLOW_NAME = "mlx-movie-director-image-review-optimize"
const PYTHON_DIR = `${PROJECT_ROOT}/python/mlx-movie-director`
const OUT_DIR     = `${PYTHON_DIR}/output`
...
// reflection.json. CRITICAL: the load-knowledge agent MUST carry the schema below.
...
// Runs at Persist, AFTER saveHistory. Keep it CURATED and SMALL (not a run dump).
...
// Image-scoped argparse smoke gate: the run.py "image" command + its sub-actions
// (the surface this workflow modifies). A regression here = a broken image sub-command.
// Pre-existing --help failures are captured at baseline and filtered (not counted as
// regressions), so an image sub-action that already needs args is not a false alarm.
const SMOKE_COMMANDS = [
  "image", "image t2i", "image i2i", "image controlnet", "image review",
  "image faceswap", "image anime2real", "image quality", "image workflow",
  "image angle", "image profile", "image swap", "image expansion", "image purify",
]
const SMOKE_ALIASES = ["generate"]
...
if (shouldSkipScan && priorHistory?.scan) {
  ...
  const scanScope = targetFiles
    ...
    : `Scan the IMAGE-COMMAND code surface only: the run.py "image" dispatcher + its sibling sub-action modules (app/commands/image*.py, import-lora-image.py, _shared.py, _model_resolver.py) + the image generation pipelines (app/flux2_*pipeline*.py, app/lens_pipeline.py, app/pipeline.py, app/seedvr2/pipeline.py) + run.py + app/cli.py. Use the find command in step 1 — it is scoped to exactly this surface.${effort === "high" ? " Also include app/tests/test_image_*.py and app/tests/test_*pipeline*.py." : " Skip tests."}`
  ...
  For each file:
  1. Run: Bash("cd '${PYTHON_DIR}' && { find app/commands -name 'image*.py'; find app/commands -name 'import-lora-image.py'; find app/commands -name '_shared.py'; find app/commands -name '_model_resolver.py'; find app -maxdepth 1 -name 'flux2_*pipeline*.py'; find app -maxdepth 1 -name 'lens_pipeline.py'; find app -maxdepth 1 -name 'pipeline.py'; find app/seedvr2 -name 'pipeline.py'; echo run.py; echo app/cli.py; } 2>/dev/null | sort -u")
...
// ── Run-log manifest → ./output/ (the per-run deliverable for THIS pipeline) ──────
// A compact, self-describing run log dropped alongside the generation outputs, so a
// cold reader (or future dashboard) sees what this image-code-review run did without
// grepping history/. Mirrors the generation <base>.manifest.json convention in output/.
const RUNLOG_MANIFEST = {
  manifest_version: 1,
  kind: "image-code-review-run",
  run_id: RUN_ID,
  workflow: WORKFLOW_NAME,
  started_at: RUN_TIMESTAMP,
  scope: {
    target: targetFiles ? "explicit" : "image-command",
    files: [...filesTouched].filter((f) => f.includes("mlx-movie-director/")).map((f) => f.replace(/^.*mlx-movie-director\//, "")),
  ...
  review: {
    raw_findings: allFindings.length + suppressedFromPrior.length,
    verified: verifiedFindings.length,
    suppressed_from_prior: suppressedFromPrior.length,
  ...
  fixes: doFix
    ? {
        ...
        skipped: skippedCount,
        failed: failedCount,
        files_changed: fixResults.filesChanged,
        regressions: reVerifyFindings.length,
        items: fixResults.fixes.map((f) => ({ id: f.findingId, status: f.status, change: (f.change || "").slice(0, 120) })),
      }
    : { mode: "review-only", applied: 0, skipped: 0, failed: 0, files_changed: [], regressions: 0 },
  verification: {
    baseline: baselineTestResults,
    smoke: smokeTestResults,
    new_regressions: reVerifyFindings.length,
    baseline_filtered: true,
  },
  restore: restoreResult,
  git: {
    head_before: checkpointResult?.headSha || "",
    stash_ref: checkpointResult?.stashRef || "",
    dirty_before: checkpointResult?.dirtyBefore || [],
  },
  phases_completed: phasesCompleted,
  phases_failed: phasesFailed,
  status: phasesFailed.length === 0 ? "complete" : "partial",
}
const runlogPath = `${OUT_DIR}/image_review_${RUN_ID}.manifest.json`
const runlogJson = JSON.stringify(RUNLOG_MANIFEST, null, 2)
const runlogWrite = await agent(
    `Write the image-code-review run-log manifest to disk RELIABLY.
1. Bash("mkdir -p '${OUT_DIR}'")
2. Remove any existing file first (the Write tool refuses to overwrite without a prior Read, silently leaving stale content): Bash("rm -f '${runlogPath}'")
3. Write the file with the Write tool: file_path='${runlogPath}', content is the JSON below — paste it VERBATIM, do not summarize or truncate:
${runlogJson}
4. Verify it landed: Bash("test -s '${runlogPath}' && echo OK || echo MISSING")
5. If step 4 printed MISSING, rewrite via a quoted heredoc (no expansion):
   Bash("cat > '${runlogPath}' <<'RUNLOG_EOF'
${runlogJson}
RUNLOG_EOF")
6. Bash("wc -c < '${runlogPath}'")
Return { written: true, bytes: <the number printed by wc> }.`,
  { label: "write-runlog-manifest", phase: "Persist", model: "haiku",
    schema: { type: "object", properties: { written: { type: "boolean" }, bytes: { type: "number", description: "Byte count printed by wc -c" } }, required: ["bytes"] } },
)
const runlogBytes = Number(runlogWrite?.bytes) || 0
log(runlogBytes > 0 ? `Run-log manifest: ${runlogBytes} bytes → ${runlogPath}` : `WARNING: run-log manifest write FAILED (0 bytes) → ${runlogPath}`)
