...
| mlx-movie-director-image-review-optimize | ✓ | ✓ | — | canonical |
...
All thirteen workflows own a committed `.claude/workflows/<wf>.knowledge.jsonl`,
...
2026-06-18 all thirteen are `load ✓ / extract ✓`.

- **Full mirror (shared `saveHistory`):** `gui-movie-director-review-optimize`,
  `mlx-movie-director-review-optimize`,
  `mlx-movie-director-image-review-optimize` (scoped variant of review-optimize for the
  run.py `image` command surface; adds a `./output/` run-log manifest per run),
  ...
  `mlx-movie-director-models-assistant`, `mlx-movie-director-video-assistant`.
- **Light variant (inline persist, `!DRY_RUN`-guarded extract, no `saveHistory`):**
  ...
  — build-system self-improvers; extract distills recurring drift/coverage patterns.

Five files are seeded from real history (109 records); eight are 0-byte placeholders
...
5. The whole-workflow-merge temptation is a dead end: both candidate pairs
