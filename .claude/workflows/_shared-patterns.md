...
All twelve workflows own a committed `.claude/workflows/<wf>.knowledge.jsonl`,
...
2026-06-18 all twelve are `load ✓ / extract ✓`.

- **Full mirror (shared `saveHistory`):** `gui-movie-director-review-optimize`,
  `mlx-movie-director-review-optimize`, `mlx-movie-director-run-self-improve-image`,
  ...
  `mlx-movie-director-ltx-self-improve` (its `Knowledge` phase was refactored to call
...
Five files are seeded from real history (109 records); seven are 0-byte placeholders
