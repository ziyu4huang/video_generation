...
- Persists-history schema grouped by normalized text. A group of size 1 is flagged
  ...
  - **Minimal** `{written:boolean, required:["written"]}` — `schema-self-improve`,
    which verifies via `test -s … && echo OK` and only gates on `.written`.
- Helper coverage matrix: which workflows define `saveHistory` / `markPhase` /
  `reliableWrite`. Absence is reported, not enforced — `schema-self-improve`
  inlines its persist logic without the shared helpers.
...
| mlx-movie-director-lora-review | ✓ | ✓ | — | canonical |
...
All eight workflows own a committed `.claude/workflows/<wf>.knowledge.jsonl`,
...
2026-06-19 all eight are `load ✓ / extract ✓`.
...
- **Full mirror (shared `saveHistory`):** `gui-movie-director-review-optimize`,
  ...
  `gui-movie-director-self-improve`, `mlx-movie-director-lora-review` (unified:
  reviews flux2-klein-9b LoRAs with multi-lane routing, OR zimage-turbo LoRAs T2I-only,
  selected by `args.arch`),
...
- **Light variant (inline persist, `!DRY_RUN`-guarded extract, no `saveHistory`):**
  `gui-movie-director-schema-self-improve` — build-system self-improver; extract
  distills recurring drift/coverage patterns.

Six files are seeded from real history (140 records); two are 0-byte placeholders
that fill organically on first real run. State is tracked in `MANIFEST.md`
...
5. The whole-workflow-merge temptation: merges work ONLY when the pair shares the
   same `run.py` command shape. The `kind` merge (t2i+i2i) and the lora-review merge
   (flux2-klein↔zimage-turbo → unified by `args.arch`, done 2026-06-19) both met that
   precondition. The mlx↔gui review-optimize pair was vetted and rejected — they differ
   at the generation/test layer AND live in separate apps (mlx Python CLI vs Bun GUI).
   Do not re-attempt a merge without that shared-command-shape precondition.
