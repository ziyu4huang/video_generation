...
description: >-
  Use when a task matches a skill, script, or CLI already developed in this repo's s2-agent ext packages (bun-apps/s2-agent-ext-*), or re-implementing devops/research/movie-director/wayfind/superpowers tooling that likely exists. Trigger situations — git sync/branch/PR/merge/CI (devops-workflow + *-cli.ts); an arc CLOSING ("done", "hands off", session ending, PR merged, milestone verified): read self-reflect-next-goal and WRITE the successor next-goal BEFORE reporting; "hands on" / "hands on next goal" / "continue" after a close-out: EXECUTE the queue head in-session (successor still written at every ticket boundary); multi-ticket efforts (.planning tickets/frontier/seed/spec): route via wayfind (ask-matt / to-spec / to-tickets) then executing-plans; new features (brainstorming); bugs/flakes (systematic-debugging). Ext skills are Markdown docs at bun-apps/s2-agent-ext-*/skills/<name>/SKILL.md (READ them; name = the dir, not the package), via list-ext-skills.ts resolve <name>.
---
...
here), so the gates below ARE the trigger layer — CLAUDE.md points here. — in s2-agent boots this gate is advertised alongside the ext skill descriptions, and routing through it is the intended model-side entry (ruled 2026-09-10, spwf-ab-closing t06). On ANY of these
situations, READ + follow the named ext SKILL.md first; never hand-roll a substitute.

| Gate | Keywords / situation | Read + follow |
|---|---|---|
| **Hands-off** | "done", "that's it", session ending/stopping, PR merged, milestone verified, task finished — ANY arc close-out | `self-reflect-next-goal` **WRITE**: FIRST push the session's work (clean tree, changes committed on a feature branch and pushed — never leave "commit my changes" to the successor), then write a validator-passing successor `output/next-goal-<ts>.md` (strict v2), re-point `LATEST-next-goal.md`, then report. The `session-closeout-sop` skill is the command-exact runbook twin. Reporting done WITHOUT the successor file is the #1 violation — it silently drops the carry. |
| **Hands-on** | "hands on", "hands on next goal", "do the next goal", "continue" (after a close-out) | `self-reflect-next-goal` **EXECUTE**: sync the tree, run the queue head end-to-end through its Done-when gate; at a ticket boundary write the successor. **"continue" after a close-out = the successor's queue head IN-session** (D2) — same discipline, the boundary successor is written at every ticket boundary too, and the loop ends when the queue drains. |
| **Git** | sync/update main, branch, rebase, PR, squash-merge, CI/typecheck before merge, verify scope | `devops-workflow` + its `*-cli.ts` (throw-free, JSON, exit 0/1/2); a CLI twin exists for every owned phase — when none does, use the `./s2-agent.sh -p` bridge — never raw-bash `git`/`gh` for owned phases; "is main green?" → `main-health-cli.ts`. |
| **Tickets/effort** | `.planning/<effort>` work, tickets, frontier, `/wayfind seed`, spec, multi-ticket build | wayfind family — route via `ask-matt` when unsure; `to-spec` then `to-tickets` (**Execution order confirm-gate** after seed — present order, ask confirm-or-rechoose, record the map's `Execution order` line) → `executing-plans` per queue head. `issue-tracker` for the gh issue/label/wayfinder-map surface. |
| **Issues/triage** | gh issue/PRD create-read-list, labels, comments, wayfinder map/child/frontier/claim/resolve ops | `issue-tracker` — the gh CLI conventions + label↔role mapping (`bun-apps/s2-agent-ext-devops/skills/issue-tracker/SKILL.md`). |
| **Naming/rename** | naming a new tool/extension/skill, or renaming an existing one | `extension-naming` — per-surface style, rename checklist, append-only history; renames are behavior changes (PR #1738 pattern). |
| **Shared state** | touching config/resolution consumed by >1 package — vault root, MLX models dir, venv, bun workspace root, sibling forks | `shared-state-index` — verified resolvers (`file#symbol`) + silent-drift risks. |
| **Tool quirk** | s2-agent toolchain behavior looks wrong (commitScope flags, skill collisions, boot hangs) — check before hand-rolling a workaround | `learnings` — dated quirk/insight/convention log; append an entry when a quirk is confirmed. |
...
| **Domain words** | fuzzy term, glossary, hard-to-reverse decision; navigating the doc ladder (L1/L2/L3/𝒵) or citing ADRs | `domain-modeling` — `CONTEXT.md` + ADRs (`ADR-<context>-NNNN`); `domain-docs` — the progressive-disclosure doc map + ADR citation rules (`bun run test:adr` enforces). |
...
- Treating "continue" (after a close-out) as "invent whatever": the successor's Immediate steps ARE the queue head — continue = execute it in-session with the same discipline, writing that next boundary's successor BEFORE reporting.
