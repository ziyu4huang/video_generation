...
| `pi-dynamic-workflows/tests/{workflow-manager,run-persistence,task-panel}.test.ts` | `startedAt/updatedAt/completedAt: new Date().toISOString()` fixture fields | **controlled (benign seed)** — record-shape fixtures; tests assert structure/ordering, not "now" |
| `pi-dynamic-workflows/tests/workflow-{parser,runtime}.test.ts` | `Date.now()`/`new Date()` only inside **string literals** fed to the parse-time determinism guard (which REJECTS them) | **controlled** — these tests *enforce* determinism (the guard rejects `Date.now`/`Math.random` in workflow scripts); the literal is the test input, not a clock |
