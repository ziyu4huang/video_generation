...
`bun-apps/pi-agent-flux2/src/binary.ts` runs `build-metallib.sh` automatically
...
## 6b. pi-agent-flux2 — agent tool wrapper

`bun-apps/pi-agent-flux2/` (a sibling Bun workspace package) exposes flux2 as a
...
`bun-apps/pi-agent-flux2/src/result.ts` therefore takes the output path from
