...
//  Flux2DirectorCLI
//
//  `flux2 edit` — image editing / i2i via Flux2KleinEdit reference conditioning.
//
...
extension Flux2CLI {
    struct Edit: ParsableCommand {
        static let configuration = CommandConfiguration(
            commandName: "edit",
            abstract: "Edit/generate via Flux2KleinEdit reference image conditioning."
        )

        @OptionGroup var globals: GlobalOptions

        @Option(help: "Text prompt describing the edit.")
        var prompt: String

        @Option(help: "Reference image path(s), comma-separated for multi-ref.")
        var images: String
        ...
        func run() throws {
            setbuf(stdout, nil)
            globals.apply()
            let imagePaths = images.split(separator: ",").map {
                URL(fileURLWithPath: String($0).trimmingCharacters(in: .whitespaces))
            }
            print("flux2 edit — Flux2KleinEdit reference conditioning")
            print("  prompt     : \(prompt)")
            print("  images     : \(imagePaths.map { $0.lastPathComponent }.joined(separator: ", "))")
            ...
            let (pixels, elapsed) = pipeline.generate(
                prompt: prompt, imagePaths: imagePaths, seed: seed,
            ...
            if !noArtifacts {
                let runConfig = RunConfig(
                    ...
                    quantBits: 8, quantGroupSize: 64, command: "edit", pipeline: "flux2")
