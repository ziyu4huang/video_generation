...
//  LTXVideoDirector
//
//  Milestone 2a+2b+2c of the native CFG/STG/modality guidance port (see
//  docs/native-i2v-dev-variant-study.md's "Status update (2026-07-07)"
//  section) — classifier-free guidance, STG (spatio-temporal guidance,
//  self-attention perturbation on specific blocks), and modality guidance
//  (cross-modal A2V/V2A attention isolation).
//
...
public enum CFGGuidance {
    ...
    /// True iff `modalityScale` differs from 1.0 (reference: `MultiModalGuider
    /// .do_isolated_modality_generation`, `not math.isclose(modality_scale, 1.0)`).
    public static func isModalityActive(modalityScale: Float) -> Bool {
        abs(modalityScale - 1.0) > 1e-6
    }
    ...
    /// Full `MultiModalGuider.calculate` blend:
    /// `cond + (cfgScale-1)*(cond-uncond) + stgScale*(cond-uncondPerturbed)
    ///      + (modalityScale-1)*(cond-uncondModality)`,
    /// optionally rescaled to preserve `cond`'s variance. `uncondPerturbed`
    /// is ignored when `stgScale == 0`, and `uncondModality` is ignored when
    /// `modalityScale == 1.0` (matches the reference: each term drops out
    /// and its forward pass need not even run).
    public static func blend(
        ...
        uncondModality: MLXArray? = nil, modalityScale: Float = 1.0
    ) -> MLXArray {
        ...
        if isModalityActive(modalityScale: modalityScale), let uncondModality {
            pred = pred + (modalityScale - 1) * (cond - uncondModality)
        }
        if rescaleScale != 0 {
