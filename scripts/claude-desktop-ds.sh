#!/usr/bin/env bash
# claude-desktop-ds.sh — configure Claude Desktop to use DeepSeek as inference backend.
#
# Edits ~/Library/Application Support/Claude-3p/configLibrary/{_meta,<uuid>}.json
# and restarts Claude Desktop. macOS only.
#
# Usage:
#   ./scripts/claude-desktop-ds.sh          # configure to DeepSeek and restart
#   ./scripts/claude-desktop-ds.sh reset    # remove DeepSeek entry (revert to Anthropic)
#   ./scripts/claude-desktop-ds.sh status   # show current applied config
#   ./scripts/claude-desktop-ds.sh -h       # help
#
# Prerequisites:
...
#   - DEEPSEEK_API_KEY set in env, or exported in ~/.zshrc / ~/.bashrc
#
...
ENTRY_NAME="claude-desktop-ds"
BASE_URL="https://api.deepseek.com/anthropic"
AUTH_SCHEME="bearer"
# DeepSeek does server-side mapping — send claude names, backend routes to DeepSeek:
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    status)
      ...
      ;;
    *)
      echo "claude-desktop-ds: unknown argument: $1" >&2
...
preflight() {
  if [[ "$(uname)" != "Darwin" ]]; then
    echo "claude-desktop-ds: macOS only." >&2
    exit 1
  fi
  if [[ ! -d "/Applications/Claude.app" ]]; then
    echo "claude-desktop-ds: /Applications/Claude.app not found. Install from https://claude.ai/download." >&2
    exit 1
  fi
  if [[ ! -x "/usr/bin/jq" ]]; then
    echo "claude-desktop-ds: /usr/bin/jq not found (required on macOS Sonoma+)." >&2
    exit 1
  fi
  ...
  if [[ ! -f "$dev_settings" ]] \
    ...
    cat >&2 <<'EOF'
claude-desktop-ds: Developer Mode not enabled in Claude Desktop.
...
try
  set theKey to text returned of (display dialog ¬
    "DEEPSEEK_API_KEY not found in env or shell rc.\nPaste your DeepSeek API Key:" ¬
    default answer "" ¬
    with hidden answer ¬
    with title "claude-desktop-ds")
  return theKey
on error
  return ""
end try
APPLESCRIPT
  ...
  if [[ -z "$key" ]]; then
    echo "claude-desktop-ds: no DeepSeek API Key provided. Aborting." >&2
    exit 1
  fi
  printf '%s' "$key"
}
...
# ── reset: remove claude-desktop-ds entry and clear appliedId ────────────────

cmd_reset() {
  preflight
  if [[ ! -f "$META" ]]; then
    echo "claude-desktop-ds: nothing to reset (_meta.json not found)."
    exit 0
  fi
  ...
  if [[ -z "$uuid" ]]; then
    echo "claude-desktop-ds: entry '$ENTRY_NAME' not found in _meta.json. Nothing to do."
    exit 0
  fi

  confirm_or_abort "remove DeepSeek entry '$ENTRY_NAME' from Claude Desktop config and restart."
  ...
  restart_claude
  echo "Done. DeepSeek entry removed. Claude Desktop is restarting."
}
...
Models configured (DeepSeek maps these to its own models on the backend):
  main : claude-opus-4-5    → deepseek-v4-pro
  fast : claude-haiku-4-5   → deepseek-v4-flash
...
Commands:
  ./scripts/claude-desktop-ds.sh status  # inspect active config
  ./scripts/claude-desktop-ds.sh reset   # revert to Anthropic
EOF
    ;;
esac
